...
- `current_fact`: bounded AutoRecall candidate-provenance observability was implemented and committed at `1cd183ff12d055ba5c5ecd4bd0d9d1b98cdff23b`; focused/privacy tests passed `19/19`, and the Node 24 full suite passed `1835`, failed `0`, skipped `8`. The active runtime still has the pre-provenance bytes for the two target files.
- `current_fact`: the first provenance runtime-install transaction closed `STOPPED` during candidate construction before Gateway stop or plugin install. The candidate path `provenance-overlay-1cd183ff-20260808` exists as a byte-identical sealed clone of the rollback release; overlay writing never began because copied target files retained mode `0400`. Active runtime/config remained unchanged and the failed candidate is immutable evidence.
- `accepted_design`: retrieval thresholds, `topK`, query shaping, FTS/vector/recent policy, fusion/ranking, Card/gate policy, capture behavior, and AutoRecall rollout remain frozen. The next bounded decision is one R2 runtime retry using a new fixed candidate path and a construction-only permission repair: temporarily add owner-write to the two copied target files, overlay the `1cd183ff...` bytes, restore mode `0400`, prove an exact two-file delta, then perform the same non-live install/identity/privacy verification. Full-HEAD installation remains excluded.

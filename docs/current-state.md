...
- `accepted_design`: retrieval thresholds, `topK`, query shaping, FTS/vector/recent policy, fusion/ranking, Card/gate policy, capture behavior, and AutoRecall rollout remain frozen. The next bounded product decision is a minimal runtime publication/install of only the provenance delta over the current verified rollback release, followed by non-live runtime identity and telemetry-contract verification. Full-HEAD installation is excluded because it would also deploy an unrelated currently-uninstalled `normalize-candidate.js` semantic change.

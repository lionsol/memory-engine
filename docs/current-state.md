...
- `historical_record`: natural-canary retry R2 also closed `STOPPED`. It successfully enabled the exact scoped AutoRecall config and produced technically valid bounded provenance, scope isolation, and exact config restoration, but no genuine Sol question was submitted: the harness ran through `bash -s <<'EOF'` and plain `read` consumed remaining heredoc script text, sending `STAGE_RESULT="INSUFFICIENT_EVIDENCE"` as the first prompt. That accidental prompt is not natural-use evidence.
- `accepted_design`: Sol explicitly approved one final R3 anti-drift exception to complete the same natural-use product question. Retrieval thresholds, query shaping, FTS/vector/recent policy, fusion/ranking, Card/gate policy, capture behavior, and broad AutoRecall rollout remain frozen. R3 reuses the exact existing dedicated `main` session, captures a fresh event baseline before enablement, requires all natural questions to be read from `/dev/tty`, temporarily uses `AutoRecall=true` with `topK=1` and exact agent/session allowlists, and restores the exact pre-canary config. No R4 harness retry is authorized.

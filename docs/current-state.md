...
- `accepted_design`: retrieval thresholds, `topK`, query shaping, FTS/vector/recent policy, fusion/ranking, Card/gate policy, capture behavior, and AutoRecall rollout remain frozen. The next bounded product change is source-only observability: add minimal bounded, privacy-safe per-channel candidate provenance plus sanitized fusion pre/post candidate provenance without changing retrieval behavior. A frozen implementation Stage Card does not authorize coding or runtime deployment.

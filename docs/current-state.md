...
- `accepted_design`: retrieval thresholds, query shaping, FTS/vector/recent policy, fusion/ranking, Card/gate policy, capture behavior, and broad AutoRecall rollout remain frozen. The next bounded decision is one execution-mechanics-only natural-canary retry that reuses the already-created dedicated `main` session, captures a fresh event baseline before any enablement, fixes only verifier/closeout mechanics, temporarily uses `AutoRecall=true` with `topK=1` and exact agent/session allowlists, then restores the exact pre-canary config.

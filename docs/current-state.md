...
- `accepted_design`: retrieval thresholds, query shaping, FTS/vector/recent policy, fusion/ranking, Card/gate policy, capture behavior, and broad AutoRecall rollout remain frozen. The next bounded decision is one dedicated-session natural-use canary with temporary `AutoRecall=true`, `topK=1`, `agentAllowlist=["main"]`, and one exact new `sessionAllowlist`; its only product question is whether real natural AutoRecall execution persists the new bounded provenance without scope leakage, followed by exact config restoration.

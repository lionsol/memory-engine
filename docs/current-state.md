...
- `historical_record`: a historical/pre-existing tool-policy observation is independent of L2: `tools.catalog` registration is complete for `memory_engine`, `memory_engine_search`, and `memory_engine_get`, while `tools.profile=coding` causes `tools.effective` to filter those tools from the `main` model. This has no causal relationship to L2 DB boundary qualification. No `alsoAllow` or config change is authorized; treat it as an independent future product-policy decision.

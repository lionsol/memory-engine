...
- `accepted_design`: retrieval thresholds, `topK`, Card/gate policy, capture behavior, and AutoRecall rollout remain frozen. The next bounded product decision is a read-only first-loss-boundary attribution over only the seven proven `RETRIEVAL_SELECTION_GAP` turns; it must not treat T1 or T3 as retrieval-selection evidence and is not execution-authorized merely by being documented.

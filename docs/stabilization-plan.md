...
**1.1-QA0 Identity & Lifecycle Correctness Gate is `NOW / NOT STARTED`.** Before Q0, run a bounded source-level correctness gate so identity and lifecycle defects cannot be misclassified as recall failures. QA0 must verify:
...
QA0 does not create a new `memory_entries` aggregate, retune ranking/vector thresholds, rewrite the retrieval pipeline, activate event-time writes, or mutate live DB/Gateway/config/LanceDB/plugin state. Its output is focused regression coverage and a decision on whether Q0 can proceed.

**1.1-Q0 Production Recall Failure Review is `THEN / NOT STARTED`.** Freeze `30–50` real production failure cases and classify the first loss as `WRITE_MISS`, `TRIGGER_MISS`, `CANDIDATE_MISS`, `RANK_MISS`, or `USE_MISS`. Each case must record at least query, expected memory and answer support, whether memory existed, whether recall was attempted, whether a candidate was generated and at what rank, whether it was disclosed, whether the answer used it, and the final failure class. Q0 must report first-loss distribution and decide whether the first product repair belongs to write, trigger, candidate generation, ranking, disclosure, or answer use.

This roadmap acceptance does not itself authorize access to private production cases, runtime telemetry creation, AutoRecall enablement, or any Gateway/config/DB/LanceDB/live-plugin mutation. `AutoRecall=false` and production `topK=3` remain unchanged. Exact Q0 data sources and any live-state read require a separately bounded Owner decision.
...
#### memory-engine 1.1 Q1–Q3

**1.1-Q1 Product Metric Contract** freezes small-budget product metrics before algorithm work: Recall-any@3, Recall-all@3, NDCG@3, first relevant rank, answer-evidence coverage@3, cross-session complete-evidence coverage, stale/conflicting evidence in top 3, unnecessary-recall and context-pollution rates, and p50/p95 latency. Recall@50 and large candidate-pool coverage remain diagnostics, not standalone production success criteria. Add an end-to-end replay contract spanning conversation → extraction → Canonical Memory → projection/index → recall decision → candidates → ranking → disclosure → answer; gold evidence stays evaluator-only and must not enter Add, Search, embedding, query planning, ranking, or answer prompts.

**1.1-Q2 Minimal Channel Ablation** is `NEXT / NOT STARTED` after Q0/Q1. Keep the matrix bounded:

| Profile | Decision purpose |
|---|---|
| Lexical only | Frozen low-latency baseline |
| Vector only | Independent semantic capability |
| Lexical + Vector | Existing B4 hybrid comparison |
| Lexical + Metadata | Entity/project/temporal marginal value |
| Lexical + Session/Episode expansion | Cross-session and multi-evidence recovery |
| Full candidate fusion | Channel complementarity |
| Lexical → selective Vector fallback | Quality/latency production candidate |

Report preference, multi-session, temporal, knowledge-update, single-session-user, and single-session-preference families separately. Primary outputs are @3 metrics, first-rank and case-level improved/regressed/unchanged counts, p50/p95 latency, vector invocation rate, and marginal gain per expensive call. Do not expand this into `2^N` combinations, broad embedding-model contests, unconditional multi-query, large Recall Hint grids, or Recall@50-only comparisons. Closed RH1/H2 experiments are not rerun.

**1.1-Q3 Retrieval Architecture Decision** follows Q2. The candidate architecture is lexical + metadata first, candidate-quality assessment, selective semantic fallback, session/episode reconstruction, lightweight rerank, then injection of `3–5` evidence items. Metadata/entity/temporal/session features may expand candidates or provide soft ranking signals; they are not unqualified hard-filter or recall/skip authority. Architecture acceptance precedes source implementation, offline qualification, and any separately authorized runtime canary.

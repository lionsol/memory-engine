...
The current B5 checkpoint is **B5-I2c = `PASS / REPO-TESTED`**. The lexical
v2 profile is `production_hybrid_lexical_dialog_locomo_time_frozen_v2`, and its
baseline is **`NEXT / NOT RUN / NOT AUTHORIZED`**. The future semantic pair is
`production_hybrid_semantic_dialog_locomo_time_frozen_v2`; semantic v2 is
**`HOLD`**. The historical B5-S1 v1 record and metrics remain preserved with
`TIME PROVENANCE INCOMPLETE` and are not strict semantic A/B authority. The
v2 vector-attempt invariant is pending its confidence distribution, the six
numerical gates remain frozen but suspended, and B5 overall remains **`OPEN`**.
...
6. **B5 — Cross-dataset conversational generalization: `B5-I1/I2/I2a/I2b REPO-TESTED; B5-I2c PASS / REPO-TESTED; LEXICAL V2 BASELINE NEXT / NOT RUN / NOT AUTHORIZED; SEMANTIC V2 HOLD; B5 OVERALL OPEN`**. The LoCoMo dataset/metric contract and isolated lexical retrieval runner use pinned upstream/dataset provenance, strict and separately named canonicalized evidence policies, conversation-owned temporary Core/Engine/FTS corpora, dialog-level primary metrics, session projection compatibility metrics, the fixed `locomo_dialog_projection_v1` format with session date plus non-empty captions, and one resolved benchmark time for the complete dataset run. The historical B5-S1 v1 execution and metrics remain preserved, but its clock provenance is incomplete and it is not strict semantic A/B authority. The time-frozen lexical v2 source is repo-tested but its baseline has not run; the future semantic pair is `production_hybrid_lexical_dialog_locomo_time_frozen_v2` and `production_hybrid_semantic_dialog_locomo_time_frozen_v2`. Use LoCoMo to test longer conversational/persona/event memory rather than retune LongMemEval-specific rules.

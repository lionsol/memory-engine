...
6. **B5 — Cross-dataset conversational generalization: `I1/I2/I2a REPO-TESTED; B5-S1 RETRIEVAL BASELINE NOT RUN; QUALITY OPEN`**. The LoCoMo dataset/metric contract and isolated lexical retrieval runner use pinned upstream/dataset provenance, strict and separately named canonicalized evidence policies, conversation-owned temporary Core/Engine/FTS corpora, dialog-level primary metrics, session projection compatibility metrics, and the fixed `locomo_dialog_projection_v1` format with session date plus non-empty captions. No official retrieval baseline or quality adjudication has run; use LoCoMo to test longer conversational/persona/event memory rather than retune LongMemEval-specific rules.

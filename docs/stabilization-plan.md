...
1. **Independent `REDACTED_CARD` evaluation evidence** — `NEXT / SEPARATE DECISION / NOT AUTHORIZED BY C.5`; evaluate only against an independent handcrafted or separately frozen design, without tuning the C.1 holdout.
2. **`INTERNAL_AGENT_CONTEXT` design for raw/tool output** — `LATER / NOT STARTED`; a separate design decision is required, and C.5 does not authorize implementation.

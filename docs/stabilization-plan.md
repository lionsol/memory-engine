...
1. **D.3-C.7 pure independent `REDACTED_CARD` evaluator** — `NEXT / NOT AUTHORIZED BY C.6`; it must consume the frozen C.6 contract without tuning the C.1 holdout or reusing C.5 unit literals.
2. **D.3-C.8 execute the C.6 holdout once** — `LATER / NOT STARTED`; requires the separate C.7 evaluator decision and remains offline evidence only.
3. **`INTERNAL_AGENT_CONTEXT` design for raw/tool output** — `LATER / NOT STARTED`; a separate design decision is required, and C.6 does not authorize implementation.
4. **D.3-C product interpretation** — `LATER / SEPARATE DECISION`; use accepted offline evidence without inferring runtime readiness or relaxing capability.
5. **D.3-D production/runtime integration** — `LATER / NOT AUTHORIZED`; requires a separate product decision and explicit runtime authorization after D.3-C evidence.

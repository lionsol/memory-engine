...
native module imports, while B6-S1 remains **`RETRY READY / PHASE C→D
...
DOCKER RUNTIME CLOSURE QUALIFIED`; B6-I3 `CLOSED`** after I2a semantic
backend/resource ownership, I2b retained-root restart/reconciliation, the
benchmark-local HTTP/Docker transport, and the bounded R2/R3 Docker packaging
corrections. B6-S1 is `RETRY READY / PHASE C→D REMAINING` and uses no real
provider.

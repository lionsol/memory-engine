...
1. **D.3-C.11 Redaction Evidence Resolution Semantics** — `NEXT / CANDIDATE / NOT AUTHORIZED BY C.10`; multiple claims, conflicts, authority precedence, detector-evidence isolation, and stale-evidence handling only.
2. **`INTERNAL_AGENT_CONTEXT` design for raw/tool output** — `LATER / NOT STARTED`; a separate design decision is required, and C.9 does not authorize implementation.
3. **D.3-C product interpretation** — `LATER / SEPARATE DECISION`; use accepted offline evidence without inferring runtime readiness or relaxing capability.

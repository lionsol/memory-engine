...
`B5-I1 LoCoMo dataset/metric contract = REPO-TESTED / CONTRACT FROZEN`; the
retrieval baseline remains `NOT RUN`. Benchmark evidence is not production
authority.

The current B5 checkpoint includes **B5-I2b benchmark time provenance
hardening = `REPO-TESTED`**. B5-S1 preflight is **`STOP / EXECUTION_COUNT=0/1 /
BASELINE NOT RUN`**, and the full lexical baseline requires fresh Owner
authorization after the new source HEAD. B5 quality adjudication remains
...
6. **B5 — Cross-dataset conversational generalization: `I1/I2/I2a/I2b REPO-TESTED; B5-S1 PREFLIGHT STOP / EXECUTION 0/1 / RETRIEVAL BASELINE NOT RUN; QUALITY OPEN`**. The LoCoMo dataset/metric contract and isolated lexical retrieval runner use pinned upstream/dataset provenance, strict and separately named canonicalized evidence policies, conversation-owned temporary Core/Engine/FTS corpora, dialog-level primary metrics, session projection compatibility metrics, the fixed `locomo_dialog_projection_v1` format with session date plus non-empty captions, and one resolved benchmark time for the complete dataset run. No official retrieval baseline or quality adjudication has run; B5-S1 requires fresh Owner authorization after the new source HEAD. Use LoCoMo to test longer conversational/persona/event memory rather than retune LongMemEval-specific rules.

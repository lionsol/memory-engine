...
1. **D.3-C.8 Planner adjudication** — `NEXT`; review the one-shot bounded C.6 representation evidence without inferring capability or production authority.
2. **`INTERNAL_AGENT_CONTEXT` design for raw/tool output** — `LATER / NOT STARTED`; a separate design decision is required, and C.8 does not authorize implementation.

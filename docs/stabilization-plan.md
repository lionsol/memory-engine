...
1. **D.3-C.7 pure independent `REDACTED_CARD` evaluator** — `SOURCE IMPLEMENTED / FROZEN HOLDOUT NOT YET EXECUTED`; tests use only handcrafted C7 material and the evaluator has no capability/selector/runtime authority.
2. **D.3-C.8 execute the C.6 holdout once** — `NEXT / NOT AUTHORIZED BY C.7`; requires a separate execution decision and remains offline evidence only.

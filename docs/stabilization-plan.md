...
6. **B5 — Cross-dataset conversational generalization: `I1 REPO-TESTED / CONTRACT FROZEN; RETRIEVAL BASELINE NOT RUN`**. The LoCoMo dataset/metric contract is now implemented with pinned upstream/dataset provenance, strict and separately named canonicalized evidence policies, dialog-level primary metrics, and session projection compatibility metrics. The retrieval runner and quality baseline remain later work; use LoCoMo to test longer conversational/persona/event memory rather than retune LongMemEval-specific rules.

...
is **`REPO-TESTED`**; real-provider sanity and semantic full A/B remain
**`NOT RUN / NOT AUTHORIZED`**. B5 quality adjudication and B5 overall remain
**`OPEN`**. Benchmark evidence is not production authority.
...
6. **B5 — Cross-dataset conversational generalization: `B5-I1/I2/I2a/I2b REPO-TESTED; B5-S1-v2 PASS_WITH_FINDINGS / BASELINE FROZEN / CLOSED; B5-I3 SOURCE IMPLEMENTATION REPO-TESTED; B5 OVERALL OPEN`**. The LoCoMo dataset/metric contract and isolated lexical retrieval runner use pinned upstream/dataset provenance, strict and separately named canonicalized evidence policies, conversation-owned temporary Core/Engine/FTS corpora, dialog-level primary metrics, session projection compatibility metrics, the fixed `locomo_dialog_projection_v1` format with session date plus non-empty captions, and one resolved benchmark time for the complete dataset run. The historical B5-S1 v1 execution and metrics remain preserved, but its clock provenance is incomplete and it is not strict semantic A/B authority. The time-frozen lexical v2 baseline is now frozen with vector attempted/skipped/error `1,978/0/0`; the six numerical gates and regression guards compare semantic v2 against lexical v2 without threshold changes. The semantic profile `production_hybrid_semantic_dialog_locomo_time_frozen_v2` has source status `REPO-TESTED`; real-provider sanity and semantic full A/B are `NOT RUN / NOT AUTHORIZED`, B5 quality adjudication remains `OPEN`, and B5 overall remains `OPEN`. Use LoCoMo to test longer conversational/persona/event memory rather than retune LongMemEval-specific rules.
7. **B6 — External controlled comparison: `LATER`**. Add Agent Memory Leaderboard Add/Search compatibility for cross-system comparison under AML's own held-out evaluator.

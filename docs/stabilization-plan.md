...
The final baseline remains conservative: AutoRecall and card-first runtime are
disabled, active attestations are `0`, and the accepted H2 deployed source is
`755ff5d396102c5a93baacf2d3bf6187a4fea713`. OpenSpec 4.4 remains unchecked and
raw disclosure remains out of scope.
...
Non-blocking follow-ups retained outside 4.3 are: external `null` confidence
coercion (`EXTERNAL_NULL_CONFIDENCE_COERCED_TO_ZERO`), the parallel explicit
memory-tool disclosure boundary (`EXPLICIT_MEMORY_TOOL_DISCLOSURE_BOUNDARY_NOT_UNIFIED`),
and the resolved EDi agent-ID baseline correction from `edi` to `main`
(`AUTO_RECALL_AGENT_ID_DEFAULT_MISMATCH`). They do not create a new stage or
authorize 4.4.
...
`PASS / SOURCE IMPLEMENTED / REPOSITORY-TESTED`, and the separate runtime
qualification is `PASS / RUNTIME QUALIFIED / CLOSED`. No 4.4 work is authorized
by this closeout.

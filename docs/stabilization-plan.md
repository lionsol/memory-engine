...
hardening = `REPO-TESTED`** and **B5-S1 = `PASS_WITH_FINDINGS / BASELINE
FROZEN / CLOSED`**. B5 overall remains `OPEN`; the next semantic A/B is source
...
6. **B5 — Cross-dataset conversational generalization: `I1/I2/I2a/I2b REPO-TESTED; B5-S1 PASS_WITH_FINDINGS / BASELINE FROZEN / CLOSED; B5 OVERALL OPEN`**. The LoCoMo dataset/metric contract and isolated lexical retrieval runner use pinned upstream/dataset provenance, strict and separately named canonicalized evidence policies, conversation-owned temporary Core/Engine/FTS corpora, dialog-level primary metrics, session projection compatibility metrics, the fixed `locomo_dialog_projection_v1` format with session date plus non-empty captions, and one resolved benchmark time for the complete dataset run. The lexical baseline is now recorded; the next semantic A/B remains source inspection/design only and is not authorized to run. Use LoCoMo to test longer conversational/persona/event memory rather than retune LongMemEval-specific rules.

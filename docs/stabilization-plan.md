...
4. **D.3-D.1 DIRECT_CARD Production Boundary Migration Design** — `PASS_WITH_FINDINGS`; direct-card source migration is `HOLD` because `PRODUCTION_SAFE_TO_DISCLOSE_AUTHORITY_MISSING`. The design is docs/OpenSpec-only and does not authorize source work.
5. **D.3-D.2 DIRECT_CARD Safe-to-Disclose Authority Decision** — `CANDIDATE / NOT AUTHORIZED`; decide who produces production `safe_to_disclose` authority and how it binds exact Canonical Memory source to the `DISCLOSURE_CARD` projection.

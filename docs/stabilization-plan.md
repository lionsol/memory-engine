...
**Phase D.3 Memory Projection Architecture** is now active. D.3-A closed the architecture contract, D.3-B is `SOURCE IMPLEMENTED / VERIFIED`, D.3-C.1 is **`HOLDOUT CONTRACT FROZEN`**, D.3-C.2 is **`SOURCE IMPLEMENTED`**, D.3-C.3 first-run evidence is **`ACCEPTED BY PLANNER FOR ARCHITECTURE INTERPRETATION`**, and D.3-C.4 is **`PROJECTION STRATEGY TAXONOMY DEFINED`**. The immutable 24-row run produced projection valid `24/24`, validity expectation match `24/24`, surface safe `20/24`, answer-bearing semantic preserved `8/12`, useful projection `6/12`, CARD-authorized useful projection `2`, and capability-blocked useful projection `4`; it supports a current card-representation gap in specific families while preserving the projection/authorization boundary. The full bounded evidence is recorded in `reports/memory-projection-holdout-v1-first-run-20260822.md`, and the accepted taxonomy is in `docs/memory-projection-strategy-taxonomy-v1.md`. No production or runtime mutation is authorized.

### NEXT

Phase 2.5 Canonical Memory Architecture is closed. No further D/D.1 deployment or qualification work remains. Intent-aware Recall v2-B and the successor Selective Recall Gate deterministic-authority experiment are closed: the full semantic route and skip-only C1 route are rejected, and v2-B6/C2-C repair are cancelled. Retrieval-first Selective Disclosure D.2-C completed its capability contract and offline v1.1 shadow evaluation. D.3-A and D.3-B are closed at architecture/source level; D.3-C.1 is **`HOLDOUT CONTRACT FROZEN`**, D.3-C.2 is **`SOURCE IMPLEMENTED`**, D.3-C.3 evidence is **`ACCEPTED BY PLANNER FOR ARCHITECTURE INTERPRETATION`**, and D.3-C.4 is **`PROJECTION STRATEGY TAXONOMY DEFINED`**. **NEXT: a separately bounded `REDACTED_CARD` offline prototype decision, not authorized by C.4**; the D.2 and D.3-C.1 holdouts remain immutable and were not tuned against. `ADD_SYNC_BACKFILL_SCOPE_FINDING` remains a non-blocking operational/lifecycle finding outside D.1 scope. No D.3 runtime authority is granted.
...
1. **`REDACTED_CARD` offline prototype decision** — `NEXT / CANDIDATE / NOT AUTHORIZED BY C.4`; first priority for a separately bounded representation-research decision.
2. **`INTERNAL_AGENT_CONTEXT` design for raw/tool output** — `LATER / NOT STARTED`; a separate design decision is required, and C.4 does not authorize implementation.
3. **D.3-C product interpretation** — `LATER / SEPARATE DECISION`; use the accepted offline evidence without inferring runtime readiness or relaxing capability.
4. **D.3-D production/runtime integration** — `LATER / NOT AUTHORIZED`; requires a separate product decision and explicit runtime authorization after D.3-C evidence.
5. **Learned/statistical recall policy** — `LATER / NOT STARTED`; requires sufficient real labeled traffic and a separate Planner decision.
6. **Recall Hint**.
7. **Statistical LTR**.

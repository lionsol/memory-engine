...
5. **D.3-D.2 DIRECT_CARD Safe-to-Disclose Authority Decision** — `PASS / DECISION CLOSED`; v1 positive authority is `OWNER_EXPLICIT_ATTESTATION` with exact Canonical Memory / `DISCLOSURE_CARD` binding, but production source migration remains `HOLD` because `PRODUCTION_DISCLOSURE_ATTESTATION_PROVIDER_NOT_IMPLEMENTED`.
6. **D.3-D.3 DIRECT_CARD Owner-Attested Authority Source Implementation** — `CANDIDATE / NOT AUTHORIZED`; a separate decision is required before implementing the dedicated attestation store, authenticated non-agent management boundary, binding validator, capability provider, or focused tests.
6. **Learned/statistical recall policy** — `LATER / NOT STARTED`; requires sufficient real labeled traffic and a separate Planner decision.
7. **Recall Hint**.
8. **Statistical LTR**.

...
1. **Phase D.3-C.3 Frozen projection-aware evaluation** — `NEXT / NOT STARTED`. Run the pure evaluator once against the frozen D.3-C.1 holdout; do not tune projector, capability, selector, or fixture.
2. **D.3-C product interpretation** — `LATER / NOT STARTED`; use the offline evidence for a separate bounded product decision without inferring runtime readiness.

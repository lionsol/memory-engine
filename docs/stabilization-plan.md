...
**memory-engine Benchmark v1 B2 is `SOURCE IMPLEMENTED / OFFLINE ONLY / AWAITING OFFICIAL DATASET RUN`**. B1 provides LongMemEval schema normalization and evaluator-only metrics; B2 adds the `production_hybrid_lexical_session_v1` isolated retrieval profile, materializing each question into temporary Core/Engine SQLite plus FTS state and calling production `hybridSearch()` without live OpenClaw/memory-engine paths or fabricated vector results. The next benchmark action is the first scored `longmemeval_s_cleaned.json` run with `top_k=50` and exact dataset SHA-256 provenance. A real semantic/vector profile, LoCoMo, and AML remain later integrations; benchmark work does not authorize runtime/config/DB mutation.

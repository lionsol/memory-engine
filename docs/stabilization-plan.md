...
6. **B5 — Cross-dataset conversational generalization: `I1/I2 REPO-TESTED; RETRIEVAL BASELINE NOT RUN`**. The LoCoMo dataset/metric contract and isolated lexical retrieval runner are implemented with pinned upstream/dataset provenance, strict and separately named canonicalized evidence policies, conversation-owned temporary Core/Engine/FTS corpora, dialog-level primary metrics, and session projection compatibility metrics. No official retrieval baseline or quality adjudication has run; use LoCoMo to test longer conversational/persona/event memory rather than retune LongMemEval-specific rules.

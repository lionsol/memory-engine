...
The current Benchmark v1 B6 route is now **B6-I1 `CLOSED`; B6-I2a `CLOSED`; B6-I2b `SOURCE IMPLEMENTATION REPO-TESTED / CLOSED`; B6-I2 `CLOSED`; B6-I3-R2 `SOURCE IMPLEMENTATION REPO-TESTED / DOCKER BUILD QUALIFIED`; B6-I3 `CLOSED`; B6-S1 `RETRY READY / PHASE B→D REMAINING / NO REAL PROVIDER`; B6-S2 official AML smoke `NOT AUTHORIZED`; B6-S3 full `NOT AUTHORIZED`**. This route records source/package readiness only and does not authorize a public service, provider request, or official AML evaluation.
...
The current benchmark decision is **B6-I3-R2 `SOURCE IMPLEMENTATION REPO-TESTED /
DOCKER BUILD QUALIFIED`; B6-I3 `CLOSED`** after I2a semantic backend/resource
ownership, I2b retained-root restart/reconciliation, the benchmark-local
HTTP/Docker transport, and the bounded native dependency builder correction.
B6-S1 is `RETRY READY / PHASE B→D REMAINING` and uses no real provider.

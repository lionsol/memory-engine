...
**memory-engine Benchmark v1 B1 is `IMPLEMENTED / OFFLINE ONLY`**. The LongMemEval adapter now validates the public dataset schema, normalizes Add/Search envelopes without leaking gold answers/evidence into Search, and computes deterministic session-level retrieval metrics. B2 is the next bounded benchmark source task: isolated temporary Core/Engine/index state that reuses production retrieval semantics without touching active OpenClaw/memory-engine data. LoCoMo and AML remain later benchmark integrations; benchmark work does not authorize runtime/config/DB mutation.

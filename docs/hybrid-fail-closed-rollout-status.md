...
| B8-A7.3 read-only health monitor and stop contract | IMPLEMENTED / REVIEW CHANGES REQUIRED | Review of implementation checkpoint `b725dd5` found that `authorized_at` does not bound observations, future-dated evidence produces negative ages that are treated as fresh, external monitor timestamps are not canonical UTC ISO, and a scheduled-healthcheck row that cannot be produced by the trusted registry can still satisfy healthcheck freshness and removal readiness. |
| B8-A7 sustained production evidence window | DESIGN AUTHORIZED / RUNTIME NOT AUTHORIZED | A7.3 requires temporal-boundary and scheduled-healthcheck evidence fixes plus final review. No 30-day full-mode or sustained AutoRecall configuration is authorized. |
...
The A7.3 temporal review fixes are implemented and review-pending. `authorized_at` now bounds the shared observation input for identity, continuity, fallback, and full-rollout evaluation; pre-authorization and post-`asOf` rows are reported as stop conditions rather than silently discarded. All external timestamps use the canonical UTC ISO contract, future evidence is never fresh, and healthcheck freshness requires the complete trusted registry evidence contract. `B8-A7.3 REVIEW FIXES IMPLEMENTED / REVIEW PENDING`; the sustained runtime window and B8-B remain unauthorized.

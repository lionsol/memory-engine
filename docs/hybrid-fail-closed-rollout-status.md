...
| B8-A7.1 evidence epoch and deployment identity | REVIEW FIXES IMPLEMENTED / THIRD REVIEW CHANGES REQUIRED | Runtime dependency hashing, effective retrieval configuration, AutoRecall `topK`, environment thresholds, and malformed field handling are substantially closed. Final review still found three fail-closed gaps: a malformed high-priority `autoRecall` object is silently skipped in favor of a lower-priority config and may enable AutoRecall while remaining `valid=true`; the supported Recent canary single-value `token` alias is dropped by normalization; and the dependency-closure guard skips non-`lib/` subdirectories while the injected `fileEntries` identity path does not require declared root runtime files. A7.2 must not begin until these residual gaps are fixed and re-reviewed. |
| B8-A7 sustained production evidence window | DESIGN AUTHORIZED / RUNTIME NOT AUTHORIZED | Stage 4 proved controlled wiring, but A7.1 remains under implementation review and A7.2 has not started. Continuity, traffic-origin, and sustained health evidence are not authorized or complete. A7.1–A7.3 review remains required before any 30-day full-mode or sustained AutoRecall configuration is authorized. |
...
The B8-A7.1 dependency/config closure fixes are implemented but third-review changes remain before A7.2. The final follow-up must reject a malformed higher-priority `autoRecall` source instead of silently activating lower-priority config, preserve the Recent canary single-value `token` compatibility alias, and make both dependency-closure validation paths require every declared root runtime dependency. Stage 4's temporary `autoRecall.enabled=true`, `agentAllowlist=["edi","main"]`, and dual `full_fail_closed` configuration do not authorize sustained production use or any long-running runtime configuration change. The preceding ledger state was `REVIEW FIXES IMPLEMENTED / REVIEW PENDING`.

...
| B8-A7.1 evidence epoch and deployment identity | REVIEW FIXES IMPLEMENTED / REVIEW PENDING | Runtime identity now covers the declared root runtime dependency scope, AutoRecall preserves the effective `memoryEngine.recall.topK` fallback, retrieval-sensitive recall/ranking/confidence and effective environment thresholds are fingerprinted, and malformed compatibility values invalidate evidence identity. A7.2 must not begin until final review accepts the dependency/config closure. |
...
The B8-A7.1 dependency/config closure fixes are implemented and remain review-pending before A7.2. Runtime identity covers the declared root-level runtime dependencies, including `query-utils.js`, and `lib/**`; the normalized config preserves effective AutoRecall fallback values, fingerprints retrieval-sensitive config plus non-secret effective environment thresholds, and invalidates malformed compatibility values. Stage 4's temporary `autoRecall.enabled=true`, `agentAllowlist=["edi","main"]`, and dual `full_fail_closed` configuration do not authorize sustained production use or any long-running runtime configuration change. The preceding ledger state was `IMPLEMENTED / SECOND REVIEW CHANGES REQUIRED`.

...
| B8-A7.1 evidence epoch and deployment identity | IMPLEMENTED / SECOND REVIEW CHANGES REQUIRED | The first three findings were fixed, but the second implementation review found that root-level runtime dependencies such as `query-utils.js` remain outside the build hash, the normalized resolver changes the legacy `memoryEngine.recall.topK` fallback behavior, and retrieval-sensitive `memoryEngine.recall` / `ranking` values can change without changing the rollout fingerprint. Invalid compatibility AutoRecall values are also not consistently marked invalid. A7.2 must not begin until the identity covers the actual runtime dependency and effective retrieval-config closure. |
...
Fix and re-review B8-A7.1 before beginning A7.2 or making any long-running runtime configuration change. The first review findings around required files, symlinks, and the initial config-source split are fixed, but the identity contract still must cover the actual root-level runtime dependency closure and the effective retrieval configuration consumed by Hybrid Search. The resolver must preserve or explicitly validate the legacy `memoryEngine.recall.topK` fallback instead of changing it from the configured value to the AutoRecall schema default, and invalid compatibility values must invalidate evidence identity rather than silently normalizing to a different runtime behavior. Stage 4's temporary `autoRecall.enabled=true`, `agentAllowlist=["edi","main"]`, and dual `full_fail_closed` configuration do not authorize sustained production use.

...
| B8-A7-R2A existing OpenClaw metadata API | BLOCKED / REVIEW FIXES IMPLEMENTED | The current OpenClaw 2026.6.9 installed-plugin index is persisted in shared state SQLite; its helper opens the database without a read-only contract and the snapshot loader falls back to discovery. Existing API is blocked for Phase 0. |
| B8-A7-R2B standalone read-only state-DB reader feasibility | NOT STARTED | Future source audit only: readOnly/file-must-exist, WAL/SHM, schema, staleness, path, symlink, duplicate, zero-write, and zero-discovery checks. |

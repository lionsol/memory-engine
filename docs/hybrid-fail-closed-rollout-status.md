...
| B8-A7.1 evidence epoch and deployment identity | CLOSED / READY FOR A7.2 | Canonical observations can now carry an explicit configured evidence epoch, installed-runtime SHA-256 identity, and canonical rollout-config SHA-256 fingerprint. The report-only identity audit rejects pre-A7 rows, disabled evidence, missing identity fields, invalid provenance, and mixed epoch/build/config identities. |

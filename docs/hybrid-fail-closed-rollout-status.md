...
| B8-A7-R2B synthetic read-only state-DB feasibility harness | IMPLEMENTED / EDI VERIFICATION PENDING | Synthetic `node:sqlite` scenarios cover rollback journal, WAL/SHM, missing SHM, permissions, immutable URI behavior, SQL-write rejection, freshness, and filesystem fingerprints. This does not authorize a production reader or real state-DB access. |

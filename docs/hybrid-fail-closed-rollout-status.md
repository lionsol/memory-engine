...
| B8-A7-R3A host-published metadata manifest synthetic contract | IMPLEMENTED / EDI CLOSURE PENDING | Synthetic canonical JSON, duplicate-key rejection, BOM/NUL rejection, permission rejection, atomic old/new generation assertions, tombstone, symlink/hardlink identity checks, and zero-consumer-write evidence are covered. This does not authorize host integration or production consumption. |

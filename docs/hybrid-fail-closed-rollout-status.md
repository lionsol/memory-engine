...
| B8-A7.2 continuity and traffic-origin evidence | IMPLEMENTED / REVIEW CHANGES REQUIRED | Implementation checkpoint `59a4f3e` adds origin metadata and a report-only continuity evaluator, but review found four blockers: the current OpenClaw `before_tool_call` contract does not expose the trigger/source hints consumed by the resolver, arbitrary origin-evidence objects can be counted as natural, per-surface leading/trailing boundary gaps are omitted, and zero threshold overrides can make empty evidence report `continuity_ready`. |
...
B8-A7.1 is closed after final review of implementation checkpoint `caf4373`. The accepted contract binds observations to one explicit evidence epoch, reviewed runtime dependency identity, and normalized effective rollout/retrieval configuration. Higher-priority malformed `autoRecall` values fail closed, Recent single-token canary compatibility is preserved, and dependency-closure validation requires every declared local runtime dependency. B8-A7.2 implementation checkpoint `59a4f3e` is not review-closed. Stage 4's temporary `autoRecall.enabled=true`, `agentAllowlist=["edi","main"]`, and dual `full_fail_closed` configuration still do not authorize sustained production use or any long-running runtime configuration change.

Current implementation state: `B8-A7.2 IMPLEMENTED / REVIEW CHANGES REQUIRED`; sustained runtime window and B8-B remain unauthorized. The follow-up must align tool-origin classification with the actual host `before_tool_call` contract, validate origin evidence rather than accepting any object, include global-window boundary gaps in per-surface continuity, and make empty or missing-surface evidence impossible to classify as ready regardless of threshold overrides.

...
| B8-A7-R2B synthetic read-only state-DB feasibility harness | EXPERIMENTAL EVIDENCE VALID / ASSERTION ALIGNMENT IMPLEMENTED / EDI CLOSURE PENDING | Node v24.8.0, module ABI 137, and SQLite 3.50.4 produced PASS for missing-database and rollback-journal, but existing SHM modification or SHM creation blocked the WAL scenarios. The normal reader advanced B to C; immutable retained checkpointed-A and remains ineligible. The standalone live state-DB reader is formally blocked by the zero-write/freshness decision. |

...
| B8-A7-R2B synthetic read-only state-DB feasibility harness | REVIEW FIXES IMPLEMENTED / EDI VERIFICATION PENDING | Synthetic `node:sqlite` scenarios use exact A/B revision markers, BigInt nanosecond fingerprints, exception-path after snapshots, WAL/SHM permissions, four SQL write probes, and explicit immutable URI evidence. This does not authorize a production reader or real state-DB access. |

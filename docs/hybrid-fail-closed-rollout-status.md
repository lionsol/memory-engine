...
| B8-A7.1 evidence epoch and deployment identity | IMPLEMENTED / REVIEW CHANGES REQUIRED | The identity fields and report-only audit are implemented, but review found three integrity gaps: `package.json` is optional instead of required, in-root symlinked directories under `lib/` are silently omitted from the build hash, and the rollout config fingerprint hashes a different source object than the multi-source configuration chain used by the runtime. A7.2 must not begin until these gaps are fixed and re-reviewed. |
| B8-A7 sustained production evidence window | DESIGN AUTHORIZED / RUNTIME NOT AUTHORIZED | Stage 4 proved controlled wiring, and A7.1 review fixes are implemented but review-pending; continuity, traffic-origin, and sustained health evidence are not authorized or complete. A7.2–A7.3 review remains required before any 30-day full-mode or sustained AutoRecall configuration is authorized. |
...
Fix and re-review B8-A7.1 before beginning A7.2 or making any long-running runtime configuration change. The follow-up must require `package.json`, fail closed or hash all in-root runtime symlink content, and fingerprint the same normalized effective configuration that controls AutoRecall and KG/Recent runtime modes. Stage 4's temporary `autoRecall.enabled=true`, `agentAllowlist=["edi","main"]`, and dual `full_fail_closed` configuration do not authorize sustained production use.

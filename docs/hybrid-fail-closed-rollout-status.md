...
| B8-A7-R2B synthetic read-only state-DB feasibility harness | SECOND REVIEW FIXES IMPLEMENTED / EDI VERIFICATION PENDING | Synthetic `node:sqlite` scenarios use exact A/B/C revision markers, post-open immutable mutation, location identity checks, phase-separated reader fingerprints, exception-path after snapshots, WAL/SHM permissions, and four SQL write probes. This does not authorize a production reader or real state-DB access. |

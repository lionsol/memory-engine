...
| B8-A7-R2B synthetic read-only state-DB feasibility harness | MODULE-BOUNDARY FIXES IMPLEMENTED / EDI RE-VERIFICATION PENDING | The prior EDI run failed before harness entry because of Node built-in import and bin CommonJS/ESM boundary errors. The CLI now lazy-loads the ESM library only after argument validation, and requiring it has no smoke side effect. This does not authorize a production reader or real state-DB access. |

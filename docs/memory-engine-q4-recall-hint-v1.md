...
Status: `PASS_WITH_FINDINGS / CLOSED / Q4-A CONTRACT FROZEN / Q4-B SOURCE CLOSED / Q4-C0 CLOSED / Q4-C1b DEVELOPMENT PASS / Q4-C2 NARROWED FRESH HOLDOUT PASS / LIMITED SYNTHETIC CANDIDATE RECOVERY REPRODUCED / FINAL TOP3 GAIN LIMITED / Q4-D CLOSED / Q4-MEASURED SYNCHRONOUS SHAPE DEFAULT NO-GO ON LATENCY / THIS REPORT GRANTS NO NEW RUNTIME AUTHORITY`
...
The frozen Q4 technical stop conditions are acceptance conditions, so this development-only run reports them as `NOT_EVALUABLE` solely because `acceptance_split_empty`. That is expected scope behavior, not a product failure. The later acceptance transaction is recorded below; AutoRecall/runtime/config/live Core/Engine/LanceDB/deployment/push/tag remain outside this evidence.

### Q4-C1b frozen acceptance transaction contract

The frozen acceptance transaction was designed to test whether the frozen Recall Hint producer plus frozen semantic/rerank profile reproduced the development candidate-recovery benefit on the untouched `32`-case acceptance split without protection regressions. No development tuning was permitted before that decision.

The acceptance execution was frozen as one transaction with no human tuning boundary between producer output and semantic scoring. The existing producer contract permits at most `32` acceptance producer requests (`2048` input / `256` output tokens per request; acceptance-only maxima `65,536` input and `8,192` output tokens). At the selected SiliconFlow non-discounted rates this has a theoretical acceptance-producer ceiling of `CNY 0.270336`; the frozen execution hard cap was `CNY 0.30`.

For semantic scoring, the untouched acceptance split has `32` original queries and Recall Hint permits at most `2` expansion queries per case. A conservative acceptance semantic envelope is therefore at most `72` synthetic canonical corpus embeddings + `32` original-query embeddings + `64` expansion embeddings = `168` embedding requests, plus `32` baseline and `32` Hint rerank requests = `64` rerank requests. Using the same maximum-token accounting as development, that bounds embedding cost at `$0.11010048`, rerank cost at `$0.15728640`, theoretical semantic total at `$0.26738688`; the frozen semantic hard cap was `$0.30`. Exact request ceilings may be tightened after the acceptance Hint outputs are generated inside the same transaction, but must never be increased above these precommitted maxima.

Acceptance egress must retain the existing boundaries: producer sees only acceptance current query plus bounded caller context; embedding sees only the `72` synthetic canonical projection texts, acceptance original queries and generated bounded expansion queries; rerank sees only acceptance original query plus bounded synthetic canonical candidate text. Gold IDs, case labels, retrieval results, full sessions, tool traces and live memory remain denied. Only temporary benchmark DB/Lance/cache/progress artifacts are permitted. No automatic retry, resume, replay, prompt/schema/model/query-plan/gate change, runtime mutation, deployment, push or tag is part of this packet.

The acceptance harness is now source-ready. It binds the frozen manifest and source commit into one `execution_binding_sha256`, runs the real semantic baseline before any producer call, stops with producer usage `0` if protection completeness or target-family headroom fails, then executes at most `32` producer calls followed immediately by the Hint semantic/rerank arm. The shared semantic session enforces one exact-input embedding cache across baseline and Hint, `168` embedding / `64` rerank hard ceilings, and the `$0.30` semantic cap. The CLI has a zero-egress `--preflight` mode and requires the exact execution binding for the real path. Immediately before the first provider-capable execution path it creates an exclusive attempt marker keyed by source HEAD; the marker is retained on success or failure, so the same bound transaction cannot automatically retry, resume or replay. Any failure is persisted as a stopped result when possible. Real acceptance egress was permitted only after separate explicit Owner authorization bound to the final clean source HEAD and preflight execution binding; that authorization was later consumed by the STOPPED transaction recorded below.

### Q4-C1b acceptance result and STOPPED review

Owner-authorized acceptance execution consumed exactly one transaction under source `afcac9b187aeaa2a1a0ff76dece0616a2d1916a7` and execution binding `9b53b850c0554cdf0a4edd4b8bc6b42443cde290e907306389676d75b7652cf4`. The harness stopped at `BASELINE_ELIGIBILITY` before any Hint producer call because standalone `temporal_relation` had no semantic candidate-recovery headroom. Producer usage remained exactly `0` requests / `0` tokens / `CNY 0`; the Hint semantic/rerank arm did not execute. Baseline semantic usage was `92` embedding requests and `32` rerank requests with conservative cost upper bound `$0.13893632`. The attempt marker is `CONSUMED`; retry, resume and replay under the same authorization are forbidden.

The semantic acceptance baseline was: `entity_reference` `3/8` `POOL_MISS`, pool coverage `0.625`, Recall-all@3 `0.375`; `temporal_relation` `0/8` `POOL_MISS`, pool coverage `1.0`, Recall-all@3 `0.25`; `multi_facet` `6/8` `POOL_MISS`, pool coverage `0.4375`, Recall-all@3 `0`; protection `0/8` `POOL_MISS`, pool coverage `1.0`, Recall-all@3 `1.0`. Therefore the execution outcome is **STOPPED / NO HINT-ON ACCEPTANCE QUALITY CLAIM**, not a Recall Hint quality failure. The consumed acceptance split must not be reused after changing target families or gates.

The STOPPED review narrows the **candidate-recovery qualification claim**, not the Recall Hint schema. Standalone `entity_reference` and `multi_facet` retain material candidate-generation headroom and remain the Q4 Recall Hint candidate-recovery targets. Standalone `temporal_relation` is removed from that target claim under the frozen semantic profile because its acceptance pool is already complete; its low Recall-all@3 (`0.25`) is a downstream ranking/final-serving problem. The `time_relation` Hint field remains in v1 as bounded supporting context for composite/entity/multi-facet queries and is not removed or retuned from this evidence.

Q4-D must not issue a final quality/cost rollout decision from the consumed split. A formal narrowed-scope acceptance claim requires a **fresh independent holdout** that was not used in this STOPPED transaction, with entity-reference and multi-facet as candidate-recovery targets and protection as a mandatory no-regression family. The frozen producer prompt/schema, semantic profile, candidate depth `20`, topK `3`, and reranker stay unchanged; no consumed-acceptance tuning is permitted. Q4-C2 now provides that fresh holdout as source-only evidence; real C2 provider egress remains separately unauthorized, so Q4-D quality/cost acceptance remains blocked and no AutoRecall/runtime/deployment authority follows.

### Q4-C2 fresh narrowed-scope holdout

Q4-C2 is an acceptance-only holdout rather than a new development split. It contains exactly `24` fresh synthetic cases and `32` fresh memory records: `8` `entity_reference`, `8` `multi_facet`, and `8` `protection`. It contains no standalone temporal family and reuses no C1/C1b case IDs, memory IDs, exact queries, or exact memory texts. Frozen identities are `corpus_sha256=cd4842fee339736f7eecd49ce525a3534fe971d83ff1d06fcc672ff3df113a13`, `holdout_sha256=64658f3827a682c9c4a0b18316a2e830f3d66081c1a7fc25eb853f263ee39282`, and `manifest_sha256=05adda04c8b9250cef4791995dbb03742155e177d881204c0d4e55224168b0f9`.

The zero-provider lexical-control preflight passes and is only corpus/headroom evidence: entity-reference has `6/8` `POOL_MISS`, multi-facet `8/8`, while protection remains `8/8` pool-complete and `8/8` Recall-all@3. Real semantic baseline eligibility remains mandatory before any producer call: both target families must have at least one semantic `POOL_MISS`, and protection must remain pool-complete with Recall-all@3 `1`.

The C2 acceptance gates are intentionally stronger than the consumed C1b aggregate gate because the product claim is now family-specific candidate recovery. For **each** target family, pool coverage must not decrease, `POOL_MISS` must strictly decrease by at least one case, Recall-any@3 and Recall-all@3 must not decrease, and aggregate paired Recall-all@3 improvements across the two target families must exceed regressions. Protection must show no pool-coverage, `POOL_MISS`, Recall-any@3, or Recall-all@3 regression.

C2 keeps the frozen C1b producer prompt/schema/model and the same production-aligned semantic/rerank profile. Its bounded one-transaction envelope is `24` producer requests, at most `49,152` producer input tokens and `6,144` output tokens, producer theoretical maximum `CNY 0.202752` with hard cap `CNY 0.23`; semantic scoring is bounded at `32` corpus embeddings + `24` original-query embeddings + at most `48` Hint-expansion embeddings = `104` embedding requests, plus `24` baseline + `24` Hint reranks = `48` rerank requests. The semantic theoretical maximum is `$0.18612224` with hard cap `$0.20`.

The source harness is wired through the same canonical projection, shared exact-input embedding cache, temporary SQLite/Lance materialization, production `hybridSearch()` vector-plan/RRF path, and offline canonical 0.6B rerank profile used by C1b. Local fake-provider verification proves the new C2 corpus can traverse that real product retrieval path; focused C1b/C2 tests pass without real provider egress. The C2 CLI binds the frozen holdout to a clean source commit, requires one exact `execution_binding_sha256`, writes one exclusive `CONSUMED` attempt marker before the provider-capable path, and forbids retry/resume/replay.

### Q4-C2 real holdout result

Owner-authorized C2 execution consumed exactly one transaction under source `d5f75bf95006c9557dd4289164c5e41c058cfc72` and execution binding `3f5fde38943125035adf0bc1bda7fbdeec9437418e461e3ea4e486e310829599`. The final result is **PASS**, result identity `bf809cd7cc15d737ccce36503f5a036e1e1987870ec558bee10c21f5a035b6d9`; the attempt marker is `CONSUMED`, so retry/resume/replay are forbidden.

The real semantic baseline eligibility also passed: entity-reference had `2/8` `POOL_MISS`, pool coverage `0.75`, Recall-all@3 `0.375`; multi-facet had `2/8` `POOL_MISS`, pool coverage `0.875`, Recall-all@3 `0`; protection was `0/8` `POOL_MISS`, pool coverage `1.0`, Recall-all@3 `1.0`. Producer execution then completed all `24` calls with `7,884` input tokens, `708` output tokens and `CNY 0.030024` accounted cost, yielding `31` expansion queries and `8` empty protection Hints. Semantic execution used `79/104` embedding requests, `111` cache hits and exactly `48/48` rerank requests with conservative benchmark-envelope cost upper bound `$0.16973824 < $0.20`; fallback count was `0`.

Candidate recovery reproduced on the narrowed fresh synthetic holdout. Across all `24` holdout cases, pool evidence coverage improved `0.875 -> 1.0` and `POOL_MISS` fell `4 -> 0`. Entity-reference improved pool coverage `0.75 -> 1.0`, `POOL_MISS 2 -> 0`, Recall-any@3 `0.375 -> 0.50`, Recall-all@3 `0.375 -> 0.50`, with paired Recall-all `1 improved / 0 regressed / 7 unchanged`. Multi-facet improved pool coverage `0.875 -> 1.0` and `POOL_MISS 2 -> 0`, while Recall-any@3 stayed `0.50` and Recall-all@3 stayed `0`. Protection remained exactly unchanged at full pool/top3 completeness in the `8` synthetic protection cases. This is sample-level no-regression evidence only and is not evidence that Recall Hint is universally regression-free. All frozen family-specific C2 gates passed. The qualified claim is deliberately narrow: **Recall Hint v1 recovered all `4/4` baseline pool misses in this fresh narrowed synthetic holdout, after recovering `5/6` development misses; benefit and coverage on a real query distribution remain unestablished.** C2 independently tests the post-C1 narrowed hypothesis; it is not evidence that the original four-family contract, including standalone temporal queries, passed.

The aggregate final-top3 gain is positive but sparse: Recall-any@3 `0.625 -> 0.6667`, Recall-all@3 `0.4583 -> 0.5000`, NDCG@3 `0.4711 -> 0.4919`, and evidence coverage@3 `0.5417 -> 0.5833`, with `1 improved / 0 regressed / 23 unchanged` paired transitions for each final metric. This supports Recall Hint v1 as a **candidate-recovery mechanism in the two bounded synthetic evaluations**, not as a general solution to final evidence selection and not as a claim about real-query prevalence. Multi-facet remains unresolved in this sample: its pool became complete while Recall-all@3 remained `0`. The earlier temporal observation is likewise sample-bounded: semantic retrieval had no standalone temporal candidate-recovery headroom in the consumed acceptance split, so Q4 does not generalize that real temporal queries are primarily a ranking problem.

### Q4-D quality × latency × cost product decision

Q4-D is **CLOSED** with a product **NO-GO for default synchronous serving for the Q4-measured execution shape**. Quality is qualified only for the narrowed synthetic candidate-recovery claim, and benchmark/provider cost is not the blocking dimension in this experiment. This is a historical decision about the measured synchronous shape, not a permanent rejection of later performance revisions; RH-L1 parallelization changed execution performance but did not establish complete end-to-end rollout evidence.

The C2 evaluator's `latency_ms` covers the semantic arm only. For each arm, the timer starts immediately before a depth-`20` `hybridSearch()` used to materialize the candidate pool with offline rerank disabled, then continues through a second topK=`3` `hybridSearch()` with the frozen offline reranker enabled; it stops after that ranked search. It therefore includes the arm's retrieval/vector work plus the ranked serving search/rerank, but excludes Hint-producer latency, corpus materialization/session setup, and work performed before `runArm()`. C2 runs all `24` baseline arms first, then the producer, then all `24` Hint arms inside one semantic session with a shared exact-input embedding cache. Consequently Hint arms reuse corpus and original-query embeddings already materialized by the baseline and pay new embedding/vector work primarily for expansions; the measured Hint latency is not an uncached production end-to-end number.

The evaluator percentile is nearest-rank: values are sorted and index `ceil(p*N)-1` is selected. For the `16` expansion-bearing target cases, p95 therefore equals the maximum observed latency in each arm. Their arm distributions were baseline mean about `520ms`, p50 `384ms`, p95/max `2,983ms`; Hint mean about `2,048ms`, p50 `1,968ms`, p95/max `3,857ms`. The difference of those two marginal p95 values is about `+874ms`; the difference of means is about `+1,528ms`. An earlier narrative also reported paired-delta p50 `+1,567ms` and p95 `+3,446ms`, but the retained evaluator contract does not compute paired latency percentiles and the summarized artifacts do not preserve enough per-case values to re-establish those two statistics without rerunning the experiment. They are therefore withdrawn from the authoritative Q4 latency conclusion rather than reinterpreted after the fact.

Across all `24` cases, using the same `latency_ms` boundary and nearest-rank algorithm, evaluator p95 moved from about `463ms` to `3,501ms`, a marginal p95 difference of about `+3,038ms`. The evaluator does not maintain a separate wall-clock timing boundary here; this “evaluator p95” is simply the percentile aggregation of per-arm `latency_ms` values. This is not inconsistent with the `16`-target p95 because nearest-rank p95 is the maximum for `N=16` but the second-highest value for `N=24`, and the latter population also includes `8` protection cases with empty Hints/no expansions. Because the producer latency was not preserved per C2 case, Q4-D does not invent an end-to-end SLA number; the measured semantic-arm latency, even under shared-cache conditions favorable to the Hint arm, was sufficient for the historical default-synchronous NO-GO.

Cost does not reverse that decision. The producer's observed `CNY 0.030024` is for all 24 holdout cases, and the semantic `$0.16973824` figure is a conservative A/B benchmark-envelope upper bound that includes corpus materialization, baseline and Hint arms rather than a production per-query bill. Therefore it must not be presented as a live unit cost. Operationally, Recall Hint adds one producer call plus approximately `31/16 = 1.94` extra expansion embeddings/vector searches per target query under this holdout, while the existing final rerank remains one serving rerank per query. The evidence makes latency, not benchmark spend, the current product blocker.

The resulting product boundary is: keep Recall Hint v1 as a **source-qualified experimental candidate-expansion capability with bounded synthetic evidence**; Q4 itself does not authorize default explicit-search enablement, AutoRecall integration, deployment, or any new runtime/config authority. Historical separately authorized canary transactions remain historical facts rather than authority granted by this report. A future performance-focused revision may investigate concurrency, conditional expansion, a faster producer, or another latency reduction technique, but any semantic change to producer/model/query-plan/gating requires a new version and fresh independent qualification. The consumed C2 holdout may remain regression evidence, not a fresh acceptance set.

Q4 is therefore **`PASS_WITH_FINDINGS / CLOSED`**. No third Recall Hint quality holdout is required. Q4 establishes a bounded synthetic candidate-recovery effect and a historical latency NO-GO for the measured synchronous shape; it does not establish real-query prevalence, universal protection safety, or the correct downstream evidence-selection mechanism. The next Q5 stage is fixed-candidate evidence-selection attribution, not precommitted Statistical LTR. Closing Q4 does not itself authorize model training, Q6 Shadow AutoRecall, Q7 runtime canary, push/tag, or any new live runtime/config/Core/Engine/LanceDB mutation.

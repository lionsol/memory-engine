...
Status: `Q4-A CONTRACT FROZEN / Q4-B SOURCE CLOSED / Q4-C0 CLOSED / Q4-C1b DEVELOPMENT PASS / Q4-C2 REAL HOLDOUT PASS / INDEPENDENT CANDIDATE RECOVERY REPRODUCED / FINAL TOP3 GAIN LIMITED / Q4-D CLOSED / DEFAULT SYNCHRONOUS SERVING NO-GO ON LATENCY / NO RUNTIME AUTHORITY`
...
Candidate recovery reproduced independently. Across all `24` holdout cases, pool evidence coverage improved `0.875 -> 1.0` and `POOL_MISS` fell `4 -> 0`. Entity-reference improved pool coverage `0.75 -> 1.0`, `POOL_MISS 2 -> 0`, Recall-any@3 `0.375 -> 0.50`, Recall-all@3 `0.375 -> 0.50`, with paired Recall-all `1 improved / 0 regressed / 7 unchanged`. Multi-facet improved pool coverage `0.875 -> 1.0` and `POOL_MISS 2 -> 0`, while Recall-any@3 stayed `0.50` and Recall-all@3 stayed `0`. Protection remained exactly unchanged at full pool/top3 completeness. All frozen family-specific C2 gates passed.

The aggregate final-top3 gain is positive but sparse: Recall-any@3 `0.625 -> 0.6667`, Recall-all@3 `0.4583 -> 0.5000`, NDCG@3 `0.4711 -> 0.4919`, and evidence coverage@3 `0.5417 -> 0.5833`, with `1 improved / 0 regressed / 23 unchanged` paired transitions for each final metric. This qualifies Recall Hint v1 as an independently reproduced **candidate-recovery mechanism**, not as a general solution to set-aware final serving; multi-facet remains primarily a downstream ranking/evidence-composition problem after candidate recovery.

### Q4-D quality × latency × cost product decision

Q4-D is **CLOSED** with a product **NO-GO for default synchronous serving at the current execution shape**. Quality is qualified for the narrowed candidate-recovery claim, and benchmark/provider cost is not the blocking dimension, but latency is.

The C2 evaluator's Hint-arm latency covers semantic retrieval/rerank only; it does **not** include the preceding V4-Flash Hint-producer call, so it is a lower bound rather than end-to-end latency. Even on that lower bound, all `16` target cases generated expansion queries (`31` total; fifteen cases used two expansions and one used one). Target-case semantic latency moved from baseline mean about `520ms` / p50 `384ms` / p95 `2,983ms` to Hint mean about `2,048ms` / p50 `1,968ms` / p95 `3,857ms`; the per-case semantic-only delta was mean about `+1,528ms`, p50 about `+1,567ms`, p95 about `+3,446ms`. Across all 24 cases, evaluator p95 moved from about `463ms` to `3,501ms` (`+3,038ms`). Because producer latency was not preserved per C2 case, Q4-D does not invent an end-to-end SLA number; the measured semantic-only lower bound is already sufficient to reject default synchronous rollout.
...
The resulting product boundary is: keep Recall Hint v1 as a **source-qualified experimental candidate-expansion capability**; do not enable it by default in explicit search, do not integrate it into AutoRecall, and do not infer deployment/runtime/config authority from Q4. A future performance-focused revision may investigate concurrency, conditional expansion, a faster producer, or another latency reduction technique, but any semantic change to producer/model/query-plan/gating requires a new version and fresh independent qualification. The consumed C2 holdout may remain regression evidence, not a fresh acceptance set.

Q4 is therefore closed with findings. No third Recall Hint quality holdout is required. Q5 Statistical LTR remains `LATER / NOT STARTED` until its separate real-label entry criteria are satisfied; closing Q4 does not authorize Q5, Q6 Shadow AutoRecall, Q7 runtime canary, push/tag, or any live runtime/config/Core/Engine/LanceDB mutation.

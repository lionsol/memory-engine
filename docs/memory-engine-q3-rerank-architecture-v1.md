...
Date: 2026-09-12
Status: ARCHITECTURE DIRECTION ACCEPTED_WITH_LIMITATIONS; R3-C0 CONTROL RUNTIME QUALIFIED; R3-C1-A-E0/E1 EXECUTION COMPLETE / QUALITY PASS / PROVIDER LATENCY-RELIABILITY FAIL; R3-C1-A-M1 SOURCE CLOSED; R3-C1-A-M2 FAIL_WITH_FINDINGS / EXECUTION COMPLETE; R3-C1-A-M3 OFFLINE ATTRIBUTION COMPLETE; R3-C1-A-M4 QUALIFICATION-CONTRACT-V2 SOURCE CLOSED; R3-C1-A-M5 V2 EXECUTION SEAM SOURCE CLOSED; R3-C1-A-M6 ZERO-PROVIDER OPERATOR/CLI SOURCE CLOSED; M7 REAL HOLDOUT FAIL_WITH_FINDINGS / EXECUTION COMPLETE; SILICONFLOW QWEN3-RERANKER-0.6B PRODUCT ACCEPTED_WITH_KNOWN_LIMITATION; C1-B SOURCE CLOSED / REAL-HOST SMOKE PASS_WITH_FINDINGS; PERSISTENT EXPERIMENTAL EXPLICIT-SEARCH PROVIDER ACTIVATION ACTIVE

The independent rerank interface, canonical text projector and orchestration boundary were accepted as the foundation for the qualified control runtime and later provider integration. At the original architecture-decision point, the completed canonical-chunk comparison did not approve a production provider, provider default, or benchmark-derived quality claim. Subsequent C1 stages, recorded below and in `docs/current-state.md`, later source-closed the 0.6B provider seam, completed a real-host smoke with one retained evidence gap, and activated the provider persistently on the experimental instance under explicit Owner authorization.
Source acceptance, the closed R3-C0 runtime evidence, and the completed bounded provider qualifications are recorded below. The C0 historical runtime state remains frozen as evidence; it must not be read as the current live-instance state after the separately authorized C1-B and later AutoRecall experiments.
...
## Deferred Q3 decisions at the original architecture point

- At the original architecture point, real provider integration, provider choice, provider token budgets and provider enablement were `NOT STARTED / NOT AUTHORIZED`; that historical state is superseded by the later C1-A/C1-B records below. C0 control runtime qualification remains closed, and the offline `50`/`10s` values remain historical experiment settings only.
...
`R3-C1-A-I0 = PASS / SOURCE IMPLEMENTED / VERIFIED` at `1324c8c05e62639ae3418b64b6e4e04c14c51adb`. The source-only tooling binds deterministic `P256/D64/S8` qualification populations, the frozen FTS-only candidate-generation limitation, canonical depth-20 `4000/48000` text projection, a fail-closed SiliconFlow `Qwen/Qwen3-Reranker-8B` adapter, conservative token/request/cost accounting, packet-bound pacing and bounded evidence/scoring. The canonical `4000` per-candidate limit is measured in Unicode code points, while the SiliconFlow adapter preflight intentionally uses UTF-8 byte length as a conservative Qwen token upper bound (`8192` document units). Those are different contracts: multi-byte CJK/emoji text can be rejected by provider preflight before reaching the canonical code-point ceiling. This is an intentional fail-safe bound, not a guarantee that every valid 4000-code-point projection is provider-admissible.
...
`R3-C1-A-M6 = PASS / SOURCE IMPLEMENTED / VERIFIED / CLOSED` at `ab4b20d8423b55ea36baaa37649a7e7ca0b9e668` (`feat(benchmark): add C1 v2 zero-provider operator`). The committed zero-provider CLI exposes `prepare`, `freeze-packet`, and `validate`; production defaults bind the observed corpus to exact M2 manifest `c723886df9083bdd72753059afc4773195707d96077946b384eb163df725b272`. `prepare` requires clean source plus explicit egress ALLOW, while packet freezing requires current price, rate-limit source, pacing, execution root and API-key env as explicit inputs. No real H256/D64/S16 ALLOW manifest or packet was generated in M6. Focused M4/M5/M6 tests pass `12/12`, C1/C0 adjacent coverage passes `97/97`, static check `766`, integrity `349/0`, OpenSpec strict `12/12`, and `git diff --check` pass.

`R3-C1-A-M7 real disjoint-holdout provider qualification = FAIL_WITH_FINDINGS / EXECUTION COMPLETE`. Exact source is `924b01b2cdf03b916b8814e29fb2ad73d3b5ddc9`, manifest `fbe0076478526cbe1b2e328d3297cc146d030650bac766d281b62c988345d5dd`, prior observed M2 manifest `c723886df9083bdd72753059afc4773195707d96077946b384eb163df725b272`. All `336/336` requests completed with no stop artifact, fallback, retry, structural invalidity, unauthorized transmission or budget/integrity failure. H256 quality passed strongly: Recall-all `+17.58pp`, Recall-any `+19.92pp`, evidence coverage `+18.85pp`; non-adversarial protect regression was `1/83 = 1.20%`. Latency passed at p95/p99/max `582/620/651ms`. S16 applied-pair and top1 stability both passed `16/16`, while frozen top3-set stability failed `15/16`; the sole mismatch `conv-47:qa:150` kept ranks 1-2 stable and replaced only rank 3. Accounted cost was about `$0.0264`. Owner accepts SiliconFlow `Qwen/Qwen3-Reranker-0.6B` for the C1 explicit-search rerank line with this known provider nondeterminism limitation; this product decision does not rewrite M7 to PASS. Subsequent C1-B source work closed at `9cbfbb0d4ce84dd934538f2d92063825cd94e1aa`; the real-host smoke closed `PASS_WITH_FINDINGS`, retaining the documented inability to reconstruct per-request `rerank.status=applied` from persisted Gateway evidence. The experimental instance was later persistently activated with `explicitSearchRerankProvider.enabled=true` under explicit Owner egress/runtime authorization. That historical Level-C execution did not preserve the controlled Stage Card/execution-binding artifact chain required by the project's current governance policy; this is retained as a governance-process deviation and is not retroactively repaired or used to create new authority. Current state remains owned by `docs/current-state.md`.

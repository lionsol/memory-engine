...
`R3-C1-A-M7 real disjoint-holdout provider qualification = AUTHORIZED / EXECUTION ENTRY SOURCE VERIFIED / COMMIT PENDING`; provider execution has not started. The authorized scope is one SiliconFlow `Qwen/Qwen3-Reranker-0.6B` run over disjoint `H256 + D64 + S16`, max `336` requests, `5000ms` deadline, `6M` input-token cap, `$1` cost cap, retry `0`, execution_count `1`, pacing `1000ms / 60000ms / 400000 estimated tokens`. Current official facts were re-verified before preparation: `$0.01/M` input tokens and public Reranker limits `RPM=2000`, `TPM=500000`. Zero-provider preflight on clean M6 source produced prospective manifest SHA `5332ebbf683393d010834db1c6f140508f11b2cd646d74533ecf2ec03e7e5b8e`, exact prior M2 manifest `c723886df9083bdd72753059afc4773195707d96077946b384eb163df725b272`, `overlap=0`, H256 eligible `256/256`, S16⊂H256. The exact-bound `execute-provider` path is source-verified: focused `13/13`, C1/C0 adjacent `98/98`, static `766`, integrity `349/0`, OpenSpec strict `12/12`, diff-check PASS. The source/test/docs changes must be committed before the one authorized execution may be consumed. No C1-B, production rerank, AutoRecall, Gateway/config/DB/LanceDB/live-plugin mutation, push or tag is authorized by this document.

...
`R3-C1-A-M4 = PASS / SOURCE CONTRACT IMPLEMENTED / VERIFIED / COMMIT PENDING`. The isolated v2 contract verifies the prior observed v1 manifest body hash, excludes exactly the prior `320` unique P256+D64 cases, then deterministically selects a disjoint `H256 + D64` from the remaining `1650` cases and `S16` from eligible H256. The real-corpus zero-provider smoke has `overlap_with_prior=0`, `H256 eligible=256/256`, with category counts `28/47/11/106/64` for categories 1–5. Aggregate quality, reliability and latency thresholds remain unchanged. Protect regression is gated prospectively only on answer-bearing categories 1–4 while category 5 remains in aggregate metrics and is reported separately as adversarial diagnostic. Sentinel qualification requires all `16/16` pairs applied, exact top1 `16/16`, and exact top3 membership `16/16`; exact ordered-top3 equality remains a diagnostic rather than a blocker. Focused v2 tests pass `4/4`, C1/C0 adjacent coverage passes `78/78`, static check `761`, integrity `347/0`, OpenSpec strict `12/12`, and `git diff --check` pass. No provider request or runtime mutation occurred.

The next decision is `R3-C1-A-M5 v2 execution seam / disjoint-holdout requalification preparation = NOT STARTED / PROVIDER NOT AUTHORIZED`. M5 must introduce a separately bound v2 packet/budget/execution path and must not mutate the historical v1 `328`-request packet contract or C0. Any later real-provider run requires separate Owner authorization, clean committed M4 source, and a regenerated holdout manifest; no C1-B, production rerank, AutoRecall, Gateway/config/DB/LanceDB/live-plugin mutation, push or tag is authorized by this document.

...
| 配置语义等价门 | [`../bin/build-config-semantic-equivalence-report.js`](../bin/build-config-semantic-equivalence-report.js) | R6.5.1 passed / closed | 只允许规范、单调递增的 `meta.lastTouchedAt` 单路径变化；任何其他 JSON 路径变化继续 fail closed，且不输出配置秘密值 |
| Live runtime remediation retry 授权包 | [`smoke-tests/personal-runtime-live-remediation-retry-authorization-20260721.md`](smoke-tests/personal-runtime-live-remediation-retry-authorization-20260721.md) | R6.5.2 passed / closed | 为同一 candidate 定义全新 C0/R0/H0/D0、语义配置门、bounded startup、loaded-method/tool 验证和独立 rollback；旧授权不可复用 |
| Live runtime remediation retry 执行决策 | [`smoke-tests/personal-runtime-live-remediation-retry-decision-20260721.md`](smoke-tests/personal-runtime-live-remediation-retry-decision-20260721.md) | Blocked / no mutation | 最终 preflight 发现 candidate 与当前 recovery root 已从 `/tmp` 消失，installed sourcePath 悬空；Gateway 未停止，原授权失效，后续必须 rebuild 或 rebase |
| 持久 artifact rebuild 与 recovery-source rebase 设计 | [`smoke-tests/personal-runtime-persistent-artifact-rebase-design-20260721.md`](smoke-tests/personal-runtime-persistent-artifact-rebase-design-20260721.md) | R6.5.3 passed / closed | 恢复 `$HOME/.openclaw/backups` 持久 authority；先离线重建 candidate/R0，再以独立 live 事务仅 rebase 相同 runtime 的 sourcePath，candidate 激活另行授权 |
| 离线持久 artifact preparation 授权包 | [`smoke-tests/personal-runtime-persistent-artifact-preparation-authorization-20260721.md`](smoke-tests/personal-runtime-persistent-artifact-preparation-authorization-20260721.md) | R6.5.3A passed / closed | 绑定执行时 clean HEAD、active runtime、Gateway 与持久 parent；只允许离线 staging、验证、冻结和 atomic publication，不允许 Gateway、OpenClaw、配置或数据 mutation |
| 离线持久 artifact preparation 执行决策 | [`smoke-tests/personal-runtime-persistent-artifact-preparation-decision-20260722.md`](smoke-tests/personal-runtime-persistent-artifact-preparation-decision-20260722.md) | Blocked / no publication | candidate/R0 parity 与 native smoke 均通过，但 exact mode identity 与 read-only freeze 在当前文件系统无法同时满足；staging 已删除，未发布 authority，后续需 freeze-model repair |
...
- **R6.5.1 config semantic equivalence**：`bin/build-config-semantic-equivalence-report.js` 只批准 `meta.lastTouchedAt` 的规范单调更新时间；所有其他路径变化继续阻塞。源码实现已通过 EDI 独立验证并关闭。
- **R6.5.2 live remediation retry authorization**：见 [`smoke-tests/personal-runtime-live-remediation-retry-authorization-20260721.md`](smoke-tests/personal-runtime-live-remediation-retry-authorization-20260721.md)。绑定未漂移的 candidate 与当前 recovery R0，要求新的 fresh C0/R0/H0/D0 和新的精确 operator approval；当前不授权执行。

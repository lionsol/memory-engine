...
| 个人运行副本修复授权设计 | [`smoke-tests/personal-runtime-remediation-authorization.md`](smoke-tests/personal-runtime-remediation-authorization.md) | R6.3 design / verification pending | 选择 npm-pack source + exact lock + Node 24 dependency-complete candidate；定义 C0/R0/D0、停机安装、Gateway RPC 验证和 rollback 分支，但不授权执行 |
...
- **R6.3 runtime remediation authorization design**：见 [`smoke-tests/personal-runtime-remediation-authorization.md`](smoke-tests/personal-runtime-remediation-authorization.md) 与 [`runtime-sync.md`](runtime-sync.md)。禁止直接复制开发仓库和直接 archive install；未来先在隔离目录用 Node 24 构建 dependency-complete candidate，再经单独 R6.5 授权安装。

...
| Host activation boundary 兼容 | [`smoke-tests/host-activation-boundary-compatibility.md`](smoke-tests/host-activation-boundary-compatibility.md) | R6.2 implemented / verification pending | 使 active-memory boundary resolver 匹配 OpenClaw 2026.6.9 的 global disable、deny、entry、non-empty allowlist 和 bundled default 顺序 |

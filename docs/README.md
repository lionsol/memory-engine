...
- **R6.5.1 config semantic equivalence**：`bin/build-config-semantic-equivalence-report.js` 只批准 `meta.lastTouchedAt` 的规范单调更新时间；所有其他路径变化继续阻塞。源码实现已通过 EDI 独立验证并关闭；live retry 仍未授权。

...
Repository preflight passed 64/64 focused R6.3-R6.5.2, semantic-config, artifact, ledger, and authorization tests; static check covered 530 files; and `git diff --check` passed. Independent EDI closeout then passed the requested nine-test scope at 64/64, the full suite with 1789 passed, 0 failed, and 8 skipped, A5 smoke 10/10, and repository status review. R6.5.2 is therefore passed and closed. Source/candidate parity and active-runtime/current-R0 parity remained zero. No fresh retry artifact, Gateway stop/start/restart, candidate install, config mutation, or data restoration occurred.
...
### B5-I2c time-frozen LoCoMo lexical v2 source correction

- B5-I2c is `REPO-TESTED`. The default-off `runtime.searchNowSec` seam resolves one fixed ranking clock per `hybridSearch()` request while preserving the production wall-clock default and normal vector latency measurement.
- Added the independent `production_hybrid_lexical_dialog_locomo_time_frozen_v2` runner/CLI with one required benchmark clock shared by materialization and search/rerank, reserved provenance keys, per-result `search_now_sec`, and fail-closed clock mismatch/spoof checks. The v1 profile, runner, CLI identity, source authority, artifact, and metrics remain preserved.
- B5-S1 v1 is corrected as `HISTORICAL LEXICAL EVIDENCE / TIME PROVENANCE INCOMPLETE / NOT STRICT SEMANTIC A/B AUTHORITY`: the existing output, logs, and mtimes cannot recover the actual per-search wall-clock sequence. The v2 source is `REPO-TESTED`; the lexical v2 baseline is `NOT RUN`.
- The reserved semantic v2 profile is `production_hybrid_semantic_dialog_locomo_time_frozen_v2`. Its vector-attempt invariant is `PENDING LEXICAL V2 CONFIDENCE DISTRIBUTION`; the six numerical gates remain frozen but suspended until lexical v2 authority exists. B5 overall remains `OPEN` and B5-I3 remains `HOLD`. No provider, baseline, runtime/live DB/LanceDB/Gateway/config/tag operation occurred.

## 2026-08-29

### B5-S1-v2 time-frozen LoCoMo lexical baseline closed

- B5-S1-v2 execution is `PASS_WITH_FINDINGS / BASELINE FROZEN / CLOSED` under profile `production_hybrid_lexical_dialog_locomo_time_frozen_v2`, source commit `b4a53522bba7a1314b2c99f1fd566872e6e5e8d4`, dataset SHA-256 `79fa87e90f04081343b8c8debecb80a9a6842b76a7aa537dc9fdf651ea698ff4`, `top_k=50`, and equal benchmark/materialization/search clocks `1705066861`. `RC=0`.
- Temporary output `/tmp/memory-engine-benchmark-v1/b5-s1-v2-b4a53522-locomo-lexical-output.json` is `31,280,002` bytes with SHA-256 `9d6c4c02a5fff3841f98552f5d7ee087c57448bc8a917606a047ac395e222f06`. Temporary log `/tmp/memory-engine-benchmark-v1/b5-s1-v2-b4a53522.log` has SHA-256 `7b066eea1c314f9e93d84deb86cb1087a8b5c1c8e5b25c40e1bc04caf5b86af7`. Output/log/RC remain temporary and are not vendored; committed provenance, metrics, invariants, and adjudication are the long-term authority.
- Strict denominator is `1,972` scored / `14` skipped; sensitivity/retrieval denominator is `1,978` scored / `8` skipped. Strict dialog metrics are Recall-any@1 `0.1546653144`, @5 `0.2728194726`, @10 `0.3220081136`, @50 `0.4239350913`; Recall-all@5 `0.2276876268`, @10 `0.2621703854`, @50 `0.3306288032`; NDCG-any@10 `0.2135986258`; MRR `0.2093299195`; mean first relevant rank `7.9629186603`; zero-hit@50 `1,136/1,972`; All@10 failures `1,455/1,972`.
- Mean retrieval latency is `15.4559368468 ms`; corpus build latency total is `754.702548 ms`; retrieval latency total is `30,571.843083 ms`. Every scoreable retrieval diagnostic has `search_now_sec=1705066861`; missing `0`, mismatch `0`.
- Lexical confidence count is `1,978`, non-finite `0`, min/max `0.3571/0.6900`, mean `0.5010601112`, p50/p90/p95/p99 `0.5063/0.5563/0.5786/0.6275`; below `0.7` is `1,978`, equal/above `0.7` is `0`.
- The semantic v2 vector gate is frozen at attempted/skipped/error `1,978/0/0`, with LanceDB `lancedb_search` and `vector_in_fusion=true` required for every attempted case, host-manager fallback `0`, no query embedding/vector search for any vector-skipped case, and no silent lexical-only degradation.
- Compared with historical v1, retrieved order changed in `434/1,978` cases; first-rank comparison is `22 improved / 36 regressed / 1,920 unchanged`; Recall-any@10 delta `+0.000507`, Recall-all@10 delta `+0.001014`, NDCG-any@10 delta `+0.001660`, and Recall-any/Recall-all@50 delta `0`. These are clock-only findings and do not authorize retrieval-heuristic tuning.
- B5-S1 v1 remains historical lexical evidence with incomplete time provenance and unchanged historical metrics. B5 overall remains `OPEN`; the six semantic numerical gates and regression guards now compare semantic v2 minus lexical v2 without threshold changes. Semantic v2 remains `NEXT / SOURCE IMPLEMENTATION NOT STARTED / PROVIDER RUN NOT AUTHORIZED`.

## 2026-08-29

### B5-I3 time-frozen LoCoMo semantic v2 source implementation

- B5-I3 source implementation is `REPO-TESTED` under the independent profile `production_hybrid_semantic_dialog_locomo_time_frozen_v2`. The runner and CLI create one conversation-owned temporary Core/Engine/FTS/LanceDB data plane, reuse it across that conversation's scoreable QA, materialize Canonical vector projection v1 with the 2,000-character limit outside the QA loop, and reuse the existing low-level embedding-cache and production `hybridSearch()` seams.
- The fixed positive-safe-integer benchmark clock is bound equally to benchmark, materialization, search, rerank, per-result diagnostics, and provenance. The frozen semantic vector gate is `vector_attempted_count=1,978`, `vector_skipped_count=0`, `vector_error_count=0`, with LanceDB `lancedb_search`, `vector_in_fusion=true`, zero host-manager fallback, and no silent lexical-only degradation required for a valid official run.
- Focused fake-provider tests cover profile separation, v1/v2 lexical preservation, isolation/reuse, projection and gold-leakage boundaries, cache restart/ownership, fixed-clock provenance, embedding/vector/fallback failures, memory-id isolation, reserved-contract rejection, and deterministic CLI smoke. The existing lexical v1 regression remains passing; no real provider was called and no semantic sanity/full baseline was run.
- B5-S1-v2 lexical baseline remains the frozen comparison authority; its six numerical gates and regression guards remain unchanged. Real-provider sanity and semantic full A/B remain `NOT RUN / NOT AUTHORIZED`; B5 quality adjudication and B5 overall remain `OPEN`. No runtime, live DB/LanceDB, Gateway, config, deployment, or tag operation occurred.

### B6-I2b retained-root restart and cross-store crash reconciliation

- B6-I2b completes the benchmark-local retained-root and reconciliation stage. AML Add now commits Engine `PENDING` metadata first with `confidence=0`, ensures the exact Core/FTS row, materializes the required semantic vector, and promotes both `memory_confidence.confidence` and `aml_add_requests.state` to production confidence/`READY` in one Engine transaction. The ledger migration is in place and deterministic: legacy rows without state are compatible `READY` rows; new rows persist `state`, `vector_required`, and `updated_at`.
- Same-payload `READY` retry is deduped. A `PENDING` retry verifies/repairs Engine metadata, Core/FTS content, and the exact vector row before promotion; payload conflicts and vector-mode mismatches fail closed. The existing production Hybrid confidence filter remains unchanged, so Core rows paired with pending confidence `0` are not retrieval-visible. A test-only child-process stage hook qualifies hard exits after pending, Core, vector, and READY commits without adding crash behavior to normal production paths.
- Ordinary failures on a newly created Add clean up its partial state; failures while reconciling an already durable PENDING request preserve that confidence-0, non-visible PENDING state for a later deterministic retry.
- Explicit retained-root mode is restricted to a child of the operating-system temporary directory. It writes an atomic registry manifest binding data-plane/semantic/vector/cache identity and a per-user ownership marker. Compatible roots reopen existing users lazily; unknown users do not allocate; corrupt, missing, mismatched, future, or symlinked ownership state fails closed. `close()` closes operation-scoped resources and preserves the retained root; explicit test cleanup destroys it.
- Fresh I2b coverage is `9/9 PASS`: crash A/B/C/D reconciliation, pending and mixed-state visibility, deterministic Core/FTS repair, semantic vector/cache reuse, restart Search, physical cross-user isolation, manifest mismatch/corruption, and legacy ledger migration. Existing B6-I1 and I2a suites remain passing. No real provider, AML service, official AML smoke/full evaluation, runtime/live data mutation, deployment, or tag operation occurred. B6-I2 is `SOURCE IMPLEMENTATION REPO-TESTED / CLOSED`; B6-I3 HTTP/Docker transport is next as a source-only stage, and B6-S2/S3 remain unauthorized.

### B5-D2 LoCoMo semantic v2 baseline closeout

- B5-S3 execution is `PASS` (`RC=0`) under profile `production_hybrid_semantic_dialog_locomo_time_frozen_v2`, source commit `dfb41ea866646cbe556e3e6c3c03782da52de216`, dataset SHA-256 `79fa87e90f04081343b8c8debecb80a9a6842b76a7aa537dc9fdf651ea698ff4`, upstream commit `3eb6f2c585f5e1699204e3c3bdf7adc5c28cb376`, dataset-file commit `cbfbc1dba6bc53d00625212a0f22d55ffee7c1fc`, license `CC BY-NC 4.0 International`, fixed clock `1705066861`, and `top_k=50`.
- The temporary full output `b5-s3-dfb41ea8-full-semantic-output.json` is SHA-256 `eb4b29c56435297f7a376f18647ca44cce4659b38a8b3fa1e980c853082b1829` / `32,273,813` bytes; the reused embedding cache is SHA-256 `2d447d76585a3749955ba017f2a0e63468b643c561cabef3d46be8caa225c458` / `171,872,256` bytes, `7,848` entries, dimensions `2560/2560`, `integrity_check=ok`; the log is SHA-256 `65eade5b211693585624025dd737b33793adeae800971adecfe73e0d21ffaa59` / `39,194` bytes; and the RC marker is SHA-256 `9a271f2a916b0b6ee6cecb2426f0b3206ef074578be55d9bc94f6f3fe3ab86aa` with value `0`. All artifacts remain under `/tmp/memory-engine-benchmark-v1/` and are not vendored.
- The run covered `1,986` cases / `1,978` retrieval cases, strict `1,972/14` and sensitivity `1,978/8` scored/skipped, `10` conversation-owned corpora, and `5,882` corpus dialog rows. Embedding accounting was `5,882` corpus + `1,978` query lookups, `7,232` provider calls and `628` cache hits. Vector attempted/skipped/error was `1,978/0/0`; every retrieval case used LanceDB `lancedb_search` with `vector_in_fusion=true`, and host-manager fallback was `0`.
- Strict dialog primary metrics were Recall-any `@1/@5/@10/@30/@50 = 0.2089249493/0.3382352941/0.3919878296/0.5167342799/0.6435091278`, Recall-all `= 0.1749492901/0.2789046653/0.3174442191/0.4224137931/0.5289046653`, and NDCG-any `= 0.2089249493/0.2574948831/0.2726391074/0.3009152306/0.3240737005`. MRR was `0.2762670443`; zero-hit@50 was `703/1,972`; All@10 failures were `1,346/1,972`.
- Compared with frozen lexical v2, the six gates were `5/6 PASS`: overall Recall-any@10 `+0.0699797160`, Recall-all@10 `+0.0552738337`, NDCG-any@10 `+0.0590404816`, Recall-any@50 `+0.2195740365`, and Multi-hop Recall-any@10 `+0.0685920578` passed; Multi-hop Recall-all@10 was `+0.0180505415`, below the `+0.0200` threshold. All regression guards passed. Explicitly labeled case-level improved/regressed/unchanged counts were First relevant rank `801/46/1,125`, Recall-any@10 `141/3/1,828`, Recall-all@10 `110/1/1,861`, and Recall-any@50 `436/3/1,533`; combined three-field dominance was `659/46/1,122` with `145` mixed cases.
- Semantic mean retrieval latency was `364.761142 ms` versus lexical v2 `15.455937 ms` (`23.60×`); semantic corpus-build and retrieval totals were `1,665,244.752032 ms` and `721,497.538846 ms`. The result is `B5 semantic v2 quality = PASS_WITH_FINDINGS` and `B5 semantic v2 = OFFLINE BASELINE RECORDED / BASELINE FROZEN / CLOSED`; B5 overall is `PASS_WITH_FINDINGS / CROSS-DATASET GENERALIZATION RECORDED / CLOSED`. B6-A1 is now `PASS_WITH_FINDINGS / SOURCE INSPECTION COMPLETE` and B6-I1 is `SOURCE IMPLEMENTATION REPO-TESTED`; B6-I2 semantic backend/resource lifecycle is next, while B6-I3 HTTP/Docker transport follows later and official AML smoke/full remain unauthorized. Benchmark evidence is not runtime or production authority, and no further provider run or production tuning is implied.

### B6-A1 / B6-I1 Agent Memory Leaderboard adapter

- Read-only B6-A1 inspection pinned AML public source authority to `AML-memory/agent-memory-leaderboard` commit `1b8142bfe0f20f1c5218d6b554aa0012de34e504`. The public participant boundary is Add/Search while AML owns answer generation, scoring, held-out evaluator, and orchestration; public repository-level license metadata is absent, so no AML source is vendored into memory-engine.
- Added `lib/benchmark/aml-adapter-v1.js` and `lib/benchmark/aml-data-plane-v1.js`. `memory_engine_aml_adapter_v1` validates the pinned public Add/Search schema, normalizes optional message timestamps, keeps Search `options` out of the retrieval query, caps `top_k` at `100`, serializes operations per `user_id`, and returns only AML-facing Add/Search responses.
- AML Add uses deterministic `aml_add_projection_v1`, persistent Engine-side `request_id` idempotency, production `autoRouteCategory()`/`catParams()` metadata, immediate searchable Core/FTS insertion, and fail-closed conflicting retry semantics. Search uses production `hybridSearch()` ranking and freezes the current `production_hybrid_text_240_v1` evidence surface instead of silently substituting full Canonical text.
- Isolation is physical rather than filter-based: each AML `user_id` owns a separate temporary Core SQLite, Engine SQLite, LanceDB path/vector-backend boundary, and Hybrid runtime. Live OpenClaw Core, memory-engine Engine, live LanceDB, workspace memory files, host memory manager fallback, Gateway, config, and plugin runtime are outside this adapter.
- The shared `createBenchmarkHybridRuntime()` gained an optional `minConfidence` input. Its default remains `0`, so B3-B5 profiles are unchanged; B6 explicitly binds production `confidence.min=0.15` and the production lexical semantic-activation threshold `0.7`. Add/Search share an injectable per-user clock seam so fixed repository tests do not create artificial confidence decay while default execution remains wall-clock based.
- B6-I1 includes an injectable per-user vector backend/Canonical vector projection seam but does not configure or call a real provider. B6 focused contract tests are `16/16 PASS` (15 AML adapter checks plus 1 shared-helper contract). The complete Benchmark v1 test family is `145 PASS / 4 SKIP / 0 FAIL`; documentation/current-state focused validation is `7/7 PASS`, static check covers `728` files, and `git diff --check` passes under Node `v24.19.0`. Added hardening covers legal empty-string Add content, disk-backed request-id idempotency across reopening the same retained temporary user data plane, and operation-scoped SQLite/Hybrid handles with a 120-user persistent-FD guard. The four skips are unchanged external/official contract conditions. No AML service, provider, public endpoint, Docker submission, live runtime/data mutation, tag, or leaderboard execution occurred.
- B6-I1 status is `SOURCE IMPLEMENTATION REPO-TESTED / NO OFFICIAL EXECUTION`. B6-I2 semantic backend/resource lifecycle is next and source-only; B6-I3 HTTP/Docker transport follows later. B6-S2 official smoke and B6-S3 full evaluation remain separately unauthorized.

### B6-I2a AML semantic backend and bounded resource ownership

- B6-I2a adds `lib/benchmark/aml-semantic-backend-v1.js` behind the existing per-user AML vector-backend seam. The backend freezes `memory_engine_aml_semantic_backend_v1` / `temporary_lancedb_per_user_v1`, reuses Canonical vector projection v1 and the existing `memory_engine_benchmark_embedding_cache_v1`, and requires an explicitly injected embedding implementation; no provider-discovery or real network path is used.
- Each Add/Search operation opens and closes its own benchmark-owned SQLite embedding cache, LanceDB connection, and `chunks` table below the AML user's temporary root. Repeated exact canonical materialization reuses one row; inconsistent rows are deleted and deterministically repaired. The actual installed LanceDB package is exercised in focused tests with finite 2,560-dimensional fake vectors.
- Scoped semantic Search passes the operation-owned table and cached query embedder into production `hybridSearch()`. It fails closed on missing vector participation, embedding/cache errors, LanceDB initialization/write/search errors, wrong dimensions, non-finite vectors, and provider errors; host memory-manager fallback remains forbidden. The AML public adapter continues to return only `{data:[...]}` while internal diagnostics prove `lancedb/lancedb_search` and vector fusion.
- Focused coverage proves cache persistence across scoped close/reopen, per-user physical Core/Engine/LanceDB/cache isolation, row idempotency, and close semantics on successful and failed Add/Search. B6-I2a semantic backend tests are `8/8 PASS`; the complete Benchmark v1 family is `153 PASS / 4 SKIP / 0 FAIL` when run one file at a time under Node `v24.19.0`, with the aggregate launcher’s existing intermittent nested LongMemEval CLI subprocess failure isolated and passing on rerun. Documentation/current-state tests are `2/2 PASS` and static check covers `730` files. B6-I1 behavior remains passing. This is `B6-I2a SOURCE IMPLEMENTATION REPO-TESTED`; B6-I2 remains in progress, B6-I2b is next for retained-root restart/resume and cross-store crash reconciliation, and B6-I3 remains later HTTP/Docker transport. No real provider, AML service, official AML smoke/full evaluation, runtime/live DB mutation, deployment, or tag operation occurred.

### B6-I3 AML HTTP and Docker transport

- B6-I3 adds the benchmark-local Node `http` transport at exactly `GET /health`, `POST /add`, and `POST /search`. The server delegates request semantics, PENDING→READY ordering, idempotency, physical `user_id` isolation, and production `hybridSearch()` behavior to `createAmlAdapter`; it returns only the frozen AML envelopes and validates adapter output before any HTTP 200. JSON bodies are bounded at 2 MiB, `top_k<=100` remains adapter-owned, health is minimal and unauthenticated, and Bearer/Token/X-Api-Key authentication uses constant-time comparison with stable sanitized error codes.
- The service runtime uses only `SILICONFLOW_API_KEY` for the frozen semantic provider identity and has no host-config credential fallback. Provider HTTP is injectable for deterministic tests; no real request was made. SIGTERM/SIGINT shutdown stops acceptance, drains in-flight operations, closes the adapter once, and preserves the retained benchmark root. `Dockerfile.aml` is a Node 24 non-root image with the retained root as a volume and a built-in `/health` healthcheck; `.dockerignore` excludes tests, reports, credentials, logs, and database artifacts.
- Fresh B6-I3 source coverage proves route/auth/error/output contracts, malformed-output fail-closed behavior, graceful shutdown barriers, HTTP Add idempotency, actual temporary LanceDB plus fake 2,560-dimensional embedding integration, restart-over-HTTP Search, unknown-user no-allocation, provider-wrapper failure handling, and Docker packaging contracts. The HTTP transport suite is `9/9 PASS`, service/provider runtime is `6/6 PASS`, Docker contracts are `2/2 PASS`, and the complete Benchmark v1 family is `179 PASS / 4 SKIP / 0 FAIL` under Node `v24.19.0`; documentation/current-state validation is `7/7 PASS` and static check covers `737` files. B6-I1/I2a/I2b remain closed, B6-I3 is `SOURCE IMPLEMENTATION REPO-TESTED / CLOSED`, and B6-S1 local synthetic HTTP/Docker qualification is next with no real provider. B6-S2 official AML smoke and B6-S3 full evaluation remain unauthorized. Benchmark evidence is not runtime or production authority; no AML service, deployment, live data mutation, Gateway/config/plugin change, or tag operation occurred.

### B6-I3-R1 Search response disclosure boundary

- The HTTP Search validator now uses the exact enumerable item allowlist `id`, `content`, optional `score`, and optional `created_at`; unknown adapter fields are rejected with `adapter_search_response_invalid` rather than silently stripped. Required strings remain non-empty, `score` and numeric `created_at` must be finite, and the existing nullable `created_at` shape remains supported.
- The focused HTTP suite is `12/12 PASS` under Node `v24.19.0`, including diagnostics/provenance/arbitrary-key disclosure fixtures, legal item shapes, and non-finite numeric failures; the complete Benchmark v1 family is `182 PASS / 4 SKIP / 0 FAIL`. B6-I3 remains `SOURCE IMPLEMENTATION REPO-TESTED / CLOSED`; B6-S1 is next as local synthetic HTTP/Docker qualification with no real provider. No provider, official AML service, deployment, runtime/live data mutation, Gateway/config/plugin change, or tag operation occurred.

## 2026-08-30

### B6-I3-R2 Docker Native Dependency Builder Hardening

- B6-S1 Phase A is `PASS`. The initial B6-S1 Phase B Docker build is recorded as `FAIL_WITH_FINDINGS`: with the frozen `node:24-bookworm-slim` base, Node 24, and `better-sqlite3@11.10.0`, `npm ci --omit=dev` fell through from `prebuild-install` to `node-gyp rebuild --release`, but the dependencies stage had no Python/build toolchain. The first incorrect boundary was packaging, not the native dependency versions or production retrieval path.
- B6-I3-R2 is the bounded Docker packaging correction. `Dockerfile.aml` now installs `python3`, `make`, and `g++` only in the discarded dependencies stage with `--no-install-recommends`, cleans `/var/lib/apt/lists/*`, and runs the existing `npm ci --omit=dev`. The independent runtime stage remains `node:24-bookworm-slim` and copies only the generated `node_modules` plus the existing application files. No package-lock, Node, LanceDB, AML service, or production Hybrid changes were made.
- The whitespace-robust Docker contract covers the dependencies-stage toolchain and cleanup, independent runtime base, no runtime `apt-get install`, and retained `USER node`, `EXPOSE 8080`, `VOLUME`, `HEALTHCHECK`, and `CMD`. Node 24 targeted HTTP/service/Docker checks passed; the complete Benchmark family passed `182` tests with `4` expected skips and `0` failures; static check passed `737` files; and `git diff --check` passed.
- The real `docker build -f Dockerfile.aml -t memory-engine-aml:b6-i3-r2 .` passed. Inside the image, `require("better-sqlite3")` and `import("@lancedb/lancedb")` produced `NATIVE_RUNTIME_OK`; the default container UID was `1000`; and `python3`, `make`, `g++`, and apt-cache entries were absent from the final runtime image. BuildKit emitted its existing secret-like ENV warning for `AML_AUTH_MODE`; it is outside this bounded native-builder correction and did not affect the build.
- B6-I3-R2 is `SOURCE IMPLEMENTATION REPO-TESTED / DOCKER BUILD QUALIFIED`; B6-I3 is `CLOSED`; B6-S1 is `RETRY READY / PHASE B→D REMAINING`. This acceptance did not execute or close B6-S1 Phase C/D, request a real provider, run official AML, deploy, touch live runtime/data, change Gateway/config/plugin state, or tag/push the image.

### B6-I3-R3 Docker Runtime Module Closure Hardening

- B6-I3-R2 is `PASS / DOCKER NATIVE DEPENDENCY PACKAGING FIXED`, and B6-S1 Phase B is `PASS`. The initial B6-S1 Phase C attempt is recorded as `FAIL_WITH_FINDINGS`: the real AML container exited with `ERR_MODULE_NOT_FOUND` because `/app/query-utils.js` was absent while `lib/recall/hybrid-search.js` imported it.
- B6-I3-R3 is the bounded Docker runtime file-closure correction. `Dockerfile.aml` now explicitly copies `query-utils.js` into the runtime image. The Docker contract freezes only the dependencies output plus `package.json`, `package-lock.json`, `query-utils.js`, `bin/`, and `lib/`; `COPY . .` remains forbidden. No `query-utils.js` logic, adapter, backend, provider, dependency, Node, or retrieval change was made.
- The Node 24 Docker contract, HTTP/service regressions, complete Benchmark family, static check, and diff check passed. Rebuild `memory-engine-aml:b6-i3-r3` passed `SERVICE_IMPORT_OK`, `NATIVE_RUNTIME_OK`, and non-root UID validation. Real provider calls, official AML calls, live runtime/data, deployment, Gateway/config/plugin changes, and tag/push were not performed.
- B6-I3-R3 is `SOURCE IMPLEMENTATION REPO-TESTED / DOCKER RUNTIME CLOSURE QUALIFIED`; B6-I3 is `CLOSED`; B6-S1 is `RETRY READY / PHASE C→D REMAINING`. This correction does not execute or close B6-S1 Phase C/D.

### B6-S1 Local Synthetic HTTP/Docker Qualification Closure

- B6-S1 resumed at source `b2063f4531a5b0136de33ffee7e31c5235017028` after independent R3 acceptance. The rebuilt real image passed Docker contract, `SERVICE_IMPORT_OK`, `NATIVE_RUNTIME_OK`, and non-root UID `1000`; the BuildKit `AML_AUTH_MODE` secret-like ENV warning remained non-blocking because the value is the non-secret mode selector `auto`.
- Phase C used the real benchmark HTTP service, actual temporary SQLite/LanceDB state, a dedicated local Docker network and named volume, and a deterministic local 2,560-dimensional fake embedding endpoint. `/health` returned `{"status":"ok"}`; unauthenticated Search returned HTTP `401` / `authentication_required`; Add and Search returned the expected synthetic memory; provider calls were exactly `2` after Add plus Search; retrying the identical `request_id` returned the same success with provider calls still `2`; unknown-user Search returned `{"data":[]}` and user-directory count remained `1`.
- Phase D stopped the first AML container through Docker SIGTERM handling with exit `0`. The retained volume preserved `manifest.json`, per-user `core.sqlite`, `engine.sqlite`, `embedding-cache.sqlite`, and `user-manifest.json`. A fresh second container started on the same volume and, without re-Add, returned the same memory id/content/score; provider calls remained `2`, proving persistent query-embedding cache reuse across restart. The second container also stopped with exit `0`; manifest and the single user root still existed before cleanup; all synthetic containers, network, and volume were then removed.
- Final adjudication: `B6-S1 PASS / CLOSED`. Real SiliconFlow provider calls, official AML service/evaluation, public deployment, live OpenClaw Core/Engine/LanceDB mutation, Gateway/config/plugin changes, and tag/push operations were not performed. B6-I3 remains `CLOSED`; B6-S2 official AML smoke and B6-S3 full evaluation remain `NOT AUTHORIZED`.

### memory-engine 1.1 Recall Quality roadmap acceptance

- Accepted the post-1.0 product line as `memory-engine 1.1 — Recall Quality`. The primary product question shifts from the closed storage/authority baseline to recall triggering, query scope, candidate coverage, cross-session evidence composition, and ranking complete/current evidence into a `3–5` item budget.
- Set `1.1-Q0 Production Recall Failure Review` to `NOW / NOT STARTED`, with `30–50` real failures classified at the first loss as `WRITE_MISS`, `TRIGGER_MISS`, `CANDIDATE_MISS`, `RANK_MISS`, or `USE_MISS`. This documentation update does not authorize private production-case reads or telemetry creation.
- Set Q1 Product Metric Contract, Q2 Minimal Channel Ablation, and Q3 Retrieval Architecture Decision to `NEXT`. New core metrics emphasize @3 evidence quality, staleness/conflict/pollution, and p50/p95 latency; the bounded architecture candidate is lexical/metadata first, selective semantic fallback, session/episode reconstruction, and lightweight rerank.
- Kept Recall Hint v1, Statistical LTR, Shadow AutoRecall, and Owner-only canary as Q4–Q7 `LATER`. Recall Hint is candidate expansion/soft weighting only and cannot grant recall/skip/hard-filter authority. RH1 and H2 remain closed; B6-S1 remains `PASS / CLOSED / NO REAL PROVIDER`, while B6-S2/S3 remain an optional unauthorized evaluation branch.
- Updated `docs/stabilization-plan.md`, `docs/memory-engine-benchmark-v1.md`, and `docs/current-state.md`. No runtime, Gateway, config, DB, LanceDB, live plugin, provider, deployment, tag, or push operation was performed; `AutoRecall=false` and production `topK=3` remain unchanged.

### memory-engine 1.1 roadmap correction: QA0 and event-time boundary

- Accepted the audit-driven sequencing correction: `1.1-QA0 Identity & Lifecycle Correctness Gate` is now `NOW / NOT STARTED`; `1.1-Q0 Production Recall Failure Review` moves to `THEN / NOT STARTED`; Q1–Q3 remain the subsequent recall-quality work, and no runtime authority is implied.
- Bounded QA0 to source-level regression coverage for exact identity and fail-closed mutation, current-state reinforcement without clearing conflicts, exact checkpoint binding, safe category routing, explicit Add outcomes, complete vectorization, and Core read-only contract consistency. A new `memory_entries` aggregate, ranking/vector retuning, retrieval rewrite, event-time write activation, and live state mutation remain out of scope.
- Recorded the P46 event-time boundary in the current roadmap: `memory_event_times` is an Engine-owned schema MVP/proposed sidecar, the real-engine preview reported `exists=false`, `would_create=true`, and `writes_db=false`, writes default to `denied_by_default_write_guard`, and the sidecar is not integrated with recall, checkpoint, or ranking. Exact `event_at` still requires trustworthy provenance; `updated_at`, file mtime, batch/import time, and smart-add path dates are not substitutes; `date_only` and `unknown` remain valid states. Benchmark temporal scores therefore do not qualify a structured event-time channel.
- Updated `docs/stabilization-plan.md`, `docs/memory-engine-benchmark-v1.md`, and `docs/current-state.md` to preserve the B6 facts and the offline/runtime boundary. No source implementation, provider request, runtime/config/Gateway/DB/LanceDB/live-plugin mutation, deployment, tag, or push operation was performed; `AutoRecall=false` and production `topK=3` remain unchanged.

### memory-engine 1.1 roadmap correction: executable repository gate

- Expanded QA0 without creating another governance stage. `1.1-QA0 Executable Correctness & Repository Integrity Gate` remains `NOW / NOT STARTED`; Q0 remains `THEN / NOT STARTED`.
- Accepted three immediate engineering-integrity blockers inside QA0: production/public `top_k` must receive consistent finite-integer range validation and one production-owned maximum before retrieval work; empty-shell and token/phase-string-only tests must stop counting as acceptance evidence; and one repository-owned executable review-gate command must compose `npm run check`, required behavior/contract tests, applicable OpenSpec strict validation, `git diff --check`, and test-integrity checks. Benchmark-local AML `top_k<=100` remains benchmark authority only and does not set the production maximum.
- Clarified that `npm run check` is syntax-only, raw test-file/pass counts and narrative PASS text are not sufficient acceptance evidence, and Planner acceptance requires fresh executable-gate output plus independent DevSpace spot-checks. `docs/current-state.md` remains the single current-state authority; the new gate is enforcement, not another documentation layer.
- Updated `docs/stabilization-plan.md`, `docs/current-state.md`, and `docs/memory-engine-benchmark-v1.md`. No source implementation, test deletion, runtime/config/Gateway/DB/LanceDB/live-plugin mutation, deployment, provider call, tag, or push operation was performed.

## 2026-09-02

### QA0 roadmap update from GitHub repository audit

- Reviewed the external GitHub audit against the current local repository rather than accepting its 101 findings as one undifferentiated backlog. The audit target was older than the local development line; local `main` at review time was `267ec403efd13c8ec5583faf32590d28089a3848`, with the first QA0 tranche already source-implemented.
- Moved QA0 from `NOW / NOT STARTED` to `NOW / IN PROGRESS`. Recorded the completed local source tranche: executable review gate, test-integrity enforcement including token-only rejection, production `top_k` bounds, current-state reinforcement authority, cite-to-served-result binding, exact checkpoint confidence binding, artifact/secret routing protection, complete Add propagation, and closure of legacy Core-write mutation authority. No live-runtime qualification is inferred from these source commits.
- Accepted six remaining pre-Q0 blockers: archived-vector exclusion; KG relevance ordering before truncation/RRF; one canonical smart-add writer/fingerprint contract with non-silent duplicate-detection failure; one configured business-date/time-zone contract; retirement of the legacy attached-Core capability in favor of `isolated-dbs.js`; and visible invalid-config handling for retrieval/AutoRecall settings.
- Added two parallel security hardenings: Console Host/auth boundary, and explicit untrusted-data boundaries for recalled memory/checkpoint raw-log prompts. The prompt boundary is a hard prerequisite for broader AutoRecall activation but does not reopen runtime rollout while `AutoRecall=false`.
- Classified static performance findings as measure-first Q1/Q2 inputs rather than immediate P1 rewrites. Full-table candidate metadata loading, backfill cost, event DDL overhead, channel serialization, and Console aggregation should be changed only after p50/p95 and channel-cost evidence identifies a real bottleneck.
- Rejected the audit recommendation to wire `lib/runtime-authority/` into `before_prompt_build`: the subsystem remains candidate/runtime qualification tooling unless a separate product decision changes its role. TypeScript/Prettier adoption is not a QA0 requirement; GitHub CI remains useful repository/release hygiene once the executable review contract is stable.
- Updated `docs/stabilization-plan.md`, `docs/current-state.md`, and `docs/memory-engine-benchmark-v1.md`. This roadmap update performs no runtime/config/Gateway/DB/LanceDB/live-plugin mutation, deployment, provider request, tag, or push operation.

## 2026-09-05

### Architecture/code-review repair plan and roadmap reconciliation

- Reviewed the current production architecture with `code-review-graph`, CodeGraph, source inspection, and focused tests. The physical Core-readonly / isolated-Engine boundary, canonical read adapter, channelized hybrid retrieval, bounded tool surfaces, and source→derived-state Add model remain accepted; no architecture rewrite is justified.
- Accepted R1 `NOW / PARALLEL / Level A`: replace SQL `LIKE prefix%` ID lookup in `memory_engine update` and Owner-only `memory_engine_get` with literal exact-or-unique-prefix semantics, and remove/replace the incorrect aggregate `cite.next_confidence` response. This repair lane must not change retrieval ranking or frozen Q1/Q2 benchmark semantics.
- Accepted R2 as a separate controlled persistent-write change: close the smart-add read-check-append TOCTOU race with the smallest writer-serialization/lock mechanism that matches the real process topology. It is not a Q2-B2 blocker.
- Accepted R3 as Q3 input, not an immediate patch: hybrid retrieval currently truncates to K before canonical projection, so failed projected candidates are not backfilled from lower ranks. Any change to `rank → canonical validate → take K valid` changes serving semantics; therefore it must wait until Q2-B2 completes and, if adopted, receive a new same-profile Q1 baseline before semantic comparison.
- Accepted R4 post-Q3 behavior-preserving extraction of the oversized `hybridSearch`, `createMemoryEngineExecute`, and `createAutoRecallHookLifecycle` orchestrators. No rewrite or policy/ranking change is implied.
- Preserved two later AutoRecall activation blockers: the deterministic intent gate must be removed/bypassed/requalified before regaining production authority, and recalled raw text needs explicit untrusted-data/instruction isolation. Card-first projection is preferred over further raw-text regex hardening. `AutoRecall=false` remains unchanged.
- Accepted R6 governance slimming as maintainability policy rather than another stage: preserve `npm run review`, test-integrity, physical DB isolation, and Owner authorization for genuinely high-risk runtime/data changes; stop adding permanent audit/readiness/evidence modules for ordinary Level A/B work; after Q2/Q3, move or retire closed historical qualification machinery from production retrieval namespaces when Git history/frozen evidence is sufficient.
- Reconciled the roadmap to actual progress: Q0 `PASS_WITH_FINDINGS / CLOSED`; Q1 metric contract `CLOSED` and current lexical baseline `FROZEN`; Q2-A1/A2 `CLOSED`; Q2-B1 `PASS / CLOSED`; Q2-B2 always-vector semantic execution is `NEXT`; Q3 follows with R3 as an explicit architecture input. Selective vector remains unimplemented until Q2-B2 measures semantic headroom.
- Updated `docs/stabilization-plan.md`, `docs/current-state.md`, and `docs/memory-engine-benchmark-v1.md`. Documentation planning only: no provider request, private production read, runtime/config/Gateway/DB/LanceDB/live-plugin mutation, deployment, AutoRecall activation, tag, or push operation was performed.

### R1 tool ID/result correctness source repair

- Implemented the bounded Level A R1 repair in `lib/tools/memory-engine-actions.js`. `memory_engine update` and Owner-only `memory_engine_get` no longer use SQL `LIKE prefix%`; both use literal `substr(...)=...` prefix selection plus one exact-or-unique resolver. Exact IDs take precedence over longer IDs that share the same prefix; ambiguous and missing lookups fail closed; `%` and `_` are treated as literal ID characters.
- Removed the fabricated aggregate `cite.next_confidence` field. Reinforcement still uses the authoritative per-row decayed confidence calculation in `batchReinforce`; the tool now reports only success, reinforced count, and bounded reinforced IDs rather than inventing one confidence value for multiple rows.
- Added `test/memory-engine-id-resolution.test.js` covering literal wildcard characters, exact-ID precedence, and SQL-shape regression; strengthened `test/memory-engine-cite.test.js` to require the fabricated field to stay absent. Existing Owner-only registration, managed-first ambiguous-prefix ordering, search projection, and cite authorization tests remain passing.
- Fresh focused verification: `node --test test/memory-engine-id-resolution.test.js test/memory-engine-cite.test.js test/memory-engine-tool-wrappers.test.js` = `18/18 PASS`; `npm run check` = PASS over `715` files; `npm run test:integrity` = PASS (`322` tests scanned, `0` invalid); `git diff --check` = PASS. A broader adjacent tool run produced `49 PASS / 3 ENVIRONMENT_FAILURE`, with all three failures occurring when real SQLite fixtures attempted to load Node-24-built `better-sqlite3` under DevSpace Node 22.
- `npm run review` reached `static_check=PASS`, `test_integrity=PASS`, `openspec_strict=PASS`, and `diff_check=PASS`, while aggregate `node_test=FAIL`. The failure surface is environmental: the repository native module is ABI `137` while DevSpace Node 22 requires ABI `127`; direct native failures and sampled higher-level HTTP/semantic assertion failures trace back to the same load error. Do not rebuild native dependencies under Node 22 merely to make this workspace green. A fresh Node 24 aggregate run remains the correct full-suite evidence when available.
- R1 is therefore recorded as `SOURCE IMPLEMENTED / FOCUSED-VERIFIED / COMMIT PENDING`. No retrieval ranking, Q1/Q2 benchmark fixture/metric/profile, AutoRecall setting, provider call, live DB/LanceDB, Gateway/config/plugin runtime, deployment, tag, or push operation changed.

...
export function detectRelatedConflicts(db, {
  ...
  return {
    ...
    ids,
    dry_run: Boolean(dryRun),
    note: "Lower-confidence related non-preference chunks in same category with divergent hit counts flagged",
  };
}
...
export function detectRelatedConflictsIsolated({
  withEngineDb,
  withCoreDb,
  overlapThreshold = 0.2,
  dryRun = false,
  ...
  if (typeof withEngineDb !== "function") throw new TypeError("detectRelatedConflictsIsolated requires withEngineDb");
  if (typeof withCoreDb !== "function") throw new TypeError("detectRelatedConflictsIsolated requires withCoreDb");

  const pairs = withEngineDb(db => db.prepare([
    "SELECT m1.chunk_id AS id1, m2.chunk_id AS id2,",
    "m1.category, m1.confidence AS c1, m2.confidence AS c2,",
    "m1.hit_count AS h1, m2.hit_count AS h2",
    "FROM memory_confidence m1",
  ...
  const idsToLoad = [...new Set(pairs.flatMap(row => [row.id1, row.id2]).filter(Boolean))];
  const chunkMap = withCoreDb(db => {
    const map = new Map();
    for (const batch of chunked(idsToLoad)) {
      if (batch.length === 0) continue;
      const placeholders = batch.map(() => "?").join(", ");
      const rows = db.prepare(`SELECT id, text, path FROM chunks WHERE id IN (${placeholders})`).all(...batch);
      for (const row of rows) map.set(String(row.id), row);
    }
    return map;
  });

  const enriched = pairs.flatMap(row => {
    const left = chunkMap.get(String(row.id1));
    const right = chunkMap.get(String(row.id2));
  ...
  if (!dryRun && ids.length > 0) {
    withEngineDb(db => {
      ...
      const transaction = db.transaction(() => {
        for (const id of ids) {
          const info = update.run(id);
          if (Number(info?.changes ?? 1) > 0 && typeof onFlagged === "function") onFlagged(id, enriched.length);
        }
      });
      transaction();
    });

...
async function materializeVectorCorpus({ corpus, runtime, table, embed, flattenMemoryRecords }) {
  const memoryIds = flattenMemoryRecords(corpus).map(row => row.id);
...
export async function createQ4RecallHintC1BSemanticSessionV1({
  ...
  materializeCorpus = materializeQ4RecallHintC1CorpusV1,
  flattenMemoryRecords = flattenQ4RecallHintC1MemoryRecordsV1,
  keepTemp = false,
} = {}) {
  ...
  if (typeof materializeCorpus !== "function") throw fail("Q4_C1B_RETRIEVAL_MATERIALIZER_REQUIRED");
  if (typeof flattenMemoryRecords !== "function") throw fail("Q4_C1B_RETRIEVAL_FLATTENER_REQUIRED");

  const materialized = materializeCorpus(corpus);
  ...
  const runArm = async ({ row, plan = null, recallHintVectorExecutionMode = "sequential" } = {}) => {
    ...
    benchmarkRuntime.runtime.recallHintVectorExecutionMode = recallHintVectorExecutionMode;
    ...
    return Object.freeze({
      ...
      pool_vector_ms: Number(poolSearch?.debug?.vector_ms),
      ranked_vector_ms: Number(rankedSearch?.debug?.vector_ms),
      vector_query_execution: String(poolSearch?.debug?.vector_query_execution || "sequential"),
    });
  };

...
export function createAmlUserDataPlane({
  ...
  function createSearchRuntime() {
    return createBenchmarkHybridRuntime(
      { corePath, enginePath },
      {
        ...
        vectorTable: selectedVectorBackend?.table || null,
        generateEmbedding: selectedVectorBackend?.generateEmbedding || (async () => [0]),
  ...
  return {
    ...
    enginePath,
    vectorPath,
    vector_mode: selectedVectorBackend ? "injected_per_user" : "disabled",
...
export function createAmlDataPlaneRegistry({
  ...
  function getOrCreate(userId) {
    ...
    mkdirSync(vectorPath, { recursive: true });
    const vectorBackend = typeof vectorBackendFactory === "function"
      ? vectorBackendFactory({ userId, root: userRoot, vectorPath })
      : null;

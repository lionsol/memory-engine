...
export const LOCOMO_DIALOG_PROJECTION_VERSION = "locomo_dialog_projection_v1";
export const LOCOMO_INCLUDE_SESSION_DATETIME = true;
export const LOCOMO_BLIP_CAPTION_POLICY = "include_when_present";
...
function normalizeSessions(conversation) {
  ...
  for (const entry of enumerateSessions(conversation)) {
    const normalizedTurns = entry.turns.map((rawTurn, turnIndex) => {
      ...
      // the neutral question-owned projection and are used by the frozen
      // conversation corpus projection below when present.
...
export function buildLocomoDialogText(turn, {
  ...
  const datePrefix = includeSessionDatetime
    ? `(${requireString(sessionDateTime, "session_date_time")}) `
    : "";
  const rawText = `${datePrefix}${speaker}: ${turn.text}`;
...
export function buildLocomoConversationDialogDocuments(record, {
  ...
  return item.sessions.flatMap(session => session.turns.map(turn => ({
    ...
    source_id: `${item.sample_id}:${session.session_id}:${turn.dia_id}`,
    content: buildLocomoDialogText(turn, {
      includeBlipCaption: true,
      includeSessionDatetime: true,

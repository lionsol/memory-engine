...
export async function runQ5FixedPoolScoreCaptureV1({
  ...
  let usageBilledInputTokens = 0;
  let usageBilledOutputTokens = 0;
  for (const row of rows) {
    ...
    usageInputTokens += Number(capture.usage?.input_tokens ?? capture.usage?.prompt_tokens ?? 0);
    usageOutputTokens += Number(capture.usage?.output_tokens ?? capture.usage?.completion_tokens ?? 0);
    usageTotalTokens += Number(capture.usage?.total_tokens || 0);
    usageBilledInputTokens += Number(capture.usage?.billed_input_tokens || 0);
    usageBilledOutputTokens += Number(capture.usage?.billed_output_tokens || 0);
  }
  ...
  return Object.freeze({
    ...
    provider_usage: Object.freeze({
      ...
      billed_input_tokens: usageBilledInputTokens,
      billed_output_tokens: usageBilledOutputTokens,
    }),
    packet,
  });
}

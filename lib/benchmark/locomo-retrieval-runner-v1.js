...
export const LOCOMO_LEXICAL_RETRIEVAL_PROFILE = "production_hybrid_lexical_locomo_dialog_v1";
 ...
 */
export function materializeLocomoConversationDataPlane(record, {
  ...
  try {
    ...
    const insertAll = core.transaction(() => {
      for (const document of documents) {
        ...
        const updatedAt = documentUpdatedAt(document, benchmarkNowSec);
        insertChunk.run(
          document.memory_id,
          path,
 ...
 */
export function createLocomoProductionHybridRuntime(materialized, { topK = 50 } = {}) {
  const runtimeAdapter = createBenchmarkHybridRuntime(materialized, {
...
function skippedMetrics(question) {
  return {
    metric_level: null,
...
function aggregatePolicy(results, policy) {
...
export async function runLocomoLexicalRetrievalDataset(records, {
  limit = null,
  topK = 50,
  benchmarkNowSec = Math.floor(Date.now() / 1000),
  includeBlipCaption = false,
  ...
  return {
    ...
    run: {
      ...
      include_blip_caption: includeBlipCaption,

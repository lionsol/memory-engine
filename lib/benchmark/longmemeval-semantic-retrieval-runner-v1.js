...
function assertNoReservedProvenanceFields(value) {
  const conflicts = Object.keys(value).filter(key => CORE_PROVENANCE_FIELDS.has(key));
  if (conflicts.length > 0) {
    throw new LongMemEvalSemanticProfileError(
      "provenance",
      "profile provenance cannot override core provenance fields",
...
function provenance({
  ...
  const extensionProvenance = profileProvenance && typeof profileProvenance === "object" && !Array.isArray(profileProvenance)
    ? profileProvenance
    : {};
  try {
    assertNoGoldFields(extensionProvenance, "semantic_profile_provenance");
    assertNoReservedProvenanceFields(extensionProvenance);
  } catch (error) {
...
function vectorDiagnostics(search, stats, vectorTopK) {
  ...
  if (debug.vector_query_mode) {
    Object.assign(diagnostics, {
      vector_query_mode: debug.vector_query_mode,
      vector_query_count: Number(debug.vector_query_count || 0),
      vector_search_count: Number(debug.vector_search_count || 0),
      vector_query_fusion: debug.vector_query_fusion || null,
      vector_query_rrf_k: Number(debug.vector_query_rrf_k || 0),
      vector_raw_total: Number(debug.vector_raw_total || 0),
      vector_unique_count: Number(debug.vector_unique_count || 0),
      vector_query_input_sha256s: Array.isArray(debug.vector_query_input_sha256s)
        ? [...debug.vector_query_input_sha256s]
        : [],
      vector_query_candidate_counts: Array.isArray(debug.vector_query_candidate_counts)
...
function vectorProfileProvenance(search) {
  const debug = search?.debug || {};
  if (!debug.vector_query_mode) return {};
  return {
    vector_query_mode: debug.vector_query_mode,
    vector_query_count: Number(debug.vector_query_count || 0),
    vector_search_count: Number(debug.vector_search_count || 0),
...
function validateSemanticSearch(search, {
  ...
  if (vectorQueryPlan !== null && vectorQueryPlan !== undefined) {
    const isBoundedMultiQuery = debug.vector_query_mode === "bounded_multi_query" &&
      Number(debug.vector_query_count) === 3 &&
      Number(debug.vector_search_count) === 3 &&
      debug.vector_query_fusion === "rrf" &&
      Number(debug.vector_query_rrf_k) === 60 &&
      debug.vector_multi_query_failed !== true;
    if (!isBoundedMultiQuery) {
      stats.vectorErrorCount += 1;
      throw new LongMemEvalSemanticProfileError(
        "vector_not_attempted",
        "bounded multi-query vector contract was not completed",
        {
          vector_query_mode: debug.vector_query_mode || null,
          vector_query_count: Number(debug.vector_query_count || 0),
          vector_search_count: Number(debug.vector_search_count || 0),
          vector_query_fusion: debug.vector_query_fusion || null,
          vector_query_rrf_k: Number(debug.vector_query_rrf_k || 0),
...
export async function runLongMemEvalSemanticRetrievalCase(record, {
  ...
  repositoryProvenanceSource = null,
  profile = LONGMEMEVAL_SEMANTIC_PROFILE,
  queryEmbeddingInputTransform = null,
  vectorQueryPlan = null,
  profileProvenance = null,
  ...
  try {
    ...
    const queryEmbedding = async query => {
      ...
      try {
        const embeddingInput = typeof queryEmbeddingInputTransform === "function"
          ? queryEmbeddingInputTransform(query)
          : query;
        if (typeof embeddingInput !== "string") {
          throw new LongMemEvalSemanticProfileError(
            "query_embedding_input",
            "query embedding input transform must return a string",
          );
        }
        return await getEmbedding(embeddingInput, "query", { provider, cache, stats });
      } catch (error) {
    ...
    let search;
    try {
      search = await hybridSearchFn(item.question, { topK: k }, runtime.runtime);
    } catch (error) {
      stats.vectorErrorCount += 1;
    ...
    validateSemanticSearch(search, { stats, fallbackCalled, queryEmbeddingError, vectorQueryPlan });
    ...
    const itemProvenance = provenance({
      ...
      profile,
      profileProvenance: {
        ...(profileProvenance && typeof profileProvenance === "object" && !Array.isArray(profileProvenance)
          ? profileProvenance
          : {}),
        ...vectorProfileProvenance(search),
      },
      provider: SEMANTIC_EMBEDDING_PROVIDER,
      baseUrlIdentity: provider.baseUrlIdentity,
      stats,

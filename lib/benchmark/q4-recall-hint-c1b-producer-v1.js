...
function requestCost(inputTokens, outputTokens, packet) {
  return (
    inputTokens * packet.input_price_per_million
    + outputTokens * packet.output_price_per_million
  ) / 1_000_000;
}
...
export async function executeQ4RecallHintC1BProducerV1({ producerInput, packet, transport } = {}) {
  ...
  const cost = requestCost(inputTokens, outputTokens, packet);
  if (cost > packet.max_cost) {

...
function normalizeBenchmarkChannelCapabilities(channelCapabilities) {
  const value = channelCapabilities === undefined ? {} : channelCapabilities;
  if (value === null || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("benchmark_channel_capabilities_must_be_object");
  }
  const allowedKeys = new Set(["isolatedKg", "isolatedRecent"]);
  const unknownKeys = Object.keys(value).filter(key => !allowedKeys.has(key));
  if (unknownKeys.length > 0) {
    throw new Error(`benchmark_channel_capabilities_unknown:${unknownKeys.join(",")}`);
  }
  const capabilities = {
    isolatedKg: value.isolatedKg === undefined ? true : value.isolatedKg,
    isolatedRecent: value.isolatedRecent === undefined ? true : value.isolatedRecent,
  };
  for (const [key, enabled] of Object.entries(capabilities)) {
    if (typeof enabled !== "boolean") {
      throw new Error(`benchmark_channel_capability_boolean_required:${key}`);
    }
  }
...
export function createBenchmarkHybridRuntime(materialized, {
  topK,
  topKPolicyMax = PRODUCTION_TOP_K_MAX,
  vectorTable = null,
  ...
  const selectedChannelCapabilities = normalizeBenchmarkChannelCapabilities(channelCapabilities);
  ...
  const cfg = {
    confidence: { min: Number.isFinite(Number(minConfidence)) ? Number(minConfidence) : 0 },
    recall: recallConfig,
  };
  ...
  const runtime = {
    withHybridDbAccessScope: async callback => callback({
      ...
      capabilities: {
        isolatedFts: true,
        isolatedKg: selectedChannelCapabilities.isolatedKg,
        isolatedRecent: selectedChannelCapabilities.isolatedRecent,
        legacyFallbackAllowed: false,
      },
    }),
    calcRealtimeConf,
    categoryMap: CATEGORY_MAP,
    topKPolicy: createTrustedTopKPolicy({ max: topKPolicyMax }),
    cfg,
    // Benchmark-only channel controls. Production hybridSearch does not read
    // this property; the access-scope capability payload above is the only
    // control consumed by the production orchestration.
    channelCapabilities: selectedChannelCapabilities,
  ...
  if (searchNowSec !== undefined) runtime.searchNowSec = searchNowSec;
...
export async function runLongMemEvalRetrievalDataset(records, {
  ...
  for (const record of selected) {
    results.push(await runLongMemEvalRetrievalCase(record, {
      topK,
      benchmarkNowSec,
      channelCapabilities,
    }));

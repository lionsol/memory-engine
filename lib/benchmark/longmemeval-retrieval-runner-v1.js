...
function materializeCaseDatabases(item, { benchmarkNowSec = Math.floor(Date.now() / 1000) } = {}) {
  ...
  try {
    ...
    const insertAll = core.transaction(() => {
      for (let index = 0; index < item.sessions.length; index += 1) {
        ...
        const memoryId = sha256(`${item.question_id}\u0000${session.session_id}`);
...
function createBenchmarkHybridRuntime(materialized, { topK }) {
  ...
  return {
    ...
    runtime: {
      ...
      // The B2 profile intentionally has no semantic backend. Supplying an
      // empty deterministic vector backend prevents production Hybrid from
      // emitting fallback warnings when FTS has no hits, while still returning
      // zero vector candidates.
      getLancedbRuntime: async () => ({
        table: emptyVectorTable,
        readyState: "ready",
        initError: null,
        timedOut: false,
      }),
...
export async function runLongMemEvalRetrievalCase(record, {
  ...
  const materialized = materializeCaseDatabases(item, { benchmarkNowSec });
  let runtime = null;
  try {

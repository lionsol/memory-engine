...
export function adjudicateQ0FailureEvaluationCase(input) {
  ...
  const markUnresolved = (stage, reason) => {
  ...
  for (const stage of Q0_STAGES) {
    ...
    if (evidence.state === 'PASS') {
      if (evidence.evidence_complete !== true) {
        markUnresolved(stage, 'REQUIRED_STAGE_UNRESOLVED');
      }
      if (
        input.evaluation_scope === 'END_TO_END_OBSERVED'
        && stage === 'rank'
        && evidence.evidence_complete === true
        && input.effective_top_k !== 3
      ) {
        markUnresolved(stage, 'Q0_END_TO_END_RANK_PREFIX_REQUIRES_EFFECTIVE_TOP_K_3');
      }
      continue;
    }
    ...
    if (evidence.state === 'FAIL') {
      ...
      if (unresolvedStage !== null) {
        return insufficient(
          result,
          unresolvedStage,
          unresolvedReason === 'Q0_END_TO_END_RANK_PREFIX_REQUIRES_EFFECTIVE_TOP_K_3'
            ? unresolvedReason
            : 'EARLIER_STAGE_UNRESOLVED',
        );
      }
  ...
  if (unresolvedStage !== null) {
    return insufficient(
      result,
      unresolvedStage,
      unresolvedReason ?? 'REQUIRED_STAGE_UNRESOLVED',
    );
  }

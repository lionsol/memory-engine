...
function scoreUnknownCase(identity, reason, extra = {}) {
  return {
    ...
    budget: {
      ...
      old_gold_evidence_count_heuristic_used: false,
    },
    ...extra,
  };
}
...
export function scoreLocomoChunkCase({
  ...
  const evidenceRows = distinctEvidenceIds.map(evidenceId => {
    if (typeof evidenceId !== "string" || evidenceId.length === 0) {
      return { evidenceId, status: "unknown", reason: "evidence_id_invalid", chunkIds: [], unknownChunkIds: [] };
    }
    const target = index.turnsById.get(turnKey(identity.sampleId, evidenceId));
    if (!target) return { evidenceId, status: "unknown", reason: "evidence_turn_not_found", chunkIds: [], unknownChunkIds: [] };
    return coverageForEvidence({ target, selectedChunks: top3Chunks });
  });
  ...
  const idealGains = [...gains].toSorted((left, right) => right - left);
  ...
  const metrics = scoreable
    ? {
      ...
      "ndcg@3": idealDcg === 0 ? 0 : dcg(gains) / idealDcg,
  ...
  return {
    ...
    ndcg_definition: "rank gain is a newly completed official evidence turn after unioning the current chunk; IDCG sorts the observed top3 gains, so this is an order metric for the selected chunk set",
  };
}
...
function legacyMetricView(row) {
  ...
  return {
    scoreable: score?.scoreable === true || row?.scoreable === true,
    unknown: score?.unknown === true || score?.unknown_or_unscoreable === true || row?.scoreable === false,
...
export function alignLocomoChunkScoreRows({ oldRows = [], chunkRows = [] }) {
  ...
  const rows = ids.map(questionId => {
    ...
    return {
      ...
      chunk_score: chunk ? {
        scoreable: chunk.scoreable === true,
  ...
  const oldScoreable = oldRows.filter(row => legacyMetricView(row).scoreable).length;
  ...
  return {
    ...
    summary: {
      ...
      old_unknown_case_count: oldRows.length - oldScoreable,
      chunk_case_count: chunkRows.length,
      chunk_scoreable_case_count: chunkScoreable,
      chunk_unknown_case_count: chunkRows.length - chunkScoreable,
...
export function scoreLocomoChunkCases({
  ...
  const cases = evidenceCases.map(item => {
    ...
    if (selection === undefined) {
      return scoreUnknownCase(identity, "selected_chunk_ids_missing", {
        ...
        evidence_ids: Array.isArray(item.evidence) ? item.evidence.map(entry => entry.evidenceId) : [],
  ...
  return {
    ...
    budget: {
      k: LOCOMO_CHUNK_EVIDENCE_BUDGET,
      unit: "complete_chunk_id",
      feasibility_definition: "observed full source-offset coverage by the selected top3 chunks; never inferred from gold evidence count",
    },
    cases,

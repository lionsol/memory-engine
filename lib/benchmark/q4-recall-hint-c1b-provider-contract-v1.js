...
} from "../recall/hint/recall-hint-v1.js";
import {
...
export const Q4_RECALL_HINT_C1B_PROMPT_VERSION = "q4_recall_hint_producer_prompt_v1";
...
export const Q4_RECALL_HINT_C1B_ABSOLUTE_MAX_COST_USD = 1;
export const Q4_RECALL_HINT_C1B_EGRESS_SCOPE = "frozen_q4_c1_query_plus_bounded_context_only";

export const Q4_RECALL_HINT_C1B_OUTPUT_SCHEMA = JSON.stringify({
  type: "object",
...
export const Q4_RECALL_HINT_C1B_PROMPT_TEXT = [
  ...
  `The output must satisfy this schema: ${Q4_RECALL_HINT_C1B_OUTPUT_SCHEMA}`,
].join(" ");
...
export const Q4_RECALL_HINT_C1B_PROMPT_SHA256 = sha256(Q4_RECALL_HINT_C1B_PROMPT_TEXT);
export const Q4_RECALL_HINT_C1B_OUTPUT_SCHEMA_SHA256 = sha256(Q4_RECALL_HINT_C1B_OUTPUT_SCHEMA);
...
export function buildQ4RecallHintC1BPrompt(producerInput) {
  ...
  return [
    Q4_RECALL_HINT_C1B_PROMPT_TEXT,
    `INPUT_JSON=${JSON.stringify({ query, bounded_context: boundedContext })}`,
  ].join("\n");
}
...
export function buildQ4RecallHintC1BExecutionPacket({
  ...
  if (!Number.isFinite(maxCostUsd) || maxCostUsd <= 0 || maxCostUsd > Q4_RECALL_HINT_C1B_ABSOLUTE_MAX_COST_USD) {
    throw fail("Q4_C1B_COST_CAP_INVALID");
  }
...
export function validateQ4RecallHintC1BExecutionPacket({ packet, manifest, sourceCommit, worktreeClean } = {}) {
  ...
  if (!Number.isFinite(packet.max_cost_usd)
      || packet.max_cost_usd <= 0
      || packet.max_cost_usd > Q4_RECALL_HINT_C1B_ABSOLUTE_MAX_COST_USD) {
    throw fail("Q4_C1B_COST_CAP_INVALID");
  }
  if (!Number.isFinite(packet.input_price_usd_per_million) || packet.input_price_usd_per_million < 0
      || !Number.isFinite(packet.output_price_usd_per_million) || packet.output_price_usd_per_million < 0) {
...
export function validateQ4RecallHintC1BUsage({
  ...
  if (!Number.isFinite(costUsd) || costUsd < 0) throw fail("Q4_C1B_USAGE_COST_INVALID");
  if (values.providerRequests > Q4_RECALL_HINT_C1B_MAX_PROVIDER_REQUESTS
      ...
      || costUsd > Q4_RECALL_HINT_C1B_ABSOLUTE_MAX_COST_USD) {
    throw fail("Q4_C1B_USAGE_BUDGET_EXCEEDED");
  }
  return Object.freeze({ ...values, costUsd });
}

...
import { buildQ4RecallHintC1FreshCorpusV1 } from "./q4-recall-hint-c1-corpus-v1.js";
import {
...
export const Q4_RECALL_HINT_C1_BASELINE_PREFLIGHT_SCHEMA = "memory_engine_q4_recall_hint_c1_baseline_preflight_v1";
export const Q4_RECALL_HINT_C1_BASELINE_PROFILE = "q4_c1_lexical_control_hybrid_v1";
export const Q4_RECALL_HINT_C1_BENCHMARK_NOW_SEC = 1_800_000_000;
...
export function materializeQ4RecallHintC1CorpusV1(corpus) {
  const normalized = validateQ4RecallHintC1CorpusV1(corpus);
  assertQ4RecallHintC1FrozenIdentityV1(corpus);
  const root = mkdtempSync(join(tmpdir(), "memory-engine-q4-c1-baseline-"));
  ...
  try {
    ...
    `);
    const records = flattenQ4RecallHintC1MemoryRecordsV1(normalized);
    const insertAll = core.transaction(() => {
      for (const [index, row] of records.entries()) {
        const path = `benchmark/q4-c1/${String(index).padStart(4, "0")}-${row.id}.md`;
        const hash = sha256(row.text);
        insertChunk.run(
          row.id,
          path,
          "benchmark_q4_recall_hint_c1",
          1,
          1,
          hash,
          "q4-c1-synthetic-v1",
          row.text,
          null,
          Q4_RECALL_HINT_C1_BENCHMARK_NOW_SEC,
        );
        insertFts.run(
          row.text,
          row.id,
...
function scoreCase(record, candidatePoolIds, rankedTop3Ids, latencyMs, split) {
...
function evaluateEligibility(rows) {
  const reasons = [];
  const acceptanceProtection = rows.filter(row => row.split === "acceptance" && row.family === "protection");
  if (acceptanceProtection.length !== 8) reasons.push("acceptance_protection_count_invalid");
  if (acceptanceProtection.some(row => row.pool_evidence_coverage !== 1)) {
    reasons.push("acceptance_protection_pool_not_complete");
  }
  if (acceptanceProtection.some(row => row.recall_all_at_3 !== 1)) {
    reasons.push("acceptance_protection_top3_not_complete");
  }
  for (const family of TARGET_FAMILIES) {
    const rowsForFamily = rows.filter(row => row.split === "acceptance" && row.family === family);
    if (rowsForFamily.length !== 8) reasons.push(`acceptance_target_count_invalid:${family}`);
    if (!rowsForFamily.some(row => row.pool_miss)) reasons.push(`acceptance_target_has_no_pool_miss:${family}`);
  }
  return {
...
export async function runQ4RecallHintC1BaselinePreflightV1({ keepTemp = false } = {}) {
  const corpus = buildQ4RecallHintC1FreshCorpusV1();
  const normalized = validateQ4RecallHintC1CorpusV1(corpus);
  const manifest = assertQ4RecallHintC1FrozenIdentityV1(corpus);
  const split = assignQ4RecallHintC1SplitV1(corpus);
  const splitById = new Map([
    ...split.development.map(caseId => [caseId, "development"]),
    ...split.acceptance.map(caseId => [caseId, "acceptance"]),
  ]);
  ...
  try {
    runtime = createBenchmarkHybridRuntime(materialized, {
      ...
      searchNowSec: Q4_RECALL_HINT_C1_BENCHMARK_NOW_SEC,
    });
    const rows = [];
    for (const record of normalized.cases) {
      ...
      rows.push(scoreCase(record, poolIds, top3Ids, latencyMs, splitById.get(record.case_id)));
    }
    const acceptance = summarize(rows, row => row.split === "acceptance");
    const development = summarize(rows, row => row.split === "development");
    const families = Object.fromEntries([
      ...TARGET_FAMILIES,
      "protection",
    ].map(family => [
      family,
      summarize(rows, row => row.split === "acceptance" && row.family === family),
    ]));
    return {
      schema: Q4_RECALL_HINT_C1_BASELINE_PREFLIGHT_SCHEMA,
      profile: Q4_RECALL_HINT_C1_BASELINE_PROFILE,

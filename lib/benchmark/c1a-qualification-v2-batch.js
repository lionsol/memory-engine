import { snapshotC1ACase } from "./c1a-qualification-manifest.js";
import { C1A_V2_MANIFEST_SCHEMA } from "./c1a-qualification-v2-contract.js";
import {
...
function compareSnapshot(expected, material) {
  ...
  for (const field of fields) {
    if (actual[field] !== expected[field]) {
      throw fail("C1A_V2_CASE_MATERIAL_IDENTITY_MISMATCH", {
...
function materialIndex(cases) {
  if (!Array.isArray(cases)) throw fail("C1A_V2_BATCH_CASES_MUST_BE_ARRAY");
  const index = new Map();
  for (const item of cases) {
    if (typeof item?.case_id !== "string" || item.case_id.length === 0) {
      throw fail("C1A_V2_BATCH_CASE_ID_INVALID");
    }
    if (index.has(item.case_id)) throw fail("C1A_V2_BATCH_CASE_ID_DUPLICATE", { caseId: item.case_id });
...
function selectedMainSnapshots(manifest) {
  const diagnostics = DIAGNOSTIC_ORDER.flatMap(key => {
    ...
    if (!Array.isArray(rows)) throw fail("C1A_V2_MANIFEST_DIAGNOSTIC_STRATUM_MISSING", { stratum: key });
    return rows.map(row => ({ ...row, qualification_stratum: key, qualification_population: "diagnostic" }));
  });
  if (!Array.isArray(manifest?.primary)) throw fail("C1A_V2_MANIFEST_PRIMARY_MISSING");
  return [
...
export async function runC1AV2QualificationBatch({
  manifest,
  cases,
  adapter,
  budget,
  ...
  if (manifest?.schema !== C1A_V2_MANIFEST_SCHEMA) throw fail("C1A_V2_MANIFEST_SCHEMA_MISMATCH");
  if (typeof adapter !== "function") throw fail("C1A_V2_BATCH_ADAPTER_REQUIRED");
  if (!budget || typeof budget.snapshot !== "function") throw fail("C1A_V2_BATCH_BUDGET_REQUIRED");
  if (typeof pacer !== "function" || typeof onEvidence !== "function") {
    throw fail("C1A_V2_BATCH_CALLBACK_INVALID");
  }
  ...
  for (const snapshot of mainSnapshots) {
    if (selectedIds.has(snapshot.case_id)) throw fail("C1A_V2_MANIFEST_CASE_OVERLAP", { caseId: snapshot.case_id });
    selectedIds.add(snapshot.case_id);
    const material = byId.get(snapshot.case_id);
    if (!material) throw fail("C1A_V2_SELECTED_CASE_MISSING", { caseId: snapshot.case_id });
  ...
  if (!Number.isFinite(minimumEligibility) || primaryEligibilityRatio < minimumEligibility) {
    throw fail("C1A_V2_PRIMARY_PROVIDER_ELIGIBILITY_BELOW_FROZEN_THRESHOLD", {
      primaryEligibleCount,
      primaryEligibilityRatio,
    });
  }

  if (!Array.isArray(manifest.sentinels)) throw fail("C1A_V2_MANIFEST_SENTINELS_MISSING");
  for (const sentinel of manifest.sentinels) {
    if (!selectedIds.has(sentinel.case_id)) throw fail("C1A_V2_SENTINEL_NOT_IN_MAIN_POPULATION", { caseId: sentinel.case_id });
    compareSnapshot(sentinel, byId.get(sentinel.case_id));
    if (preflightById.get(sentinel.case_id)?.eligible !== true) {
      throw fail("C1A_V2_SENTINEL_NOT_PROVIDER_ELIGIBLE", { caseId: sentinel.case_id });
    }
  }
  ...
  return {
    schema: "memory_engine_r3_c1a_qualification_batch_v2",

...
export function createQ4RecallHintC1BSiliconFlowSemanticProvidersV1({
  contract,
  env = process.env,
  requestImpl = httpsRequest,
  rerankTransport = undefined,
} = {}) {
  if (!contract || typeof contract !== "object") throw fail("Q4_C1B_RETRIEVAL_SF_CONTRACT_REQUIRED");
  if (contract.embedding?.endpoint !== Q4_C1B_EMBEDDING_ENDPOINT
      || contract.embedding?.dimension !== 2560
      || contract.rerank?.endpoint !== "https://api.siliconflow.cn/v1/rerank") {
    throw fail("Q4_C1B_RETRIEVAL_SF_CONTRACT_BINDING_MISMATCH");
  }
  ...
  const embeddingProvider = async input => {
    ...
    const parsed = await postJson({
      requestImpl,
      endpoint: contract.embedding.endpoint,
      apiKey,
      timeoutMs: contract.embedding.deadline_ms,
      maxResponseBytes: contract.embedding.max_response_bytes,
      body: {
...
export function createQ4RecallHintC1BSiliconFlowRetrievalProvidersV1({
  ...
  return createQ4RecallHintC1BSiliconFlowSemanticProvidersV1({
    contract,
    env,
    requestImpl,
    rerankTransport,

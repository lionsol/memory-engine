...
export function ensureDisclosureAttestationsTable(db) {
  db.exec(`
    CREATE TABLE IF NOT EXISTS disclosure_attestations (
      attestation_id TEXT PRIMARY KEY,
      memory_id TEXT NOT NULL,
      canonical_id TEXT NOT NULL,
      source_content_hash TEXT NOT NULL,
      surface TEXT NOT NULL CHECK (surface = 'DISCLOSURE_CARD'),
      projection_schema_version INTEGER NOT NULL,
      projection_kind TEXT NOT NULL,
      projection_hash TEXT NOT NULL,
      projection_adapter_version TEXT NOT NULL,
      authority_kind TEXT NOT NULL CHECK (authority_kind = 'OWNER_EXPLICIT_ATTESTATION'),
      audience_scope TEXT NOT NULL CHECK (audience_scope = 'OWNER_SELF'),
  ...
  `);
  db.exec("CREATE INDEX IF NOT EXISTS idx_disclosure_attestations_memory ON disclosure_attestations(memory_id)");
  db.exec("CREATE INDEX IF NOT EXISTS idx_disclosure_attestations_state ON disclosure_attestations(state)");
}
...
export function migrateLegacyMemoryEventsFromCore(engineDb, coreDb) {
  ensureMemoryEventsTable(engineDb);
  let coreHasEvents = false;
  try {
    const row = coreDb.prepare("SELECT name FROM main.sqlite_master WHERE type = 'table' AND name = 'memory_events'").get();
  ...
  const localCount = Number(engineDb.prepare("SELECT COUNT(*) AS c FROM memory_events").get()?.c || 0);
  ...
  const rows = coreDb.prepare(`SELECT ${colSql} FROM memory_events ORDER BY id ASC`).all();
  if (rows.length === 0) return { migrated: 0, reason: "ok" };

  const placeholders = columns.map(() => "?").join(", ");
  const insert = engineDb.prepare(`INSERT INTO memory_events (${colSql}) VALUES (${placeholders})`);

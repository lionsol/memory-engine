...
export const CORE_CHUNK_TIME_MIGRATION_PROVENANCE_GATE = "denied_by_provenance_audit";
export const EVENT_AT_MANUAL_RECOVERY_CANDIDATE_PREVIEW_CHARS = 240;
...
function eventAtManualRecoveryTagHints(candidate) {
  const tags = [];
  if (candidate?.has_preference_tag) tags.push("preference");
  if (candidate?.has_decision_tag) tags.push("decision");
  if (candidate?.has_todo_tag) tags.push("todo");
  if (tags.length === 0) tags.push("no_tag");
  return tags;
}

function normalizeEventAtSourceForTemplate(value) {
  return String(value || "").trim() === "null" ? null : value;
}
...
function normalizeReviewPacketFormat(value, outPath) {
  const explicit = String(value || "").trim().toLowerCase();
  if (explicit) {
    if (explicit !== "md" && explicit !== "jsonl") {
      throw new Error("format must be md or jsonl");
    }
    return explicit;
  }
  const normalizedOut = String(outPath || "").trim().toLowerCase();
  if (normalizedOut.endsWith(".jsonl")) return "jsonl";
  return "md";
}

function defaultEventAtPilotReviewPacketOutPath(date, count, format) {
  return resolve(tmpdir(), "memory-engine-reports", `event-at-manual-recovery-pilot${count}-review.${format}`);
}

function renderEventAtPilotReviewPacketMarkdown(report) {
  const lines = [
    `# Event Time Recovery Pilot Review - ${report.date}`,
    "",
    `- labels: \`${report.labels_path}\``,
    `- candidates: \`${report.candidates_path}\``,
    ...
    "- null",
    "",
    "updated_at is forbidden as event_at_source.",
    "",
  ];

  if (report.missing_candidate_count > 0) {
    lines.push("## Missing Candidates", "");
    for (const id of report.missing_candidate_ids) {
      lines.push(`- ${id}`);
    }
    lines.push("");
  }
  ...
  report.rows.forEach((row, index) => {
    lines.push(`### ${index + 1}. ${row.id.slice(0, 12)}`, "");
    lines.push(`- id: \`${row.id}\``);
    lines.push(`- date: \`${row.date}\``);
    lines.push(`- sha: \`${row.text_sha256_16}\``);
    lines.push(`- role: ${row.role_hint}`);
    lines.push(`- tags: ${row.tag_hints.join(", ")}`);
    lines.push(`- length: ${row.text_length}`);
    lines.push(`- pilot_reason: ${row.pilot_reason}`);
    lines.push(`- current_review_action: ${row.review_action}`);
    lines.push(`- current_manual_review_status: ${row.manual_review_status}`);
    lines.push(`- current_event_at: ${row.event_at === null ? "null" : row.event_at}`);
    lines.push(`- current_event_at_source: ${row.event_at_source}`);
    lines.push(`- current_confidence: ${row.confidence === null ? "null" : row.confidence}`);
    lines.push(`- preview: ${row.preview || "(empty)"}`);
    lines.push("");
    ...
    lines.push(JSON.stringify({
      review_action: row.review_action,
      event_at: row.event_at,
      event_at_source: normalizeEventAtSourceForTemplate(row.event_at_source),
      confidence: row.confidence,
      reviewer_note: row.reviewer_note,
    }, null, 2));
    lines.push("```", "");
...
export function buildEventAtPilotReviewPacket(options = {}) {
  const labelsPath = String(options.labelsPath || options.labels || "").trim();
  const candidatesPath = String(options.candidatesPath || options.candidates || "").trim();
  if (!labelsPath) throw new Error("labels path is required");
  if (!candidatesPath) throw new Error("candidates path is required");
  ...
  for (const label of labels) {
    const candidate = candidateMap.get(label.id);
    if (!candidate) {
      missingCandidateIds.push(label.id);
      continue;
    }
    if (String(candidate.text_sha256_16).toLowerCase() !== label.text_sha256_16) {
      throw new Error(`candidate text_sha256_16 mismatch for label id: ${label.id}`);
    ...
    rows.push({
      id: label.id,
      date: label.date,
      text_sha256_16: label.text_sha256_16,
      role_hint: String(candidate.role_hint || "unknown"),
      tag_hints: eventAtManualRecoveryTagHints(candidate),
      text_length: Number(candidate.text_length || 0),
      pilot_reason: String(label.pilot_reason || ""),
      preview: buildPreview(candidate.preview || "", EVENT_AT_MANUAL_RECOVERY_CANDIDATE_PREVIEW_CHARS),
      manual_review_status: label.manual_review_status,
      review_action: label.review_action,
      event_at: label.event_at,
      event_at_source: label.event_at_source,
      confidence: label.confidence,
      reviewer_note: label.reviewer_note,
      pilot_sample: Boolean(label.pilot_sample),
    });
  }

  const format = normalizeReviewPacketFormat(options.format, options.outPath || options.out);
  const outPath = String(options.outPath || options.out || defaultEventAtPilotReviewPacketOutPath(labels[0]?.date || "unknown-date", rows.length, format)).trim();
  assertPilotOutPathAllowed(outPath);
  ...
  const report = {
    ...
    missing_candidate_count: missingCandidateIds.length,
    missing_candidate_ids: missingCandidateIds.slice().sort((a, b) => String(a).localeCompare(String(b))),
    raw_text_exported: false,
    preview_chars: EVENT_AT_MANUAL_RECOVERY_CANDIDATE_PREVIEW_CHARS,
    rows,
  };

  mkdirSync(dirname(outPath), { recursive: true });
  if (format === "jsonl") {
    writeJsonlRows(outPath, rows);
  } else {
    writeFileSync(outPath, renderEventAtPilotReviewPacketMarkdown(report), "utf8");
  }
...
export function applyCoreChunkTimeMigration(options = {}) {
  throw new Error(
    `${CORE_CHUNK_TIME_MIGRATION_PROVENANCE_GATE}: Core event_at migration is suspended and must not be applied`,
  );

  // Kept below the permanent gate so the historical confirmation protocol remains
  // visible for a future ADR-approved reactivation without providing a bypass.

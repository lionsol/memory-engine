...
export function buildRuntimeBuildIdentity({ rootDir = DEFAULT_RUNTIME_ROOT, fileEntries } = {}) {
  ...
  for (const entry of entries) {
    if (seenPaths.has(entry.path)) errors.push(`duplicate_runtime_path:${entry.path}`);
  ...
  for (const required of ROOT_RUNTIME_FILES) {
    if (!requiredPaths.has(required)) errors.push(`missing_runtime_file:${required}`);
  }
  ...
  return {
    ...
    identity: uniqueErrors.length === 0 ? hash.digest("hex") : null,

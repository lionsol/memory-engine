...
import { projectCandidateToMemoryCard } from "./auto-recall-memory-card.js";

export const TURN_GOLD_SET_SCHEMA_VERSION = 1;

export const TURN_TASK_INTENTS = AUTO_RECALL_TASK_INTENTS;

export const TURN_RECALL_INTENTS = AUTO_RECALL_RECALL_INTENTS;
...
function expectedDisclosureLevel(row = {}) {
  const explicit = String(row.disclosure_level || "").trim();
  if (DISCLOSURE_LEVEL_SET.has(explicit)) return explicit;
  return row.expected_should_recall === true ? "memory_card" : "none";
}

function expectedCardKind(recallIntents = []) {
  const intents = Array.isArray(recallIntents) ? recallIntents : [];
  if (intents.includes("prior_decision")) return "decision";
  if (intents.includes("task_state")) return "task_state";
  if (intents.includes("workflow_rule")) return "workflow_rule";
  if (intents.includes("user_preference")) return "preference";
  if (intents.includes("project_state")) return "project_state";
  if (intents.includes("historical_context")) return "episode";
  return "fact";
}

function expectedCardCategory(recallIntents = []) {
  const intents = Array.isArray(recallIntents) ? recallIntents : [];
  if (intents.includes("project_state") || intents.includes("prior_decision") || intents.includes("task_state")) return "project";
  if (intents.includes("user_preference")) return "user_identity";
  if (intents.includes("workflow_rule")) return "workflow";
  if (intents.includes("historical_context")) return "episodic";
  return "unknown";
}

function expectedCardScope(recallIntents = []) {
  const intents = Array.isArray(recallIntents) ? recallIntents : [];
  if (intents.includes("project_state")) return "project_state";
  if (intents.includes("prior_decision")) return "prior_decision";
  if (intents.includes("task_state")) return "task_state";
  if (intents.includes("workflow_rule")) return "workflow_rule";
  if (intents.includes("user_preference")) return "user_preference";
  if (intents.includes("historical_context")) return "historical_context";
  return "entity_background";
}

function buildExpectedProjectionCandidate(row = {}, { lineNumber = null } = {}) {
  const turnId = safeTurnId(row.turn_id);
  const recallIntents = normalizeStringArray(row.recall_intent);
  const taskIntent = String(row.task_intent || "unknown");
  const disclosureLevel = expectedDisclosureLevel(row);
  const category = expectedCardCategory(recallIntents);
  const kind = expectedCardKind(recallIntents);
  const scope = expectedCardScope(recallIntents);
  const title = `Expected memory card for ${turnId}`;
  const summary = [
    `Expected ${disclosureLevel} disclosure for turn ${turnId}.`,
    `Task intent: ${taskIntent}.`,
    `Recall intent: ${recallIntents.length ? recallIntents.join(", ") : "none"}.`,
  ].join(" ");

  return {
    id: `turn_gold_${turnId}`,
    title,
    summary,
    salience_reason: `Gold-set expected disclosure for ${taskIntent}.`,
    path: "test/fixtures/auto-recall-turn-gold-set.seed.jsonl",
    ...
    scope,
    agent_scope: "edi",
    confidence: row.label_confidence === "low" ? 0.4 : row.label_confidence === "medium" ? 0.7 : 1,
    sources: ["turn_gold_set"],
    disclosure_level: disclosureLevel,
    risk_flags: Array.isArray(row.expected_card_risk_flags) ? row.expected_card_risk_flags : [],
  };
}

export function buildTurnGoldSetCardProjection(row = {}, actual = {}, { lineNumber = null } = {}) {
  const disclosureLevel = expectedDisclosureLevel(row);
  const cardExpected = row.expected_should_recall === true && disclosureLevel !== "none";
  const fullContentOnGetExpected = cardExpected && disclosureLevel === "full_content_on_get";

  const projection = {
    mode: "read_only_turn_gold_set_card_projection",
    expected: {
      disclosure_level: disclosureLevel,
      card_expected: cardExpected,
      full_content_on_get_expected: fullContentOnGetExpected,
    },
    actual: {
      runtime_should_recall: actual?.should_recall === true,
      focused_query: actual?.focused_query || "",
      projection_status: cardExpected ? "projected_expected_card" : "not_expected",
    },
    memory_object: null,
    memory_card: null,
    side_effects: {
  ...
  if (!cardExpected) return projection;

  const projected = projectCandidateToMemoryCard(buildExpectedProjectionCandidate(row, { lineNumber }), {
    agentScope: "edi",
    traceId: row.turn_id ? `turn_gold_set:${row.turn_id}` : null,
    retrievalRank: null,
  });

  return {
    ...projection,
    memory_object: projected.memory_object,
    memory_card: projected.memory_card,
  };
}
...
export function validateTurnGoldSetRow(row, { lineNumber = null } = {}) {
  ...
  if (row.task_intent != null && !isAutoRecallTaskIntent(String(row.task_intent))) {
    errors.push("task_intent");
  }

  const recallIntents = normalizeStringArray(row.recall_intent);
  if (row.recall_intent != null && !isAutoRecallRecallIntentArray(recallIntents)) {
    errors.push("recall_intent");
  }
  for (const intent of recallIntents) {
    if (!isAutoRecallRecallIntent(intent)) errors.push("recall_intent");
  }
...
function replayOne(row, { lineNumber = null } = {}) {
  ...
  if (actual.task_intent !== row.task_intent) {
    mismatches.push("task_intent");
  }

  const expectedRecallIntent = normalizeStringArray(row.recall_intent);
  if (JSON.stringify(actual.recall_intent) !== JSON.stringify(expectedRecallIntent)) {
    mismatches.push("recall_intent");
  }
  ...
  return {
    ...
    card_projection: buildTurnGoldSetCardProjection(row, actual, { lineNumber }),
  };
}
...
export function replayTurnGoldSet(rows) {
  ...
  const cardExpected = results.filter(result => result.card_projection?.expected?.card_expected === true);
  const cardProjected = results.filter(result => result.card_projection?.memory_card);
  const fullContentOnGetExpected = results.filter(result => result.card_projection?.expected?.full_content_on_get_expected === true);
  ...
  return {
    summary: {
      ...
      card_expected_count: cardExpected.length,
      card_projection_count: cardProjected.length,
      full_content_on_get_expected_count: fullContentOnGetExpected.length,
...
function classifyMismatchToken(result, mismatch) {
  ...
  if (mismatch === "task_intent") return "task_intent_mismatch";
  if (mismatch === "recall_intent") return "recall_intent_mismatch";
...
function suggestionForCluster(category) {
  switch (category) {
    ...
    case "task_intent_mismatch":
      return {
        target: "task_intent_classifier_or_label",
        confidence: "high",
        suggested_action: "Inspect deterministic task-intent precedence or the gold-set task_intent label; do not change recall policy to resolve this mismatch.",
      };
    case "recall_intent_mismatch":
      return {
        target: "recall_intent_classifier_or_label",
        confidence: "high",
        suggested_action: "Inspect conservative memory-specific recall signals or the gold-set recall_intent label; do not change should_recall policy to resolve this mismatch.",
      };
...
function expansionActionForCategory(category) {
  switch (category) {
    ...
    case "task_intent_mismatch":
      return "review_task_intent_classifier_or_label";
    case "recall_intent_mismatch":
      return "review_recall_intent_classifier_or_label";
    case "invalid_label_row":
    case "json_parse_error":

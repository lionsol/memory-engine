...
const B5_RUNTIME_MIN_PRECISION = 0.9;
const B5_RUNTIME_MIN_RECALL = 0.9;
const B5_RUNTIME_MAX_FALSE_POSITIVES = 2;
const B5_RUNTIME_MAX_FALSE_NEGATIVES = 2;
const B5_MAX_FAMILY_DECISION_ERRORS = 1;
const B5_EVIDENCE_ROLE = "fresh_independent_holdout_v2b5";
const B5_CANDIDATE_POLICY_STATUS = "NOT RUNTIME AUTHORIZED";
...
function familyMetrics(results, families) {
  return families.map(family => {
    ...
    return {
      ...
      task_mismatch: familyResults.filter(result => (
        result?.classification?.task_intent_match === false
      )).length,
      recall_mismatch: familyResults.filter(result => (
        result?.classification?.recall_intent_match === false
      )).length,
    };
  });
...
export function buildAutoRecallPolicyHoldoutV2B5Diagnostics(report) {
  const results = Array.isArray(report?.results) ? report.results : [];
  const taskMismatches = results.filter(result => (
    result?.classification?.task_intent_match === false
  ));
  const recallMismatches = results.filter(result => (
    result?.classification?.recall_intent_match === false
  ));
  ...
  return {
    ...
    task_intent_mismatch_case_ids: taskMismatches.map(result => result.turn_id),
    recall_intent_mismatch_count: recallMismatches.length,
    recall_intent_mismatch_case_ids: recallMismatches.map(result => result.turn_id),
    ...
    semantic_only_mismatch_cases: semanticOnlyMismatches.map(result => (
      descriptor(result, "semantic_intent_mismatch")
    )),
    ...
    task_classifier_mismatch_cases: taskMismatches.map(result => (
      descriptor(result, "task_intent_mismatch")
    )),
    recall_classifier_mismatch_cases: recallMismatches.map(result => (
      descriptor(result, "recall_intent_mismatch")
    )),
  };
}

export function assessAutoRecallPolicyHoldoutV2B5(
  report,
  ...
  const diagnostics = buildAutoRecallPolicyHoldoutV2B5Diagnostics(report);
  ...
  const quantitative_gate_pass = dataset_contract_valid
    ...
    && runtime.precision >= B5_RUNTIME_MIN_PRECISION
    && runtime.recall >= B5_RUNTIME_MIN_RECALL
    && runtime.false_positive <= B5_RUNTIME_MAX_FALSE_POSITIVES
    && runtime.false_negative <= B5_RUNTIME_MAX_FALSE_NEGATIVES;
  const family_gate_failures = family_metrics.filter(metric => (
    metric.decision_error_count > B5_MAX_FAMILY_DECISION_ERRORS
  ));
  ...
  return {
    ...
    readiness: {
      ...
      thresholds: {
        minimum_precision: B5_RUNTIME_MIN_PRECISION,
        minimum_recall: B5_RUNTIME_MIN_RECALL,
        maximum_false_positives: B5_RUNTIME_MAX_FALSE_POSITIVES,
        maximum_false_negatives: B5_RUNTIME_MAX_FALSE_NEGATIVES,
        maximum_family_decision_errors: B5_MAX_FAMILY_DECISION_ERRORS,
      },
      overall_status,
      independent_readiness_evidence: true,
      candidate_policy_status: B5_CANDIDATE_POLICY_STATUS,
    },
  };
}

function finalizeReport(report, contract) {
  const assessment = assessAutoRecallPolicyHoldoutV2B5(report, contract);
  const { family_metrics, readiness, ...b5Diagnostics } = assessment;
  return {
    ...report,
    mode: "offline_auto_recall_policy_holdout_v2b5",
    dataset: {
    ...
    family_metrics,
    readiness,
    evidence_role: B5_EVIDENCE_ROLE,
    current_evaluation_status: readiness.overall_status,
    candidate_policy_status: B5_CANDIDATE_POLICY_STATUS,
    b5_evaluator_status: "PASS",
  };
}

export function evaluateAutoRecallPolicyHoldoutV2B5Rows(rows, options = {}) {
  const contract = options.contract || AUTO_RECALL_POLICY_HOLDOUT_V2B5_CONTRACT;
...
export function evaluateAutoRecallPolicyHoldoutV2B5Jsonl(content, options = {}) {
  const contract = options.contract || AUTO_RECALL_POLICY_HOLDOUT_V2B5_CONTRACT;

...
const RECALL_WORKFLOW_RE = /工作流|工作规则|流程规则|workflow|process rule|runbook|procedure/i;
...
export function classifyTaskIntent(text, _features = {}) {
  ...
  const rules = [
    ["continue_prior_work", TASK_CONTINUATION_RE],
...
export function classifyRecallIntent(text, features = {}) {
  ...
  const hasProjectEntity = (Array.isArray(features.project_entities) && features.project_entities.length > 0)
    || RECALL_PROJECT_CUE_RE.test(input);

  if (hasHistory && RECALL_PREFERENCE_RE.test(input)) {
    return ["user_preference", "historical_context"];
  }
  if (hasHistory && RECALL_WORKFLOW_RE.test(input)) {
    return ["workflow_rule", "historical_context"];
  }
  if (hasHistory && RECALL_ENTITY_BACKGROUND_RE.test(input)) {
    return ["entity_background", "historical_context"];
  }
  if (taskIntent === "continue_prior_work" && hasHistory) {
    return ["task_state", "historical_context"];
  }
  if (taskIntent === "debug_error" && hasHistory) {
    return hasProjectEntity ? ["project_state", "historical_context"] : ["historical_context"];
  }
  if (taskIntent === "review_plan" && hasHistory && hasProjectEntity) {
    return ["project_state", "prior_decision"];
  }
  if (taskIntent === "rewrite_current_text" && hasHistory && hasProjectEntity) {
    return ["prior_decision", "project_state"];
  }
  if (taskIntent === "answer_question" && hasHistory && hasProjectEntity) {

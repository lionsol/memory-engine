...
export const AUTO_RECALL_POLICY_HOLDOUT_V2B3_FAMILIES = Object.freeze([
  "implicit_project_state_holdout",
  "implicit_continuation_holdout",
  "prior_decision_holdout",
  "historical_entity_holdout",
  "preference_workflow_lookup_holdout",
  "mixed_history_holdout",
  "fresh_project_analysis_holdout",
  "new_preference_workflow_instruction_holdout",
  "fresh_entity_factual_holdout",
  "history_suppression_holdout",
  "quoted_history_language_holdout",
  "fresh_state_vocabulary_holdout",
]);

export const AUTO_RECALL_POLICY_HOLDOUT_V2B3_CONTRACT = Object.freeze({
  expected_count: 48,
  expected_yes: 24,
  expected_no: 24,
  required_families: AUTO_RECALL_POLICY_HOLDOUT_V2B3_FAMILIES,
  allowed_families: AUTO_RECALL_POLICY_HOLDOUT_V2B3_FAMILIES,
});

export const AUTO_RECALL_POLICY_HOLDOUT_V2B5_FAMILIES = Object.freeze([
  "contextual_state_lookup_holdout",
  "continuation_temporal_holdout",
  "decision_rationale_holdout",
  "historical_entity_unicode_holdout",
  "preference_workflow_indirect_holdout",
  "mixed_scope_history_required_holdout",
  "suppression_scope_no_recall_holdout",
  "nested_supplied_history_holdout",
  "metalinguistic_history_vocab_holdout",
  "fresh_project_design_holdout",
  "new_instruction_memory_vocab_holdout",
  "counterfactual_meta_history_holdout",
]);

export const AUTO_RECALL_POLICY_HOLDOUT_V2B5_CONTRACT = Object.freeze({
  expected_count: 48,
  expected_yes: 24,
  expected_no: 24,
  required_families: AUTO_RECALL_POLICY_HOLDOUT_V2B5_FAMILIES,
  allowed_families: AUTO_RECALL_POLICY_HOLDOUT_V2B5_FAMILIES,
});
...
function validatePolicyEvaluationRow(row, { lineNumber = null, allowed_families = null } = {}) {
  const errors = [];
  ...
  const familyContract = Array.isArray(allowed_families)
    ? allowed_families
    : AUTO_RECALL_POLICY_EVALUATION_FAMILIES;
  if (typeof row.family !== "string" || row.family.trim().length === 0) {
    errors.push("family");
  } else if (!familyContract.includes(row.family)) {
...
export function validateAutoRecallPolicyEvaluationDataset(rows, options = {}) {
  ...
  items.forEach((row, index) => {
    ...
    const validation = validatePolicyEvaluationRow(row, {
      lineNumber,
      allowed_families: options.allowed_families,
    });
    if (!validation.valid) {
      rowErrors.push({
...
function evaluateCase(row, index, options = {}) {
  const validation = validatePolicyEvaluationRow(row, {
    lineNumber: index + 1,
    allowed_families: options.allowed_families,
  });
...
export function evaluateAutoRecallPolicyDataset(rows, options = {}) {
  ...
  const results = items.map((row, index) => evaluateCase(row, index, options));

...
function looksLikeRawLog(category = "", text = "") {
  const normalizedCategory = String(category || "").toLowerCase();
...
function collectRiskFlags(candidate = {}, { agentScope = "unknown" } = {}) {
  ...
  if (looksLikeRawLog(category, text)) flags.add("raw_log_like");
...
function canonicalProjectionError(message) {
  return new TypeError(`canonical memory projection requires ${message}`);
}

function canonicalProjectionInput(canonicalMemory = {}) {
  ...
  const memoryIdValue = canonicalMemory.memory_id;
  if (typeof memoryIdValue !== "string" || memoryIdValue.trim().length === 0) {
  ...
  if (!contentRef || typeof contentRef !== "object" || Array.isArray(contentRef)) {
    throw canonicalProjectionError("content_ref authority");
  }

  return {
    memoryIdValue,
    canonicalId: canonicalMemory.canonical_id,
    source,
    classification,
    lifecycle,
    contentRef,
...
function runtimeSafetyLifecycleState(candidate = {}) {
  const explicit = String(candidate.lifecycle_state || candidate.lifecycle?.state || "").trim().toLowerCase();
  if (["quarantined", "stale_index_candidate", "deleted_shadow"].includes(explicit)) return explicit;
  if (candidate.is_quarantined === true || Number(candidate.is_quarantined || 0) === 1) return "quarantined";
  if (candidate.stale_index_candidate === true) return "stale_index_candidate";
  if (candidate.needs_review === true || explicit === "needs_review") return "needs_review";
  if (["candidate"].includes(explicit)) return explicit;
  return null;
}

function canonicalProjectionLifecycleState(lifecycle = {}, runtimeCandidate = {}) {
  const runtimeSafetyState = runtimeSafetyLifecycleState(runtimeCandidate);
  if (runtimeSafetyState) return runtimeSafetyState;
  if (lifecycle.archived === true || Number(lifecycle.archived || 0) === 1) return "archived";
  return "active";
}

function isManagedCanonicalLifecycle(lifecycle = {}) {
  return lifecycle.management === "managed";
}

function canonicalConfidenceBaseline(lifecycle = {}) {
  return typeof lifecycle.confidence === "number" && Number.isFinite(lifecycle.confidence)
    ? lifecycle.confidence
    : undefined;
}

function buildCanonicalProjectionCandidate(canonicalMemory, runtimeCandidate = {}) {
  const canonical = canonicalProjectionInput(canonicalMemory);
  const runtime = runtimeCandidate && typeof runtimeCandidate === "object" && !Array.isArray(runtimeCandidate)
    ? runtimeCandidate
    : {};
  const runtimeSource = runtime.source && typeof runtime.source === "object" && !Array.isArray(runtime.source)
    ? runtime.source
    : {};
  const runtimeClassification = runtime.classification && typeof runtime.classification === "object" && !Array.isArray(runtime.classification)
    ? runtime.classification
    : {};
  const managed = isManagedCanonicalLifecycle(canonical.lifecycle);
  const lifecycleState = canonicalProjectionLifecycleState(canonical.lifecycle, runtime);
  const confidenceBaseline = managed ? canonicalConfidenceBaseline(canonical.lifecycle) : undefined;
  const path = canonical.source.path ?? "";
  const lineStart = canonical.source.line_start ?? null;
  const lineEnd = canonical.source.line_end ?? null;
  const updatedAt = canonical.source.updated_at ?? null;
  const category = canonical.classification.category ?? "unknown";
  const kind = canonical.classification.kind ?? "fact";

  return {
    ...runtime,
    id: canonical.memoryIdValue,
    memory_id: canonical.memoryIdValue,
    chunk_id: canonical.memoryIdValue,
    ...
    is_archived: canonical.lifecycle.archived === true || Number(canonical.lifecycle.archived || 0) === 1 ? 1 : 0,
    conflict_flag: canonical.lifecycle.conflict === true || Number(canonical.lifecycle.conflict || 0) === 1 ? 1 : 0,
    confidence: confidenceBaseline,
    confidence_score: confidenceBaseline,
  };
}
...
export function projectCanonicalMemoryToMemoryObject(canonicalMemory, runtimeCandidate = {}, options = {}) {
  const canonical = canonicalProjectionInput(canonicalMemory);
  const memoryObject = normalizeCandidateToMemoryObject(
    buildCanonicalProjectionCandidate(canonicalMemory, runtimeCandidate),
    options,
  );
  const managed = isManagedCanonicalLifecycle(canonical.lifecycle);
  const confidenceBaseline = managed ? canonicalConfidenceBaseline(canonical.lifecycle) : null;
  ...
  return {
    ...
    source: {
      ...
      line_start: canonical.source.line_start ?? null,
      line_end: canonical.source.line_end ?? null,
      updated_at: canonical.source.updated_at ?? null,
    },
    classification: {
      ...memoryObject.classification,
      category: canonical.classification.category ?? "unknown",
      kind: canonical.classification.kind ?? "fact",
    },
    content_ref: {
      ...memoryObject.content_ref,
      content_hash: canonical.contentRef.content_hash ?? null,
    },
    confidence: {
      ...memoryObject.confidence,
      score: confidenceBaseline,
    },
  };
}
...
export function projectCanonicalMemoryToMemoryCard(canonicalMemory, runtimeCandidate = {}, options = {}) {
  const memoryObject = projectCanonicalMemoryToMemoryObject(canonicalMemory, runtimeCandidate, options);
  const cardProjectionObject = memoryObject.confidence?.score === null
    ? {
      ...memoryObject,
      confidence: {
        ...memoryObject.confidence,
  ...
  return {
    memory_object: memoryObject,
    memory_card: projectMemoryObjectToCard(cardProjectionObject),
    side_effects: {
      db_writes: false,
      memory_file_mutation: false,
      dataset_file_mutation: false,

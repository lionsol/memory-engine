...
export function evaluateRecallHintRuntimeCanaryV1({
  ...
  const executionProbe = normalizeString(source.executionProbe);
  ...
  const probeAllowed = executionProbe === "rh_l3_canonical_v1"
    && requestedMode === "parallel";
  return baseDecision({
    enabled: true,
    in_scope: true,
    provider_allowed: !probeAllowed,
    probe_allowed: probeAllowed,
    execution_probe: probeAllowed ? executionProbe : null,
    vector_execution_mode: requestedMode,

...
function resolveToolOrigin(context) {
  ...
  if (source === "scheduled_healthcheck_wrapper") {
    const healthcheckRunId = readString(context, ["healthcheckRunId", "healthcheck_run_id"]);
    return result(
      "scheduled_healthcheck",
      context,
...
export function createHybridTrafficOriginRegistry({
  ...
  return {
    ...
    recordScheduledHealthcheck({
      ...
      const normalizedRunId = nonEmptyString(healthcheckRunId);
      if (!normalizedId || !normalizedRunId) return false;
      recordContext(normalizedId, {
        ...
        healthcheckRunId: normalizedRunId,
      });
      return true;
    },

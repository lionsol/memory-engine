...
function resolveToolOrigin(context) {
  ...
  if (context.registry_reason) {
    return result("unknown", context, [context.registry_reason], "before_tool_call", common);
  }
...
export function createHybridTrafficOriginRegistry({
  ...
  function store(toolCallId, context) {
    cleanup();
    if (entries.size >= maxEntries && !entries.has(toolCallId)) {
      entries.delete(entries.keys().next().value);
    }
    entries.set(toolCallId, { context, expiresAt: now() + ttlMs });
  }
  ...
  return {
    recordBeforeToolCall({ event = {}, ctx = {}, surface = null } = {}) {
      ...
      store(toolCallId, context);
      return true;
    },

    recordScheduledHealthcheck({ toolCallId, agentId = null, sessionId = null } = {}) {

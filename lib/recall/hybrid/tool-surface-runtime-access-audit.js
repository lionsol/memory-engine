...
function countObservedToolSurfaces(observations) {
  ...
  for (const row of Array.isArray(observations) ? observations : []) {
    ...
    if (!provenance.valid) {
      ...
      if (provenance.reasons.includes("unsupported_schema_version")
        || provenance.reasons.includes("missing_schema_version")) {
        unsupportedSchemaVersionCount += 1;
      } else if (provenance.reasons.includes("search_not_executed")) {
  ...
  return {
    ...
    invalidProvenanceObservationIds,
    invalidProvenanceReasonDistribution: Object.fromEntries(
      Object.entries(invalidProvenanceReasonCounts)
...
export function buildToolSurfaceRuntimeAccessAudit({
  ...
  if (surfaceEvidence.invalidProvenanceObservationCount > 0) {
    addIssue(violations, "invalid_observation_provenance", {
      count: surfaceEvidence.invalidProvenanceObservationCount,
    });
  }
  ...
  return {
    ...
    invalid_provenance_observation_count: surfaceEvidence.invalidProvenanceObservationCount,
    invalid_provenance_observation_ids: surfaceEvidence.invalidProvenanceObservationIds,
    invalid_provenance_reason_distribution: surfaceEvidence.invalidProvenanceReasonDistribution,

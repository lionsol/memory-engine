...
function addOriginIssues(blockers, metadata, surface) {
  ...
  if (metadata.traffic_origin_schema_version !== TRAFFIC_ORIGIN_SCHEMA_VERSION) {
    addIssue(blockers, issue("invalid_traffic_origin_schema", metadata.traffic_origin_schema_version));
    valid = false;
  }
  if (!isObject(metadata.traffic_origin_evidence)) {
    addIssue(blockers, issue("invalid_traffic_origin", "missing_evidence"));
    valid = false;
  }
...
export function evaluateProductionEvidenceContinuity({ observations = [], thresholds: thresholdInput } = {}) {
  ...
  const maxGapBySurface = emptySurfaceCounts();
  for (const surface of PRODUCTION_HYBRID_OBSERVATION_SURFACES) {
    ...
    maxGapBySurface[surface] = maxGapHours(rows);
  ...
  for (const surface of PRODUCTION_HYBRID_OBSERVATION_SURFACES) {
    ...
    if (maxGapBySurface[surface] > thresholds.maximum_observation_gap_hours) {
      addIssue(evidenceGaps, issue(`surface_gap_above_threshold:${surface}`, maxGapBySurface[surface], thresholds.maximum_observation_gap_hours));
    }
  }
  ...
  return {
    ...
    maximum_gap_hours_by_surface: maxGapBySurface,

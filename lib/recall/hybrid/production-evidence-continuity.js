...
import { isTrafficOrigin, TRAFFIC_ORIGIN_SCHEMA_VERSION } from "./traffic-origin.js";
...
function normalizeThresholds(input, blockers) {
  ...
  if (!isObject(input)) {
    if (input !== undefined && input !== null) addIssue(blockers, issue("invalid_thresholds"));
    return thresholds;
  }
  for (const key of Object.keys(input)) {
    if (!Object.hasOwn(thresholds, key)) addIssue(blockers, issue("unknown_threshold", key));
  }
  for (const key of Object.keys(thresholds)) {
    ...
    if (typeof value !== "number" || !Number.isFinite(value) || value < 0 || invalidInteger || invalidRatio) {
      addIssue(blockers, issue("invalid_threshold", key));
      continue;
    }
...
function addOriginIssues(blockers, metadata, surface) {
  ...
  if (!evidenceShape) {
    addIssue(blockers, issue("origin_evidence_shape_invalid"));
    addIssue(blockers, issue("origin_evidence_mismatch", origin));
    valid = false;
  }
  if (metadata.traffic_origin_valid !== true) {
    addIssue(blockers, issue("origin_evidence_mismatch", "traffic_origin_valid"));
    valid = false;
  }
  if (!Array.isArray(metadata.traffic_origin_reasons) || metadata.traffic_origin_reasons.length > 0) {
    addIssue(blockers, issue("origin_evidence_mismatch", "traffic_origin_reasons"));
    valid = false;
  }
  ...
  if (evidenceShape && origin === "natural_user_turn") {
    if (evidence.source !== "before_prompt_build"
  ...
  if (evidenceShape && origin === "natural_agent_tool_call") {
    if (evidence.source !== "before_tool_call_agent"
      || evidence.agent_id_present !== true
      || evidence.run_id_present !== true
      || evidence.session_id_present !== true
  ...
  if (evidenceShape && origin === "operator_verification_probe") {
    if (evidence.source !== "gateway_tools_invoke"
  ...
  if (evidenceShape && origin === "scheduled_healthcheck"
    && evidence.source !== "scheduled_healthcheck_wrapper") {
    addIssue(blockers, issue("origin_evidence_mismatch", origin));
    valid = false;
  }
  if (origin === "unknown") {
    addIssue(blockers, issue("unknown_traffic_origin"));
    valid = false;
  }
  return valid;
}

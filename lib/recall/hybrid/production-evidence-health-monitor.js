...
function validateBaseline(value) {
  ...
  if (!validIso(value.authorized_at)) errors.push(issue("invalid_baseline_authorized_at", value.authorized_at));
...
function validateRuntimeParity(value) {
  ...
  if (!validIso(value.checked_at)) errors.push(issue("invalid_runtime_parity_checked_at", value.checked_at));
...
function validateProductHealth(value) {
  ...
  if (!validIso(value.checked_at)) errors.push(issue("invalid_product_health_checked_at", value.checked_at));
...
function readCheckedAt(report) {
  return validIso(report?.checked_at) ? Date.parse(report.checked_at) : null;
}

function collectCanonicalTimes(observations, baseline) {
  ...
  for (const row of Array.isArray(observations) ? observations : []) {
    ...
    if (baseline?.evidence_epoch_id && metadata.evidence_epoch_id !== baseline.evidence_epoch_id) continue;
    ...
    if (metadata.traffic_origin === "scheduled_healthcheck"
      && metadata.traffic_origin_valid === true
      && Array.isArray(metadata.traffic_origin_reasons)
      && metadata.traffic_origin_reasons.length === 0
...
export function evaluateProductionEvidenceHealth({
  ...
  const asOfTimestamp = asOf === undefined ? Date.now() : Date.parse(asOf);
  const asOfIso = asOf === undefined
    ? new Date(asOfTimestamp).toISOString()
    : canonicalIso(asOf);
  ...
  const identity = evaluateProductionEvidenceIdentity({ observations });
  ...
  if (baselineActive) {
    ...
    const times = collectCanonicalTimes(observations, baseline);

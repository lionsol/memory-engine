...
const HEALTHCHECK_SURFACES = Object.freeze([
...
function validateBaseline(value) {
  ...
  const authorizedAt = canonicalIso(value.authorized_at);
  if (!authorizedAt) errors.push(issue("invalid_baseline_authorized_at", value.authorized_at));
  if (value.active === true) {
    const activatedAt = canonicalIso(value.activated_at);
    if (!activatedAt) errors.push(issue("invalid_baseline_activated_at", value.activated_at));
    if (authorizedAt && activatedAt && Date.parse(activatedAt) < Date.parse(authorizedAt)) {
      errors.push(issue("baseline_activation_before_authorization"));
    }
    if (value.activation_source !== "sustained_runtime_activation_finalizer") {
      errors.push(issue("invalid_baseline_activation_source", value.activation_source));
    }
    const planGeneratedAt = canonicalIso(value.authorization_plan_generated_at);
    if (!planGeneratedAt) errors.push(issue("invalid_baseline_authorization_plan_generated_at", value.authorization_plan_generated_at));
    if (planGeneratedAt && activatedAt) {
      const ageHours = (Date.parse(activatedAt) - Date.parse(planGeneratedAt)) / 3_600_000;
      if (ageHours < 0 || ageHours > 1) errors.push(issue("invalid_baseline_authorization_plan_age_hours", ageHours));
    }
    if (typeof value.openclaw_runtime_version !== "string" || !value.openclaw_runtime_version.trim()) {
      errors.push(issue("invalid_baseline_openclaw_runtime_version"));
    }
    if (typeof value.openclaw_config_file_path !== "string" || !value.openclaw_config_file_path.trim()) {
      errors.push(issue("invalid_baseline_openclaw_config_file_path"));
    }
    if (!isSha256Identity(value.openclaw_config_file_sha256)) errors.push(issue("invalid_baseline_openclaw_config_file_sha256"));
    if (!Number.isInteger(value.openclaw_config_file_byte_count) || value.openclaw_config_file_byte_count <= 0) {
      errors.push(issue("invalid_baseline_openclaw_config_file_byte_count"));
    }
    if (!isSha256Identity(value.openclaw_config_fingerprint)) errors.push(issue("invalid_baseline_openclaw_config_fingerprint"));
  }
  return { valid: errors.length === 0, errors };
}

function baselineEvidenceStart(value) {
  if (!isPlainObject(value)) return null;
  if (value.active === true) return canonicalIso(value.activated_at);
  return canonicalIso(value.authorized_at);
}
...
function runtimeParityStatus(validation, report, baseline) {
  if (!validation.valid) return "invalid";
  if (report.runtime_build_identity !== baseline.runtime_build_identity) return "identity_mismatch";
  if (report.source_runtime_equal !== true || report.difference_count !== 0) return "drift";
  return "clean";
}
...
export function partitionAuthorizedEvidenceObservations({ observations = [], baseline, asOf } = {}) {
  ...
  const observationsBeforeEvidenceStart = [];
  ...
  for (const row of Array.isArray(observations) ? observations : []) {
    ...
    if (production && sameEpoch && evidenceStartedAt && timestamp < evidenceStartedAt) {
      observationsBeforeEvidenceStart.push(row);
  ...
  return {
    ...
    observationsBeforeEvidenceStart,
    observationsBeforeAuthorization: observationsBeforeEvidenceStart,
...
function completeHealthcheckTimestamp(observations) {
  const runs = new Map();
  for (const row of Array.isArray(observations) ? observations : []) {
    ...
    const metadata = parseHybridObservationMetadata(row);
    if (metadata?.production_evidence_enabled !== true || metadata.traffic_origin !== "scheduled_healthcheck") continue;
    const evidence = validateHybridTrafficOriginEvidence({
      surface: metadata.surface,
      origin: metadata.traffic_origin,
      evidence: metadata.traffic_origin_evidence,
    ...
    const timestamp = canonicalIso(metadata.completed_at);
    const runId = typeof metadata?.traffic_origin_evidence?.healthcheck_run_id === "string"
      ? metadata.traffic_origin_evidence.healthcheck_run_id.trim()
      : "";
    if (!evidence.valid || !timestamp || !runId || !HEALTHCHECK_SURFACES.includes(metadata.surface)) continue;
    const run = runs.get(runId) || { surfaces: new Set(), latest: null };
    run.surfaces.add(metadata.surface);
    const value = Date.parse(timestamp);
    if (value > (run.latest ?? -Infinity)) run.latest = value;
    runs.set(runId, run);
  }
  let latest = null;
  for (const run of runs.values()) {
    if (!HEALTHCHECK_SURFACES.every(surface => run.surfaces.has(surface))) continue;
    if (run.latest > (latest ?? -Infinity)) latest = run.latest;
  }
  return latest;
}
...
export function collectCanonicalTimes(observations) {
  ...
  return {
    latest,
    latestBySurface,
    latestHealthcheck: completeHealthcheckTimestamp(observations),
  };
}
...
export function evaluateProductionEvidenceHealth({
  ...
  if (baselineActive) {
    ...
    const evidenceStartedAt = baselineEvidenceStart(baseline);
    const evidenceStartedAtTimestamp = Date.parse(evidenceStartedAt);
    if (evidenceStartedAtTimestamp > asOfTimestamp) addUnique(stopConditions, issue("baseline_evidence_start_after_as_of"));
    if (partition.observationsBeforeEvidenceStart.length > 0) {
      addUnique(stopConditions, issue("observation_before_evidence_start", partition.observationsBeforeEvidenceStart.length));
    }
    ...
    const outOfWindowHealthcheckTimestamp = healthcheckTimestamp([
      ...partition.observationsBeforeEvidenceStart,
      ...partition.observationsAfterAsOf,
    ]);
    if (parityTimestamp !== null && parityTimestamp < evidenceStartedAtTimestamp) {
      addUnique(stopConditions, issue("runtime_parity_before_evidence_start"));
    }
    if (productTimestamp !== null && productTimestamp < evidenceStartedAtTimestamp) {
      addUnique(stopConditions, issue("product_health_before_evidence_start"));
    }
    if (outOfWindowHealthcheckTimestamp !== null) {
      if (outOfWindowHealthcheckTimestamp < evidenceStartedAtTimestamp) {
        addUnique(stopConditions, issue("healthcheck_before_evidence_start"));
    ...
    const observationFreshnessBySurface = Object.fromEntries(PRODUCTION_HYBRID_OBSERVATION_SURFACES.map(surface => [
      surface,
      freshnessStatus(times.latestBySurface[surface], asOfTimestamp, monitorValidation.thresholds.maximum_latest_observation_age_hours),
    ]));
    const parityHealthStatus = runtimeParityStatus(parityValidation, runtimeParity, baseline);
    const productHealthStatus = productValidation.valid ? productHealth.status : "invalid";
    ...
    for (const surface of PRODUCTION_HYBRID_OBSERVATION_SURFACES) {
      const status = observationFreshnessBySurface[surface];
      if (status !== "fresh") addUnique(stopConditions, issue(`surface_observation_stale:${surface}`, status));
    }
    if (parityHealthStatus === "drift") addUnique(stopConditions, issue("runtime_source_parity_drift"));
    if (parityHealthStatus === "identity_mismatch") addUnique(stopConditions, issue("runtime_parity_identity_mismatch"));
    if (parityStatus !== "fresh") addUnique(stopConditions, issue(`runtime_parity_${parityStatus}`));
    if (productHealthStatus === "rollback_required") addUnique(stopConditions, issue("product_health_rollback_required"));
    if (productHealthStatus === "not_evaluated") addUnique(stopConditions, issue("product_health_not_evaluated"));
    if (productHealthStatus === "healthy" && productHealth.blockers.length > 0) addUnique(stopConditions, issue("product_health_blockers_present"));
    ...
    const freshnessStatuses = [latestStatus, ...Object.values(observationFreshnessBySurface), healthcheckStatus, parityStatus, productStatus];
    const monitorFreshnessStatus = freshnessStatuses.every(status => status === "fresh")
      ? "fresh"
      : freshnessStatuses.find(status => status !== "fresh") || "stale";
    ...
    return {
      ...
      observation_before_evidence_start_count: partition.observationsBeforeEvidenceStart.length,
      ...
      runtime_parity_status: parityHealthStatus,
      runtime_parity_freshness_status: parityStatus,
      product_health_status: productHealthStatus,
      product_health_freshness_status: productStatus,
      ...
      observation_freshness_status_by_surface: observationFreshnessBySurface,
      ...
      evidence: {
        baseline: summary(baseline, ["schema_version", "active", "activation_source", "authorization_plan_generated_at", "evidence_epoch_id", "runtime_build_identity", "rollout_config_fingerprint", "expected_kg_mode", "expected_recent_mode", "authorized_at", "activated_at"]),
  ...
  return {
    ...
    activated_at: baselineValidation.valid ? baseline.activated_at ?? null : null,
    evidence_started_at: baselineValidation.valid ? baselineEvidenceStart(baseline) : null,
    ...
    runtime_parity_freshness_status: "not_evaluated",
    product_health_status: "not_evaluated",
    product_health_freshness_status: "not_evaluated",
    ...
    observation_freshness_status_by_surface: Object.fromEntries(PRODUCTION_HYBRID_OBSERVATION_SURFACES.map(surface => [surface, "not_evaluated"])),

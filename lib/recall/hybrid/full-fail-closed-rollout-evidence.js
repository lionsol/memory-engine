...
function readExplicitValue(row, metadata, key) {
  if (row && Object.hasOwn(row, key)) return { present: true, value: row[key] };
  if (metadata && Object.hasOwn(metadata, key)) return { present: true, value: metadata[key] };
...
function channelFacts(row, metadata, channel) {
  ...
  const fallback = isKg ? fallbackFacts.kgFallback : fallbackFacts.recentFallback;
  const scopeMatchKey = isKg ? "kg_fail_closed_scope_match" : "recent_fail_closed_scope_match";
  const scopeMatchMarker = readExplicitValue(row, metadata, scopeMatchKey);
  const explicitFull = runtimeMode === FULL_MODE
    && rolloutScope === "full"
    && scopeRequired === false
    && scopeMatchMarker.present
    && scopeMatchMarker.value === null;
...
function controlledRunCoverage({
  ...
  if (windowFallbackEvents > 0 || classified.kgFallbackEvents > 0 || classified.recentFallbackEvents > 0) {
    addIssue(
      blockers,
      "fallback_events_present",
      Math.max(windowFallbackEvents, classified.kgFallbackEvents, classified.recentFallbackEvents),
      0,
    );
  }
  if (scopeMismatchEvents > 0) addIssue(blockers, "scope_mismatch_events_present", scopeMismatchEvents, 0);
...
export function buildFullFailClosedRolloutEvidence({
  ...
  const controlledCoverage = controlledRunCoverage({
    ...
    unsupportedSchemaVersionEvents,
    invalidProvenanceObservationCount,
    windowFallbackEvents: windowReport.counts?.fallback_events || 0,
    scopeMismatchEvents: classified.scopeMismatchEvents,
  ...
  if (windowReport.counts?.fallback_channel_attribution_missing_events > 0) {
    warnings.push({
      code: "fallback_channel_attribution_missing",
      actual: windowReport.counts.fallback_channel_attribution_missing_events,
    });
  }
  ...
  if (blockers.length > 0 && controlledCoverage.eligible) {
    controlledCoverage.status = "blocked";
    controlledCoverage.eligible = false;
    controlledCoverage.blockers.push({ code: "safety_blocker_present" });
  }
  ...
  return {
    ...
    fallback_channel_attribution_missing_events:
      windowReport.counts?.fallback_channel_attribution_missing_events || 0,

...
function channelFacts(row, metadata, channel) {
  ...
  const fallback = isKg
    ...
    : accessMode === "guarded_fallback"
      || runtimeMode === "legacy_fallback"
      || numericPositive(fallbackCount);
  const explicitFull = (runtimeMode === FULL_MODE || accessMode === FULL_MODE)
    && rolloutScope === "full"
    && scopeRequired !== true;
...
function classifyRows(observations, windowReport) {
  const kgModeCounts = {};
...
export function buildFullFailClosedRolloutEvidence({
  ...
  const classified = classifyRows(rows, windowReport);
  ...
  if (classified.invalidObservationInput > 0) addIssue(blockers, "invalid_observation_input", classified.invalidObservationInput);

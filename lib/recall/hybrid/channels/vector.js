...
function initializeMultiQueryDebug(debug, mode, queries = []) {
...
export async function collectVectorCandidates(ctx) {
  ...
  if (multiQueryEnabled) {
    ...
    initializeMultiQueryDebug(debug, resolved?.mode || "bounded_multi_query", multiQueryInputs || []);
    if (!multiQueryInputs) {
  ...
  if (!lancedbTable) {
    ...
    if (multiQueryEnabled) {
      vectorHandled = await collectBoundedMultiQueryCandidates(ctx, multiQueryInputs, lancedbTable);

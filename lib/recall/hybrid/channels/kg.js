...
function selectLegacyKgRows(withDb, likePatterns) {
  return withDb(db => {
...
function finiteRelevance(value) {
  const number = Number(value);
  return Number.isFinite(number) ? number : 0;
}

export function compareKgCandidateRelevance(left = {}, right = {}) {
  for (const field of KG_RELEVANCE_FIELDS) {
    const delta = finiteRelevance(right[field]) - finiteRelevance(left[field]);
...
export async function collectKgCandidates(ctx) {
  ...
  try {
    const fullFailClosedActive = kgFailClosedDecision?.mode === "full_fail_closed"
      && kgFailClosedDecision?.enabled === true
      && kgFailClosedDecision?.eligible === true;
    const canaryActive = kgFailClosedDecision?.mode === "fail_closed_canary"
      && kgFailClosedDecision?.enabled === true
      && kgFailClosedDecision?.in_scope === true;
    const failClosedActive = fullFailClosedActive || canaryActive;
    if (failClosedActive) {
      debug.kg_runtime_mode = fullFailClosedActive ? "full_fail_closed" : "fail_closed_canary";
      debug.kg_rollout_scope = fullFailClosedActive ? "full" : "scoped_canary";
      debug.kg_scope_required = fullFailClosedActive ? false : true;
      debug.kg_fail_closed_scope_match = fullFailClosedActive ? null : true;
    ...
    if (queryTerms.length > 0 || exactFragments.length > 0) {
      ...
      if (likePatterns.length > 0) {
        ...
        if (kgAccessMode === "isolated") {
          const isolatedRows = selectIsolatedKgRows({ withEngineDb, withCoreDb }, likePatterns);
        ...
        if (fallbackRequired) {
          ...
          if (!legacyFallbackAllowed || fullFailClosedActive || (canaryActive && kgIsolationRequested)) {
            kgRows = [];
            ...
            if (!legacyFallbackAllowed) debug.kg_legacy_fallback_disabled = true;
          } else {
            kgRows = selectLegacyKgRows(withDb, likePatterns);
          }
        }
        ...
        const scoredKg = uniqueById(
          ...
          .sort(compareKgCandidateRelevance)
          .slice(0, ftsTopK);

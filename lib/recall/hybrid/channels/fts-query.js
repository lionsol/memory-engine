...
export function buildIsolatedFtsSql({
  ...
  const samplePredicate = sampleScoped ? "AND c.sample_id = ?" : "";
  return `
        SELECT
          c.id,
          c.text,
...
export const ISOLATED_FTS_SAMPLE_SQL = buildIsolatedFtsSql({ sampleScoped: true });
export const ISOLATED_FTS_SAMPLE_FALLBACK_SQL = buildIsolatedFtsSql({
  sampleScoped: true,
  stableFallbackOrder: true,
});
...
export function selectIsolatedFtsRows(
  ...
  return withCoreDb(db => {
    ...
    const sampleScoped = sampleId !== undefined;
    const params = sampleScoped
      ? [query, sampleId, archivedJson, ftsTopK]
      : [query, archivedJson, ftsTopK];
    return db.prepare(buildIsolatedFtsSql({ ...fts, stableFallbackOrder, sampleScoped }))
      .all(...params);

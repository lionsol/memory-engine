...
export function selectCoreKgRows({ withCoreDb }, candidateIds) {
  return withCoreDb(db => db.prepare(CORE_KG_JSON_JOIN_SQL).all(JSON.stringify(candidateIds)));
}
...
export function selectIsolatedKgRows(access, likePatterns) {
  ...
  const coreRows = selectCoreKgRows(access, guarded.candidate_ids);
  return {
    ...guarded,

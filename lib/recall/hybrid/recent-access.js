...
export function inspectRecentIsolationTopology({ withCoreDb, withEngineDb } = {}) {
  ...
  const read = (accessor) => {
    try {
      return accessor(db => {
        const databaseNames = db.prepare("PRAGMA database_list").all().map(row => String(row.name));
        return {
          readonly: db.readonly === true,
          database_names: databaseNames,
          persistent_database_names: databaseNames.filter(name => name !== "temp"),
        };
      });
    } catch {
      return null;
  ...
  const valid =
    ...
    && core.persistent_database_names.length === 1
    && core.persistent_database_names[0] === "main"
    && engine.persistent_database_names.length === 1
    && engine.persistent_database_names[0] === "main";
...
function recentArchivedExclusionSql() {
  return `
    c.id NOT IN (
...
export function buildIsolatedRecentLikeSql(patternCount) {
  ...
  return recentCoreSql(`
      ...
      AND ${recentArchivedExclusionSql()}
      AND (${likeWhere})
  `);
}

export const ISOLATED_RECENT_SCORED_SQL = recentCoreSql(`
    c.path NOT LIKE 'memory/generated-smart-add/%'
    AND ${recentArchivedExclusionSql()}
    AND (c.path LIKE 'memory/smart-add/%' OR c.path LIKE 'memory/episodes/%')
`);

export const ISOLATED_RECENT_FALLBACK_SQL = recentCoreSql(`
    c.path NOT LIKE 'memory/generated-smart-add/%'
    AND ${recentArchivedExclusionSql()}
    AND (c.path LIKE 'memory/smart-add/%' OR c.path LIKE 'memory/episodes/%')
`);
...
export function selectIsolatedRecentScoredRows({ withCoreDb }, archivedJson, topK) {
  const rows = withCoreDb(db => db.prepare(ISOLATED_RECENT_SCORED_SQL).all(archivedJson, topK));
  return { sql: ISOLATED_RECENT_SCORED_SQL, rows };
}

export function selectIsolatedRecentFallbackRows({ withCoreDb }, archivedJson, topK) {
  const rows = withCoreDb(db => db.prepare(ISOLATED_RECENT_FALLBACK_SQL).all(archivedJson, topK));
  return { sql: ISOLATED_RECENT_FALLBACK_SQL, rows };
}

...
import { createLegacyAttachedCoreRetiredError } from "../../db/legacy-attached-core.js";
...
async function executeMeasuredRun({
  ...
  const ctx = buildContext({
    ...
    recentIsolationRequested,
    recentIsolationFallbackReason,
    // This flag is scoped to the audit's direct collector comparison. It is
    // never passed through hybridSearch or a production channel runtime.
    legacyFallbackAllowed: mode === "legacy" || recentAccessMode === "guarded_fallback",
    timingState,
  });
...
function openReadonlyHandlesForPaths({ coreDbPath, engineDbPath }) {
  // The worker only executes the isolated branch. Keep the historical
  // `legacyDb` field for the measured-run shape, but make it a standalone
  // readonly Core handle rather than an attached Engine/Core connection.
  let legacyDb;
  let isolatedCoreDb;
  let isolatedEngineDb;
  try {
    legacyDb = openCoreDbReadonly({ coreDbPath, engineDbPath });
    ...
    return { legacyDb, isolatedCoreDb, isolatedEngineDb };
  } catch (error) {

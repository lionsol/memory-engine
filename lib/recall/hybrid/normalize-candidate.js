...
export function normalizeExternalMemory(row = {}, {
  ...
  const rawConfidence = row.confidence === null || row.confidence === undefined
    ? null
...
export function isCandidateAllowedForRerank(item, minConfidence) {
  ...
  const archived = toFiniteNumber(item.is_archived);
  if (archived === null || archived !== 0) return false;

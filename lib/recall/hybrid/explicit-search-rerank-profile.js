...
function observedAdapterIdentity(identity) {
  return isUnknownAdapterIdentity(identity) ? null : identity;
}
...
function adapterIdentityMatches(configured, observed) {
  ...
  return isRecord(observed)
    && observed.provider === configured.provider
    && observed.model === configured.model
    && observed.revision === configured.revision;
}
...
export async function executeExplicitSearchRerankProfile({
  ...
  try {
    ...
    const debug = errorDebug(profile, {
      ...
      projectionError: String(error?.message || "projection_rejected").slice(0, 120),
    }, {
      canonicalReadElapsedMs,
  ...
  if (profile.mode === "control") {
    ...
    const observed = observedAdapterIdentity(reranked.adapterIdentity);

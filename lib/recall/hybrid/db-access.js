...
function legacyAccess(withDb) {
  return {
    ...
    withEngineDb: withDb,
    withLegacyDb: withDb,
    capabilities: { isolatedFts: false },
  };
}
 ...
 * defaults isolatedFts to false; only an explicit scope provider may opt in.
 * Capabilities describe reader topology, not a security sandbox. Other
 * channels still use withLegacyDb, and Phase 1B1/1B2 do not remove runtime
 * Core write risk.
 */

...
function normalizedFallbackChannels(value) {
  return Array.isArray(value)
    ? value.filter(channel => typeof channel === "string").map(channel => channel.trim().toLowerCase())
    : [];
}

export function getHybridFallbackFacts(row, metadata) {
  const kgAccessMode = readValue(row, metadata, "kg_access_mode");
  const recentAccessMode = readValue(row, metadata, "recent_access_mode");
  const kgRuntimeMode = readValue(row, metadata, "kg_runtime_mode");
  const recentRuntimeMode = readValue(row, metadata, "recent_runtime_mode");
  const kgFallbackCount = readValue(row, metadata, "kg_fallback_events");
  const recentFallbackCount = readValue(row, metadata, "recent_fallback_events");
  const fallbackUsed = readValue(row, metadata, "legacy_db_fallback_used") === true;
  const fallbackChannels = normalizedFallbackChannels(
    readValue(row, metadata, "legacy_db_fallback_channels"),
  );
  const hasKgChannel = fallbackChannels.includes("kg");
  const hasRecentChannel = fallbackChannels.includes("recent");
  const kgFallback = hasKgChannel
    || kgAccessMode === "legacy_fallback"
    || kgRuntimeMode === "legacy_fallback"
    || numericPositive(kgFallbackCount);
  const recentFallback = hasRecentChannel
    || recentAccessMode === "guarded_fallback"
    || recentRuntimeMode === "legacy_fallback"
    || numericPositive(recentFallbackCount);
  const hasFallbackMarker = fallbackUsed || fallbackChannels.length > 0;
  const fallback = hasFallbackMarker || kgFallback || recentFallback;
  const channelAttributionMissing = fallback && (
    (fallbackUsed && fallbackChannels.length === 0)
    || (fallbackChannels.length > 0 && !hasKgChannel && !hasRecentChannel)
  );
...
export function evaluateHybridFallbackEvidenceWindow(
  ...
  let fallbackChannelAttributionMissingEvents = 0;
  ...
  for (const row of rows) {
    ...
    const fallbackFacts = getHybridFallbackFacts(row, metadata);
    if (fallbackFacts.kgFallback) kgFallbackEvents += 1;
    if (fallbackFacts.recentFallback) recentFallbackEvents += 1;
    if (fallbackFacts.fallback) fallbackEvents += 1;
    if (fallbackFacts.kgFallback && fallbackFacts.recentFallback) bothFallbackEvents += 1;
    if (fallbackFacts.channelAttributionMissing) fallbackChannelAttributionMissingEvents += 1;
  }
  ...
  return {
    ...
    counts: {
      ...
      fallback_channel_attribution_missing_events: fallbackChannelAttributionMissingEvents,
    },
    coverage: {
    ...
    both_fallback_events: bothFallbackEvents,
    fallback_channel_attribution_missing_events: fallbackChannelAttributionMissingEvents,
    warnings: fallbackChannelAttributionMissingEvents > 0
      ? [{
        code: "fallback_channel_attribution_missing",
        actual: fallbackChannelAttributionMissingEvents,
      }]
      : [],

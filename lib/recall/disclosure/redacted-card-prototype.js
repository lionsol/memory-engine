...
const REDACTION_PLAN_FIELDS = new Set(["schema_version", "directives"]);
...
function validateRedactionPlan(redactionPlan) {
  if (!isRecord(redactionPlan) ||
      Object.keys(redactionPlan).length !== REDACTION_PLAN_FIELDS.size ||
      !Object.keys(redactionPlan).every(key => REDACTION_PLAN_FIELDS.has(key)) ||

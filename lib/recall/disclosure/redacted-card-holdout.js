...
function validateLabel(label, answerBearing, planLiterals, diagnostics) {
  ...
  const requiredLiterals = label.semantic_preservation.required_literals;
  validateLiteralList(
    requiredLiterals,
  ...
  if (!answerBearing && Array.isArray(requiredLiterals) && requiredLiterals.length !== 0) {
    addDiagnostic(
      diagnostics,
      "non_answer_semantic_literals_not_empty",
      "label.semantic_preservation.required_literals",
    );
  }
}

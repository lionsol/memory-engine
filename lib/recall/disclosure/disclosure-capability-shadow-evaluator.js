...
/**
 * Calculates the future DisclosureCapability v1 state from bounded canonical
...
export function calculateShadowCapability(canonicalContext = {}) {
  ...
  if (!Array.isArray(context.risk_flags)) return "RETRIEVAL_ONLY";
  if (context.projection_valid !== true) return "RETRIEVAL_ONLY";
  if (hasBlockingLifecycle(context)) return "RETRIEVAL_ONLY";
  if (BLOCKING_ARTIFACT_STATES.has(String(context.artifact_state || "").trim().toLowerCase())) {
    return "RETRIEVAL_ONLY";
  }
  if (hasBlockingRisk(context) || hasBlockingScope(context)) return "RETRIEVAL_ONLY";
  if (hasSensitiveRisk(context) || hasSensitiveScope(context)) return "INTERNAL_CONTEXT";
  if (String(context.artifact_state || "").trim().toLowerCase() !== "safe") return "RETRIEVAL_ONLY";
  return "CARD_DISCLOSABLE";
}
...
export function evaluateShadowDisclosure(candidate) {
  ...
  const predictedCapability = calculateShadowCapability(candidate.canonical_context);
...
export function evaluateShadowDisclosureCandidates(candidates = []) {
  ...
  return {
    mode: "offline_disclosure_capability_shadow_evaluation_v1",

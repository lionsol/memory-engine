...
} from "./cite-authority.js";
import { formatDirectCardContext } from "./disclosure/direct-card-formatter.js";
import { evaluateDirectCardProductionBoundary } from "./disclosure/direct-card-production-boundary.js";
import { DISCLOSURE_DECISIONS } from "./disclosure/disclosure-types.js";
import {
...
function boundedDirectCardDiagnosticMemoryId(value) {
  if (typeof value !== "string" && typeof value !== "number") return null;
  const id = String(value).slice(0, 16);
  return id || null;
}

function boundedDirectCardDiagnosticString(value) {
...
function directCardDiagnosticLimit(topK) {
  const numericTopK = Number(topK);
  if (!Number.isFinite(numericTopK) || numericTopK <= 0) return 0;
  return Math.min(Math.floor(numericTopK), MAX_DIRECT_CARD_DIAGNOSTIC_ITEMS);
}

function projectDirectCardDiagnosticRecords(value, fields, limit) {
  ...
  return value
    ...
    .map(item => {
      ...
      const projected = {};
      const memoryId = boundedDirectCardDiagnosticMemoryId(item.memory_id);
      if (memoryId !== null) projected.memory_id = memoryId;
      for (const field of fields) {
        const bounded = boundedDirectCardDiagnosticString(item[field]);
...
function classifyDirectCardBoundaryError(error) {
  const reason = boundedDirectCardDiagnosticString(error?.reason);
  return reason || "direct_card_boundary_error";
}

function hasDirectCardBoundaryError(result) {
  return Array.isArray(result?.capabilityResults) && result.capabilityResults.some(item => (
    item?.reason === "direct_card_boundary_error"
  ));
}
...
function buildMemoryPromptSupplement(turnState, params) {
  ...
  return [
    ...
    "3. **引用强化** → 只可调 `memory_engine` action=`cite` 强化本轮 Search 返回的 id，chunk_ids=[结果中的id]",
...
export function createAutoRecallHookLifecycle({
  ...
  const resolveExplicitSearchRuntimeContext = toolCallId => {
    ...
    return Object.freeze({
      ...
      sessionIdentity: scope.sessionId,
      runIdentity: scope.runId || null,
      requestIdentity: toolCallId || null,
    });
  };
  ...
  const onMemoryEngineSearchSuccess = (result, invocation = {}) => {
    turnState.cleanupExpired();
    const toolCallId = invocation?.toolCallId || null;
    const scope = toolCallId ? turnState.getToolInvocationScope(toolCallId) : null;
    if (!scope?.runId || !turnState.getTurnState(scope.runId)) {
      if (toolCallId) turnState.deleteToolInvocationScope(toolCallId);
      return false;
    }
    const memoryIds = Array.isArray(result?.results)
      ? result.results.map(item => item?.memory_id || item?.chunk_id)
        .filter(id => typeof id === "string")
      : [];
    const recorded = turnState.recordMemoryEngineSearch({
      runId: scope.runId,
      memoryIds,
    });
    turnState.deleteToolInvocationScope(toolCallId);
  ...
  const authorizeMemoryEngineCite = (toolCallId, requestedIds) => {
    turnState.cleanupExpired();
    const scope = toolCallId ? turnState.getToolInvocationScope(toolCallId) : null;
    const state = scope?.runId ? turnState.getTurnState(scope.runId) : null;
    if (toolCallId) turnState.deleteToolInvocationScope(toolCallId);
    const servedIds = state?.memoryEngineSearchExactIds instanceof Set
      ? [...state.memoryEngineSearchExactIds]
      : Array.isArray(state?.memoryEngineSearchExactIds)
        ? state.memoryEngineSearchExactIds
        : [];
    const resolution = state
      ? resolveAuthorizedMemoryIds(requestedIds, servedIds)
      : null;
    if (!resolution?.authorized) {
      const code = resolution?.code || resolution?.error || MEMORY_CITE_NOT_AUTHORIZED;
      return { authorized: false, error: code, code };
    }
    return { authorized: true, resolved_ids: resolution.resolved_ids };
  };
  ...
  const register = (hybridRuntimeContext) => {
    ...
    if (typeof api?.on === "function") {
      api.on("before_tool_call", async (event, ctx) => {
        ...
        const sessionId = ctx?.sessionId || resolveAutoRecallHookSessionId(event, ctx);
        if (toolCallId && runId && !turnState.getTurnState(runId)) {
          turnState.createTurnState({ runId, sessionId });
        }
        if (toolCallId && runId) {
          turnState.recordToolInvocationScope({ toolCallId, runId, sessionId });
        }
    ...
    if (autoRecallConfig?.enabled && typeof api?.on === "function") {
      ...
      api.on("before_prompt_build", async (event, ctx) => {
        try {
          ...
          // Candidate-first AutoRecall: deterministic intent classification is
          // advisory only. It may focus a long query and remains observable in
          // debug metadata, but it no longer has authority to suppress
          // retrieval before candidate evidence exists.
          const focusedQuery = String(recallIntent.focused_query || "").trim();
          const searchPrompt = recallIntent.long_input_detected && focusedQuery
            ? focusedQuery
            : prompt;
          if (!String(searchPrompt || "").trim()) {
            const intentResult = {
              results: [],
              debug: {
                recall_intent_should_recall: recallIntent.should_recall,
                recall_intent_reason: "focused_query_empty",
            ...
            recordSkippedRecall({
              ...
              prompt,
              result: intentResult,
              skipReason: "focused_query_empty",
              sessionId,
              traceId,
              startedAt,
              now: currentTime,
          ...
          let selectedDirectCards = [];
          if (cardFirstRuntimeEnabled) {
            let directCardResult = null;
            let directCardBoundaryError = null;
            try {
              directCardResult = await evaluateDirectCardProductionBoundary({
                event,
                candidates: gatedResults.slice(0, autoRecallTopK),
                runtimeAgentScope: runtimeGate.agentId,
                withHybridDbAccessScope: hybridRuntimeContext.dataAccess?.withHybridDbAccessScope,
              });
              directCardBoundaryError = hasDirectCardBoundaryError(directCardResult)
                ? "direct_card_boundary_error"
                : null;
              selectedDirectCards = Array.isArray(directCardResult?.selected)
                ? directCardResult.selected.filter(item => item?.decision === DISCLOSURE_DECISIONS.DISCLOSE_CARD)
                : [];
            } catch (error) {
              directCardBoundaryError = classifyDirectCardBoundaryError(error);
              selectedDirectCards = [];
            }
            const diagnosticLimit = directCardDiagnosticLimit(autoRecallTopK);
            result.debug = {
              ...(result?.debug || {}),
              direct_card_event_sender_is_owner: event?.senderIsOwner === true,
              direct_card_owner_audience_authenticated:
                directCardResult?.ownerAudienceAuthenticated === true ||
                directCardResult?.ownerAudienceAuthenticated === false
                  ? directCardResult.ownerAudienceAuthenticated
                  : null,
              injected_count: selectedDirectCards.length,
              direct_card_selected_count: selectedDirectCards.length,
              direct_card_capability_results: projectDirectCardDiagnosticRecords(
                directCardResult?.capabilityResults,
                ["capability", "reason"],
                diagnosticLimit,
              ),
              direct_card_selections: projectDirectCardDiagnosticRecords(
                directCardResult?.selections,
                ["decision", "reason"],
                diagnosticLimit,
              ),
              direct_card_boundary_error: directCardBoundaryError,
            };
          }
          ...
          const prependContext = cardFirstRuntimeEnabled
            ? formatDirectCardContext(selectedDirectCards, { topK: autoRecallTopK })
          ...
          const selectedEntries = cardFirstRuntimeEnabled
            ? selectedDirectCards
              .map(selection => ({
                selection,
                memory: gatedResults[Number(selection.candidate_index)],
              }))
              .filter(entry => entry.memory)
            : gatedResults.slice(0, autoRecallTopK).map(memory => ({ memory, selection: null }));
          const exactMemoryIdForEntry = ({ memory, selection }) => {
            const candidates = cardFirstRuntimeEnabled
              ? [selection?.memory_id, selection?.chunk_id, memory?.memory_id, memory?.chunk_id]
              : [memory?.memory_id, memory?.chunk_id, selection?.memory_id, selection?.chunk_id];
            return candidates.find(value => typeof value === "string" && value.trim())?.trim() || null;
          };
          const injectedIds = cardFirstRuntimeEnabled
            ? selectedEntries.map(({ selection }) => String(selection.memory_id || "").slice(0, 16))
            : selectedEntries.map(({ memory }) => String(memory.id || "").slice(0, 16));
          const reinforcementAllowedEntries = selectedEntries
            .filter(({ memory, selection }) => (
              !cardFirstRuntimeEnabled ||
              gateDecisionById.get(String(selection.memory_id || "").slice(0, 16))?.reinforcement_allowed !== false
            ) && gateDecisionById.get(String(memory.id || "").slice(0, 16))?.reinforcement_allowed !== false);
          const reinforcementAllowedExactIds = reinforcementAllowedEntries
            .map(exactMemoryIdForEntry)
            .filter(Boolean);
          const reinforcementAllowedIds = reinforcementAllowedExactIds.map(id => id.slice(0, 16));
          selectedEntries.forEach(({ memory, selection }) => {
            const id = cardFirstRuntimeEnabled
              ? String(selection.memory_id || "").slice(0, 16)
              : String(memory.id || "").slice(0, 16);
          ...
          writeEvent({
            ...
            latency_ms: currentTime() - startedAt,
            candidate_count: hits,
            injected_count: selectedEntries.length,
            source: "autoRecall",
            metadata_json: buildRecallCompletedMetadata({
              skipped: false,
              skip_reason: null,
      ...
      api.on("before_agent_finalize", async (event, ctx) => {
        ...
        try {
          ...
          const allowlist = buildReinforcementAllowedIds({
            ...
            currentTurnMemoryEngineGetExactIds: [...(activeTurnState?.memoryEngineGetExactIds || [])],
          });
          const filtered = filterCitedIdsForReinforcement(
            citedIds,
            allowlist.reinforcement_allowed_ids,
            allowlist.reinforcement_allowed_exact_ids,
          );
          const authorized = resolveAuthorizedMemoryIds(
            filtered.reinforced_ids,
            allowlist.reinforcement_allowed_exact_ids,
          );
          const idsToReinforce = authorized.authorized ? authorized.resolved_ids : [];
          writeEvent({
            event_type: "auto_recall_debug",
            session_id: sessionId,
          ...
          const fullIds = runWithDb(db => {
            const reinforcedIds = [];
            const count = reinforceBatch(db, idsToReinforce, Math.floor(currentTime() / 1000), {
              onReinforced: id => reinforcedIds.push(id),
            });
            return reinforcedIds.length > 0
              ? [...new Set(reinforcedIds)]
              : count === idsToReinforce.length
                ? idsToReinforce
                : [];
          });

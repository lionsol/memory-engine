...
export function createAutoRecallHookLifecycle({
  ...
  const onMemoryEngineSearchSuccess = (result, invocation = {}) => {
    ...
    const memoryIds = Array.isArray(result?.results)
      ? result.results.map(item => item?.id).filter(id => typeof id === "string")
      : [];
  ...
  const authorizeMemoryEngineCite = (toolCallId, requestedIds) => {
    ...
    if (toolCallId) turnState.deleteToolInvocationScope(toolCallId);
    const servedIds = state?.memoryEngineSearchIds instanceof Set
      ? [...state.memoryEngineSearchIds]
      : Array.isArray(state?.memoryEngineSearchIds)
        ? state.memoryEngineSearchIds
        : [];
    if (!state || !citeIdsAreAuthorized(requestedIds, servedIds)) {
      return { authorized: false, error: MEMORY_CITE_NOT_AUTHORIZED, code: MEMORY_CITE_NOT_AUTHORIZED };
    }
    ...
    if (autoRecallConfig?.enabled && typeof api?.on === "function") {
      ...
      api.on("before_prompt_build", async (event, ctx) => {
        try {
          ...
          const reinforcementAllowedIds = selectedEntries
            ...
            .map(({ memory, selection }) => cardFirstRuntimeEnabled
      ...
      api.on("before_agent_finalize", async (event, ctx) => {
        ...
        try {
          ...
          const fullIds = runWithDb(db => {
            ...
            const reinforcedIds = [];
            const count = reinforceBatch(db, resolved, Math.floor(currentTime() / 1000), {

...
function normalizeExactAllowlist(value, fallback) {
  const values = Array.isArray(value) ? value : fallback;
...
export function evaluateAutoRecallRuntimeGate({ event, ctx, config = {} }) {
  ...
  const sessionId = normalizeExact(
    pick(event, ["session_id", "sessionId", "sessionKey"]) ||
    pick(ctx, ["session_id", "sessionId", "sessionKey"])
  );
  ...
  const allowSessions = normalizeExactAllowlist(
    config.sessionAllowlist ?? config.session_allowlist,
    [],
  );
  ...
  if (allowSessions.length > 0 && !sessionId) {
    return { allowed: false, reason: "denied_missing_session_id", agentId };
  }

  if (allowSessions.length > 0 && !allowSessions.includes(sessionId)) {
    return {
      allowed: false,
  ...
  return {
    allowed: true,
    agentId,
    ...(allowSessions.length > 0 ? { sessionId } : {}),
    trigger,
    chatType: contextChatType.value,
    messageRole: contextMessageRole.value,
  };

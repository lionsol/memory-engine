...
export function buildHybridSearchObservation({
  ...
  const metadata = {
    ...
    evidence_epoch_id: identityContext?.productionEvidenceEnabled === true
      ? identityContext.evidenceEpochId ?? null
      : null,
    runtime_build_identity: typeof identityContext?.runtimeBuildIdentity === "string"
      ? identityContext.runtimeBuildIdentity
      : null,
    rollout_config_fingerprint: typeof identityContext?.rolloutConfigFingerprint === "string"
      ? identityContext.rolloutConfigFingerprint
      : null,
    production_evidence_enabled: identityContext?.productionEvidenceEnabled === true,
...
export function recordHybridSearchObservation({
  ...
  const metadata = buildHybridSearchObservation({ surface, result, completedAtMs, identityContext });
  try {
    recordMemoryEvent({

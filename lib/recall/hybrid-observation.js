...
export function buildHybridSearchObservation({
  ...
  if (["sequential", "parallel"].includes(debug.vector_query_execution)) {
    metadata.vector_query_execution = debug.vector_query_execution;
  }
  const vectorQueryCount = boundedFiniteNumber(debug.vector_query_count, {
    integer: true,
    maximum: 3,
  });
  if (vectorQueryCount !== null) metadata.vector_query_count = vectorQueryCount;
  const vectorSearchCount = boundedFiniteNumber(debug.vector_search_count, {
    integer: true,
    maximum: 3,
  });
  if (vectorSearchCount !== null) metadata.vector_search_count = vectorSearchCount;
  if (Array.isArray(debug.vector_query_candidate_counts)) {
    metadata.vector_query_candidate_counts = debug.vector_query_candidate_counts
      .slice(0, 3)
      .map(value => boundedFiniteNumber(value, { integer: true, maximum: 50 }));
  }
  for (const key of [
    "kg_access_mode",
  ...
  if (debug.recall_hint && typeof debug.recall_hint === "object") {
    ...
    if (debug.hint_execution_probe) {
      recallHint.execution_probe = boundedToken(debug.hint_execution_probe);
    }

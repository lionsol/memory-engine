...
function normalizeExactMemoryId(value) {
...
function uniqueExactIds(values) {
  ...
  const result = [];
  for (const value of values || []) {
    const exactId = normalizeExactMemoryId(value);
...
export function createAutoRecallTurnStateManager(options = {}) {
  ...
  function createTurnState({ runId, sessionId = null, traceId = null } = {}) {
    ...
    const state = {
      ...
      memoryEngineGetExactIds: new Set(),
      memoryEngineSearchIds: new Set(),
      memoryEngineSearchExactIds: new Set(),
      injectedIds: [],
  ...
  function recordMemoryEngineGet({ runId, memoryId } = {}) {
    ...
    const exactId = normalizeExactMemoryId(memoryId);
    const shortId = normalizeShortMemoryId(exactId);
    if (!state || !exactId || !shortId) return false;
    state.memoryEngineGetExactIds.add(exactId);
    state.memoryEngineGetIds.add(shortId);
    touchRecord(state, now(), ttlMs);
    return true;
  }

  function recordMemoryEngineSearch({ runId, memoryIds = [] } = {}) {
    const state = getTurnState(runId);
    if (!state || !Array.isArray(memoryIds)) return false;
    for (const exactId of uniqueExactIds(memoryIds)) {
      state.memoryEngineSearchExactIds.add(exactId);
      state.memoryEngineSearchIds.add(normalizeShortMemoryId(exactId));
    }
    touchRecord(state, now(), ttlMs);
    return true;
  }
  ...
  function updateTurnRecallState({
    ...
    injectedIds = [],
    reinforcementAllowedIds = [],
    reinforcementAllowedExactIds = null,
    traceId = null,
    sessionId = null,
  } = {}) {
    ...
    const exactAllowedIds = reinforcementAllowedExactIds === null
      ? reinforcementAllowedIds
      : reinforcementAllowedExactIds;
    state.reinforcementAllowedExactIds = uniqueExactIds(exactAllowedIds);
    state.reinforcementAllowedIds = uniqueShortIds(
      reinforcementAllowedIds.length > 0 ? reinforcementAllowedIds : state.reinforcementAllowedExactIds,
    );

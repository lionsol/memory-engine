...
function resultFor({ candidates, status, reason, startedAt, adapterIdentity, usage = null, scoredByIndex }) {
  return {
...
function fallbackResult({ candidates, reason, startedAt, adapter, response = null }) {
  return resultFor({
    candidates,
 ...
 * @returns {Promise<{orderedIds: string[], status: string, reason: string, scores: Record<string, number|null>, elapsedMs: number, adapterIdentity: unknown, usage: unknown}>}
 */
export async function rerankCandidates({ query, candidates, deadlineMs, adapter } = {}) {
  ...
  const scoredByOriginalIndex = new Map(
    submitted.map(candidate => [candidate.index, scoredBySubmittedIndex.get(submitted.indexOf(candidate))]),
  );
  const orderedIds = [

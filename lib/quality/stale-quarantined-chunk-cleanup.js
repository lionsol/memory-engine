...
function readQuarantineLog(memoryDir) {
  ...
  for (const line of lines) {
    ...
    const movedAt = parsed?.moved_at || null;
    const entry = {
      schema_version: Number.isFinite(Number(parsed?.schema_version))
        ? Number(parsed.schema_version)
        : 1,
      moved_at: movedAt,
      timestamp: parsed?.timestamp || movedAt,
      moved_from: movedFrom,
      moved_to: movedTo,
      episode_path: parsed?.episode_path ? normalizePath(parsed.episode_path) : null,
      episode_format: parsed?.episode_format ? String(parsed.episode_format) : "unknown",
      reason,
      similarity: parsed?.similarity ?? null,
      daily_sha256: parsed?.daily_sha256 || null,
      episode_summary_sha256: parsed?.episode_summary_sha256 || null,
    };
    entries.push(entry);
...
function classifyPathGroup(group, { rootDir, memoryDir, quarantineByMovedFrom }) {
  ...
  const base = {
    ...
    quarantine_timestamp: quarantineEntry?.timestamp || quarantineEntry?.moved_at || null,
    moved_to: quarantineEntry?.moved_to || null,
    moved_to_exists: movedToExists,
    episode_path: quarantineEntry?.episode_path || null,
    episode_format: quarantineEntry?.episode_format || "unknown",
...
export function applyStaleQuarantinedChunkCleanup() {
  denyCoreWriteCapability("stale-quarantined-chunk-cleanup");
}

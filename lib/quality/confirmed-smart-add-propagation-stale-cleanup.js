...
function collectConfirmedStaleCandidates(options = {}) {
  ...
  try {
    const ftsColumns = tableColumns(coreDb, "main", "chunks_fts");
    ...
    const report = {
      ...
      memory_confidence_cleanup_strategy: engineDb && tableExists(engineDb, "main", "memory_confidence")
    ...
    for (const confirmedPath of confirmedPaths) {
      ...
      const chunkRows = collectPathChunkRows(coreDb, confirmedPath);
      const candidateRows = [];
      ...
      const ftsRows = collectFtsRowsByChunkId(coreDb, candidateRows.map(row => row.chunk_id), hasFtsIdColumn);
...
export function applyConfirmedSmartAddPropagationStaleChunkCleanup() {
  denyCoreWriteCapability("confirmed-smart-add-propagation-stale-cleanup");
}

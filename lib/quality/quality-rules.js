...
function resolveQualityScope(candidate) {
  const fallback = classifyQualityScope(candidate?.path);
  const hasExpectedConfidence = candidate && Object.hasOwn(candidate, "expected_confidence");
  return {
    ...fallback,
    owner: candidate?.quality_scope_owner ?? fallback.owner,
    family: candidate?.quality_scope_family ?? fallback.family,
    expected_confidence: hasExpectedConfidence
      ? Boolean(candidate.expected_confidence)
      : Boolean(fallback.expected_confidence),
    default_quality_score_scope: candidate?.default_quality_score_scope ?? fallback.default_quality_score_scope,
    diagnostic_scope: candidate?.diagnostic_scope ?? fallback.diagnostic_scope,
    retrieval_visible: candidate?.retrieval_visible ?? fallback.retrieval_visible,
    reason: candidate?.quality_scope_reason ?? fallback.reason,
  };
}
...
export function evaluateQualityFlags(candidate, context = {}) {
  ...
  const qualityScope = resolveQualityScope(candidate);
  const expectsQualityMetadata = qualityScope.expected_confidence === true;
  ...
  if (!category) {
    if (expectsQualityMetadata) addP0(p0PerMemoryFlags.missing_category);
  ...
  if (expectsQualityMetadata && !candidate?.has_confidence_record) {
    addP0(p0PerMemoryFlags.chunks_without_confidence);
  }

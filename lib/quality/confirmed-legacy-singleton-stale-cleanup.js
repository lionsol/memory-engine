...
import { denyCoreWriteCapability } from "../db/core-write-guard.js";
import {
  openCoreDbReadonly,
...
function evaluatePreflight(options = {}) {
  ...
  const coreDb = openCoreDbReadonly(resolved);
  const engineDb = existsSync(resolved.engineDbPath)
    ? openEngineDbIsolated({ ...resolved, readonly: true })
    : null;
  try {
    const exactChunkCount = chunkId
      ? toCount(querySingleValue(
        coreDb,
        "SELECT COUNT(*) AS c FROM chunks WHERE id = ? AND path = ?",
        chunkId,
        targetPath,
      ))
      : 0;
    ...
    const ftsExists = tableExists(coreDb, "main", "chunks_fts");
    const ftsColumns = ftsExists ? tableColumns(coreDb, "main", "chunks_fts") : [];
    ...
    if (ftsExists && ftsHasIdColumn && chunkId) {
      exactFtsCount = toCount(querySingleValue(
        coreDb,
        "SELECT COUNT(*) AS c FROM chunks_fts WHERE id = ?",
        chunkId,
      ));
      if (ftsHasPathColumn) {
        const mismatchedFtsCount = toCount(querySingleValue(
          coreDb,
          "SELECT COUNT(*) AS c FROM chunks_fts WHERE id = ? AND path <> ?",
          chunkId,
          targetPath,
        ));
    ...
    const hasConfidenceTable = Boolean(engineDb && tableExists(engineDb, "main", "memory_confidence"));
    const exactConfidenceCount = hasConfidenceTable && chunkId
      ? toCount(querySingleValue(
        engineDb,
        "SELECT COUNT(*) AS c FROM memory_confidence WHERE chunk_id = ?",
        chunkId,
      ))
      : 0;
...
- apply_prohibited: ${report.apply_prohibited}
- write_prohibition_code: ${report.write_prohibition_code}
...
export function applyConfirmedLegacySingletonStaleCleanup() {
  denyCoreWriteCapability("confirmed-legacy-singleton-stale-cleanup");
}

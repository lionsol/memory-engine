...
export function inferAuditPathPrefix(path) {
  ...
  if (
    normalized.startsWith("memory/generated-smart-add/")
...
function readCandidateRows(coreDb, engineDb) {
  const hasFiles = schemaHasTable(coreDb, "main", "files");
  ...
  const coreRows = selectRows(coreDb, `
    SELECT
      c.id AS id,
      c.path AS path,
      c.source AS source,
      ...
      ${fileSourceSelect}
    FROM chunks c
    ${hasFiles ? "LEFT JOIN files f ON f.path = c.path" : ""}
  ...
  `);
  const confidenceRows = schemaHasTable(engineDb, "main", "memory_confidence")
    ? selectRows(engineDb, "SELECT chunk_id, category FROM memory_confidence")
    : [];
  ...
  return coreRows.map(row => {
    ...
    return {
      ...row,
      confidence_chunk_id: confidence?.chunk_id ?? null,
      category: confidence?.category ?? null,
      has_confidence: Boolean(confidence),
    };
  });
}

function readEventAggRows(engineDb, coreDb) {
  const unifiedEvents = readUnifiedMemoryEvents(engineDb, { coreDb });
...
export function openAuditDb(dbPaths = resolveAuditDbPaths()) {
  ...
  let engineDb;
  try {
    coreDb = openCoreDbReadonly(dbPaths);
    engineDb = openEngineDbIsolated({ ...dbPaths, readonly: true });
    return {
      coreDb,
      engineDb,
      close() {
...
export function buildChunksWithoutConfidenceAudit({ db, coreDb = db?.coreDb, engineDb = db?.engineDb, dbPaths = resolveAuditDbPaths(), generatedAt = new Date().toISOString() } = {}) {
  if (!coreDb || !engineDb) {
    throw new Error("buildChunksWithoutConfidenceAudit requires isolated Core and Engine handles");
  }
  const candidateRows = readCandidateRows(coreDb, engineDb);
  const eventAggRows = readEventAggRows(engineDb, coreDb);

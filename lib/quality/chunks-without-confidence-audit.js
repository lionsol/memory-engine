...
export function inferAuditFamily(path) {
  ...
  if (normalized === "MEMORY.md") return "curated_memory";
  ...
  if (normalized.startsWith("memory/raw_log/")) return "raw_log";
  if (/^memory\/\d{4}-\d{2}-\d{2}(?:-[^/]+)?\.md$/i.test(normalized)) return "daily_memory";
  return "unknown";
}
...
export function inferManagedStatus(path) {
  ...
  if (family === "smart_add" || family === "episode") return "confidence_lifecycle_managed";
  if (family === "dreaming" || family === "raw_log") return "generated_artifact_outside_lifecycle";
  if (family === "project" || family === "daily_memory" || family === "curated_memory") {

...
function attachCore(db, coreDbPath) {
  db.exec(`ATTACH DATABASE '${String(coreDbPath).replace(/'/g, "''")}' AS core`);
}

function withAttachedDbs(options, fn, dbOptions = {}) {
...
function collectOrphanDiagnostics(db, sampleLimit) {
...
export function collectOrphanConfidenceDryRun(options = {}) {
  ...
  return withAttachedDbs(options, (db, paths) => {
    const diagnostics = collectOrphanDiagnostics(db, sampleLimit);
...
export function applyOrphanConfidenceCleanup(options = {}) {
  ...
  const orphanChunkIds = withAttachedDbs({
    ...options,
    engineDbPath: paths.engineDbPath,
    coreDbPath: paths.coreDbPath,
  }, (db) => collectOrphanDiagnostics(db, sampleLimit).orphanRows.map((row) => String(row.chunk_id || "")), {
    readonly: true,
  });

  const deletedCount = withAttachedDbs({
    ...options,
    engineDbPath: paths.engineDbPath,
    coreDbPath: paths.coreDbPath,
  }, (db) => {
    const tx = db.transaction(() => deleteOrphanConfidenceRows(db, orphanChunkIds));
    return tx();
  }, { readonly: false });

...
function readSingletonRows(coreDb, engineDb, targetPath) {
  const confidenceRows = engineDb.prepare(`
    SELECT chunk_id
    FROM memory_confidence
  `).all();
  const confidenceIds = new Set(confidenceRows.map(row => String(row.chunk_id ?? "")));
  ...
  `).all({ targetPath }).map(row => ({
    ...
    has_confidence_record: confidenceIds.has(String(row.id || "")),
  }));
}
...
function aggregateUsage(rows, targetPath, events = null) {
  ...
  const eventRows = Array.isArray(events)
    ? events
    : readUnifiedMemoryEvents(rows.engineDb, { coreDb: rows.coreDb });
...
export function buildLegacySingletonReview({
  ...
  if (!coreDb || !engineDb) {
    throw new Error("buildLegacySingletonReview requires isolated Core and Engine handles");
  }
  const normalizedTargetPath = normalizeReviewPath(targetPath);
  const scope = classifyQualityScope(normalizedTargetPath);
  const rows = readSingletonRows(coreDb, engineDb, normalizedTargetPath);
  ...
  const usage = aggregateUsage({ coreDb, engineDb, items: rows }, normalizedTargetPath);

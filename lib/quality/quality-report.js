...
function buildBreakdowns(items) {
  ...
  const by_quality_scope_family = {};
  const by_quality_scope_owner = {};
  ...
  for (const item of items) {
    ...
    incrementNestedCount(by_quality_scope_family, item.quality_scope_family || "unknown", item);
    incrementNestedCount(by_quality_scope_owner, item.quality_scope_owner || "unknown", item);
  ...
  return {
    by_category,
    by_path_family,
    by_quality_scope_family,
    by_quality_scope_owner,
    by_path_prefix,
    by_source,
  };
}
...
function buildMarkdown(report, options = {}) {
  ...
  const ownerLines = renderKeyValueBlock(
    Object.fromEntries(Object.entries(report.breakdowns.by_quality_scope_owner).map(([key, value]) => [key, value.count]))
  ) || "- none";

  const actionLines = renderKeyValueBlock(report.summary.suggested_actions) || "- none";
  const warningLines = (report.diagnostics.non_lifecycle_recall_warnings?.examples || []).length > 0
    ? report.diagnostics.non_lifecycle_recall_warnings.examples
      .map(item => `- owner=${item.owner} family=${item.family} path=${item.path} retrieved=${item.retrieved_count} injected=${item.injected_count}`)
      .join("\n")
    : "- none";
...
${renderKeyValueBlock({
  ...
  chunks_without_confidence_lifecycle_owned_count: report.diagnostics.chunks_without_confidence_lifecycle_owned_count,
  chunks_without_confidence_core_owned_count: report.diagnostics.chunks_without_confidence_core_owned_count,
  chunks_without_confidence_generated_diagnostic_count: report.diagnostics.chunks_without_confidence_generated_diagnostic_count,
  chunks_without_confidence_legacy_manual_count: report.diagnostics.chunks_without_confidence_legacy_manual_count,
  chunks_without_confidence_unknown_count: report.diagnostics.chunks_without_confidence_unknown_count,
...
- default quality score scope is ownership-aware, not path-family-only.
- retrieval visibility does not imply memory-engine confidence ownership.
...
- quality_scope_family_distribution: ${JSON.stringify(report.diagnostics.quality_scope_family_distribution || {})}
- quality_scope_owner_distribution: ${JSON.stringify(report.diagnostics.quality_scope_owner_distribution || {})}
...
- non_lifecycle_retrieved_count: ${report.diagnostics.non_lifecycle_recall_warnings?.non_lifecycle_retrieved_count ?? 0}
- non_lifecycle_injected_count: ${report.diagnostics.non_lifecycle_recall_warnings?.non_lifecycle_injected_count ?? 0}
${warningLines}

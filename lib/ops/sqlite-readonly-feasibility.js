...
function runReadOnlyChecks(result, databasePath, before, rootDir) {
  ...
  try {
    if (database) {
      ...
      const writeRejections = assertReadOnlyWrites(database);
...
function setupWalDatabase(databasePath) {
  ...
  ).run(WAL_GENERATED_AT_MS, JSON.stringify({
    [PLUGIN_ID]: {
      installPath: INSTALL_PATH,
      sourcePath: SOURCE_PATH,
      version: "0.0.0-synthetic-wal",
      installedAt: "2026-07-20T00:00:00.000Z",
      fixtureRevision: WAL_REVISION,
    },
  }), INDEX_KEY);
  return writer;
}
...
function immutableScenario(rootDir) {
  ...
  try {
    ...
    const normalObserved = readLatestRow(normal);
    const normalWriteRejections = assertReadOnlyWrites(normal);
    normal.close();
    result.sql_write_rejections = normalWriteRejections;
    result.sql_write_rejected = Object.values(normalWriteRejections).every(Boolean);
    if (!result.sql_write_rejected) result.blockers.push("read_only_sql_write_not_rejected");
    const immutableUrl = pathToFileURL(databasePath);
    immutableUrl.searchParams.set("immutable", "1");
    let immutable;
    let immutableObserved;
    try {
      immutable = new DatabaseSync(immutableUrl, { readOnly: true });
      result.immutable_open_succeeded = true;
      immutableObserved = readLatestRow(immutable);
      result.immutable_database_shape_verified = immutableObserved.query_succeeded;
    } catch (error) {
      result.immutable_open_error_code = error?.code ?? error?.constructor?.name ?? "immutable_open_error";
      result.blockers.push("immutable_uri_semantics_not_proven");
    } finally {
      try {
        immutable?.close();
      } catch (error) {
        result.immutable_close_error_code = error?.code ?? error?.constructor?.name ?? "immutable_close_error";
        result.blockers.push("immutable_close_failed");
      }
    }
    result.open_succeeded = true;
    result.query_succeeded = normalObserved.query_succeeded;
    result.normal_observed_revision = normalObserved.observed_revision;
    result.normal_latest_row_visible = normalObserved.observed_revision === result.expected_revision;
    result.observed_revision = result.normal_observed_revision;
    result.observed_generated_at_ms = normalObserved.observed_generated_at_ms;
    result.latest_row_visible = result.normal_latest_row_visible;
    result.sql_write_rejected = true;
    if (immutableObserved) {
      result.immutable_observed_revision = immutableObserved.observed_revision;
      result.immutable_latest_row_visible = immutableObserved.observed_revision === result.expected_revision;
      if (!result.immutable_latest_row_visible) result.blockers.push("immutable_reader_missed_wal_row");
    }
    const after = capture(rootDir);
    Object.assign(result, compareFingerprints(before, after));
    result.after_fingerprint = serialiseFingerprint(after);
    if (!result.normal_latest_row_visible) result.blockers.push("latest_committed_row_not_visible");
    if (result.observable_write_detected) result.blockers.push("observable_filesystem_write");
  } finally {
    writer.close();
  }

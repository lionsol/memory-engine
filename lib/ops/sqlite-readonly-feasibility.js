...
function verifyDatabaseLocation(database, databasePath, result, prefix) {
  try {
    ...
    if (!matches) {
      addBlocker(result, prefix === "normal"
...
export function classifyImmutableBehavior({
  locationMatches,
  initialQueryError,
  postQueryError,
  initialRevision,
  ...
  if (locationMatches !== true) return "uri-or-location-unproven";
  if (postQueryError) return "query-failed-after-update";
  if (initialQueryError) return "initial-query-failed";
  if (postRevision === POST_OPEN_WAL_REVISION) return "saw-post-open-update";
  if (
    initialRevision !== null
    && initialRevision !== undefined
    && postRevision === initialRevision
    && postRevision !== POST_OPEN_WAL_REVISION
  ) {
    return "retained-stale-snapshot";
  }
  return "other";
}
...
function immutableScenario(rootDir) {
  ...
  try {
    try {
      ...
      addBlocker(result, "read_only_open_failed");
    }
    ...
    try {
      ...
      addBlocker(result, "immutable_uri_semantics_not_proven");
    }
    ...
    if (result.immutable_initial_query_error_code || !immutableInitial || immutableInitial.query_succeeded !== true) {
      addBlocker(result, "immutable_initial_query_failed");
    }
    if (!result.immutable_database_shape_verified) {
      addBlocker(result, "immutable_database_shape_not_verified");
    }
    ...
    if (normal) {
      ...
      if (!result.sql_write_rejected) addBlocker(result, "read_only_sql_write_not_rejected");
    }
    ...
    try {
      ...
      addBlocker(result, "post_open_writer_update_failed");
    }
    ...
    if (result.immutable_post_update_query_error_code || !immutablePost || immutablePost.query_succeeded !== true) {
      addBlocker(result, "immutable_post_update_query_failed");
    }
    ...
    if (!result.normal_initial_latest_visible || !result.normal_post_update_latest_visible) {
      addBlocker(result, "latest_committed_row_not_visible");
    }
    try {
      normal?.close();
      normal = null;
      ...
      addBlocker(result, "normal_close_failed");
      normal = null;
    }
    try {
      immutable?.close();
      ...
      addBlocker(result, "immutable_close_failed");
      immutable = null;
    }
    ...
    addBlocker(result, `scenario_error:${result.scenario_error_code}`);
    const after = capture(rootDir);
    ...
    try {
      ...
      addBlocker(result, "writer_close_failed");
    }
  }
  ...
  if (result.observable_write_detected) addBlocker(result, "observable_filesystem_write");
  result.immutable_behavior = classifyImmutableBehavior({
    locationMatches: result.immutable_location_matches,
    initialQueryError: result.immutable_initial_query_error_code,
    postQueryError: result.immutable_post_update_query_error_code,
    initialRevision: result.immutable_initial_revision,
    postRevision: result.immutable_post_update_revision,
  });
  if (result.blockers.length) result.status = "BLOCKED";
  return result;
}

...
export function readSyntheticManifestSnapshot(rootDir) {
  ...
  try {
    ...
    if (before.nlink !== 1n) {
      snapshotError(result, "manifest_link_count_invalid");
      snapshotError(result, "manifest_hardlink");
      return result;
    }
...
export function runSyntheticManifestSmoke() {
  ...
  try {
    ...
    add("atomic-replacement", (dir) => {
      ...
      const oldSnapshotComplete = old.generation === "1" && canonicalJson(old) === decodeUtf8(oldBytes);
      const currentSnapshotComplete = current.valid && current.generation === "2";
      if (!oldSnapshotComplete || !currentSnapshotComplete || old.generation === current.generation) {
        atomicFailures.push("atomic_snapshot_mismatch");
      }
      const unexpectedFailures = [...new Set([...result.unexpected_failures, ...atomicFailures])];
      return {
        ...result,
        status: unexpectedFailures.length === 0 ? "PASS" : "BLOCKED",
        old_generation: old.generation,
        current_generation: current.generation,
        old_snapshot_complete: oldSnapshotComplete,
        current_snapshot_complete: currentSnapshotComplete,
        unexpected_failures: unexpectedFailures,
      };
    });
    for (const [id, raw] of [
    ...
    add("duplicate-manifest-key", (dir) => {
      const raw = canonicalJson(createSyntheticManifest()).replace(
    ...
    add("final-hardlink", (dir) => {
      const target = path.join(dir, "target");
      const fd = openSync(target, constants.O_CREAT | constants.O_WRONLY, 0o600);
      writeSync(fd, Buffer.from(canonicalJson(createSyntheticManifest()), "utf8"));
      closeSync(fd);
      linkSync(target, path.join(dir, MANIFEST_NAME));
      return consumerRead(dir, { valid: false });
    });
  } finally {

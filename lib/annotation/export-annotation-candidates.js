...
function defaultOutPath({ cwd = process.cwd(), format = "jsonl", now = new Date(), includeBuckets = [] } = {}) {
  const bucketSuffix = normalizeBucketList(includeBuckets).length
    ? `-${normalizeBucketList(includeBuckets).join("-")}`
    : "";
  return resolve(cwd, "reports", `annotation-candidates${bucketSuffix}-${timestampForFile(now)}.${format}`);
}
...
function hasDreamingMaintenanceLogPattern(text) {
  const value = String(text || "");
  return (
    /#\s*Deep Sleep\b/i.test(value)
    || /Repaired recall artifacts/i.test(value)
    || /Ranked .* candidate(?:\(s\)|s)? for durable promotion/i.test(value)
    || /Promoted .* candidate(?:\(s\)|s)? into MEMORY\.md/i.test(value)
  );
}

function hasDreamingCandidateStagingPattern(text) {
  const value = String(text || "");
  return (
    /-\s*Candidate:/i.test(value)
    && /confidence:/i.test(value)
    && /evidence:/i.test(value)
    && /status:\s*staged/i.test(value)
  );
}
...
function classifyBuckets(candidate, analysis) {
  ...
  if (pathFamily === "dreaming" && hasDreamingMaintenanceLogPattern(text)) buckets.add("dreaming_maintenance_log");
  if (pathFamily === "dreaming" && hasDreamingCandidateStagingPattern(text)) buckets.add("dreaming_candidate_staging");
...
function readChunkTextsFromDb(ids, options = {}) {
  if (!ids.length) return new Map();
  const configuredAccess = options.dbAccess && typeof options.dbAccess === "object"
    ? options.dbAccess
    : {};
  const withCore = configuredAccess.withCoreDbReadonly || withCoreDbReadonly;
  try {
    return withCore((db) => {
...
function resolveChunkTexts(ids, resolver, options = {}) {
  if (typeof resolver === "function") return resolver(ids);
  return readChunkTextsFromDb(ids, options);
}
...
function matchesBucketFilter(sampleBuckets, includeBuckets, excludeBuckets) {
  const sampleSet = new Set(sampleBuckets || []);
  if (includeBuckets.length && !includeBuckets.some(bucket => sampleSet.has(bucket))) return false;
  if (excludeBuckets.length && excludeBuckets.some(bucket => sampleSet.has(bucket))) return false;
  return true;
}
...
export function exportAnnotationCandidates(options = {}) {
  ...
  const includeBuckets = normalizeBucketList(options.includeBuckets);
  const excludeBuckets = normalizeBucketList(options.excludeBuckets);
  const outPath = options.out || defaultOutPath({ format, now, includeBuckets });
  ...
  const dbOptions = options.dbOptions && typeof options.dbOptions === "object"
    ? options.dbOptions
    : {};
  const dbAccess = options.dbAccess && typeof options.dbAccess === "object"
    ? options.dbAccess
    : {};
  ...
  const joinedTextState = resolveChunkTexts(
    baseCandidates.map(item => String(item?.id || "")).filter(Boolean),
    chunkTextResolver,
    { dbOptions, dbAccess },
  );
  ...
  const analyzed = candidates.map(candidate => {
    ...
    .filter(item => matchesBucketFilter(item.analysis.sampleBuckets, includeBuckets, excludeBuckets))
    .sort(sortAnalyzed);

...
function scoreCandidate(sample = {}, options = {}) {
  ...
  return {
    ...
    _threshold: scoring.threshold,
    _unsure_threshold: scoring.unsure_threshold,
    _score_parts: scoring.parts,
    _score_signals: scoring.signals,
    _score_manual_review_flags: scoring.manual_review_flags || [],
    _raw_predicted_keep_active: scoring.raw_predicted_keep_active,
...
function selectActiveSamplerSamples(samples = [], options = {}) {
  ...
  const scored = samples.map(sample => scoreCandidate(sample, { threshold, unsureThreshold }));
  const selected = [];

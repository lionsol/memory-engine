...
export function registerMemoryEngineTools(api, executors) {
  ...
  api.registerTool({
    ...
    description: [
      ...
      `episodic: 情节摘要（τ=30天）\n`,
      `\n重要：cite 只接受本轮 Search 返回的 id（或 update --hit），否则记忆会衰减。`,
    ].join(""),
    parameters: {
      type: "object",
      properties: {
        ...
        chunk_ids: {
          type: "array",
          items: { type: "string" },
          description: "List of chunk ID prefixes returned by Search in the current turn; unrestricted historical IDs are rejected",
        },
        hit: { type: "boolean" },

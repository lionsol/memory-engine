        ...
        try {
          if (action === "add") {
            ...
            try {
              ...
              const indexedChunks = runCoreDb(db => db.prepare(
              ...
              result = runEngineDb(db => {
                ...
                const newChunks = indexedChunks.filter(row => !exists.get(row.id));
            ...
            if (result.newChunks) {
              if (!lancedbTable) {
                ...
                try {
                  const exactId = String(result.newChunks[0].id);
                  ...
                  await lancedbTable.add([lanceRow]);
                  lanceWritten = 1;
                } catch (error) {

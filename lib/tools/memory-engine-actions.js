...
function createSearchRunner(runtime) {
  ...
  function recallHintDebug({ mode = "none", status, hint = null, expansionCount = 0 } = {}) {
    return {
      mode,
      status,
  ...
  async function resolveRecallHint(query) {
    const provider = retrievalPolicy.recallHintProvider;
    if (provider === undefined || provider === null) {
      return {
        vectorQueryPlan: null,
        debug: recallHintDebug({ status: "provider_absent" }),
      };
    }
    if (typeof provider !== "function") {
      return {
        vectorQueryPlan: null,
        debug: recallHintDebug({ status: "provider_invalid" }),
      };
    }
    ...
    if (!validation.valid) {
      return {
        vectorQueryPlan: null,
        debug: recallHintDebug({ mode: "recall_hint_v1", status: "invalid_hint" }),
      };
    }
  ...
  return async function runMemoryEngineSearch(params = {}, invocation = {}) {
    ...
    if (invocation.action === "memory_engine_search") {
      const recallHint = await resolveRecallHint(queryText);

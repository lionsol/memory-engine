...
function resolveExactOrUniquePrefixRows(rows, lookupId, idField) {
          ...
          if (action === "update") {
            ...
            return runEngineDb(db => {
              const matches = db.prepare([
                ...
                "WHERE substr(chunk_id, 1, length(?)) = ?",
                "ORDER BY CASE WHEN chunk_id = ? THEN 0 ELSE 1 END, chunk_id ASC",
                "LIMIT 2",
              ].join(" ")).all(lookupId, lookupId, lookupId);
  ...
  return async function runMemoryEngineGet(params = {}) {
    ...
    const coreRows = runCoreDb((db) => {
      ...
      const selectSql = [
        ...
        "FROM chunks c",
        "WHERE substr(c.id, 1, length(?)) = ?",
        "ORDER BY c.updated_at DESC, c.id ASC",
      ].join(" ");
      return db.prepare(selectSql).all(lookupId, lookupId);
    });

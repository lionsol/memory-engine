...
const { resolveBusinessTimeZone } = require("../business-time.cjs");
...
function buildScope(overrides = {}) {
  ...
  const preliminaryDescriptor = createMemoryEngineRuntimeDescriptor({ pathOverrides });
  const timeZone = resolveBusinessTimeZone({
    explicitTimeZone: normalizedOverrides.timeZone,
    env: process.env,
    config: readRuntimeConfig(preliminaryDescriptor.paths.configJsonPath),
  });
  const descriptor = createMemoryEngineRuntimeDescriptor({
    pathOverrides: { ...pathOverrides, timeZone },
  });
...
function getRuntime() {
  ...
  return Object.freeze({
    ...
    reconcileSessionFlushManagedState: overrides.reconcileSessionFlushManagedState
      || runtimeFallbacks.reconcileSessionFlushManagedState,

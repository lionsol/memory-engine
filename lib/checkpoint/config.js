...
const DEFAULT_CHECKPOINT_LLM_MAX_INPUT_CHARS = 45000;
const DEFAULT_CHECKPOINT_LLM_MAX_TOKENS = 4096;
const DEFAULT_CHECKPOINT_LLM_TIMEOUT_MS = 120000;
...
function parsePositiveIntegerWithDefault({
  ...
  warnings.push(`[checkpoint] Invalid ${key}=${JSON.stringify(rawValue)}; using default ${defaultValue}`);
  return defaultValue;
}
...
function resolveCheckpointLlmRequestConfig(env = process.env, logger = console) {
  const warnings = [];
  const maxInputChars = parsePositiveIntegerWithDefault({
    env,
    key: "MEMORY_ENGINE_CHECKPOINT_LLM_MAX_INPUT_CHARS",
    defaultValue: DEFAULT_CHECKPOINT_LLM_MAX_INPUT_CHARS,
    warnings,
  });
  const maxTokens = parsePositiveIntegerWithDefault({
    env,
    key: "MEMORY_ENGINE_CHECKPOINT_LLM_MAX_TOKENS",
    defaultValue: DEFAULT_CHECKPOINT_LLM_MAX_TOKENS,
    warnings,
  });
  const timeoutMs = parsePositiveIntegerWithDefault({
    env,
    key: "MEMORY_ENGINE_CHECKPOINT_LLM_TIMEOUT_MS",
    defaultValue: DEFAULT_CHECKPOINT_LLM_TIMEOUT_MS,
    warnings,
  });

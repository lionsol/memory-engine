...
const RESET_FILE_GRACE_MS = 24 * 60 * 60 * 1000;
...
function collectDbRawLogs(targetDate, timeZone, range, stats) {
  ...
  try {
    withMeDb((meDb) => {
      const rows = meDb
        .prepare(
          `SELECT c.id, c.text, mc.category, c.updated_at
           FROM chunks_db.chunks c
           JOIN memory_confidence mc ON c.id = mc.chunk_id
           WHERE mc.category = 'raw_log'
             AND (
               (c.updated_at >= @startSec AND c.updated_at < @endSec)
               OR
               (c.updated_at >= @startMs AND c.updated_at < @endMs)
             )
           ORDER BY CASE
             WHEN c.updated_at >= 1000000000000 THEN c.updated_at
             ELSE c.updated_at * 1000
           END ASC, c.id ASC`
        )
        .all({
      ...
      for (const row of rows) {
        ...
        if (!body) continue;
        const timestampMs = toEpochMs(row.updated_at);
        if (!isTimestampInRange(timestampMs, range)) continue;
...
function shouldConsiderSessionFile(fileName, allFiles, range, stats) {
  if (fileName.includes(".trajectory.")) {
    stats.droppedNoise.trajectoryFile += 1;
    return false;
  }
  if (!(fileName.includes(".reset.") || fileName.endsWith(".jsonl"))) return false;

  if (fileName.endsWith(".jsonl")) {
    const base = fileName.replace(/\.jsonl$/, "");
    if (allFiles.some((other) => other.startsWith(base) && other.includes(".reset."))) {
      stats.skippedResetFileCount += 1;
      stats.droppedNoise.shadowedLiveJsonl += 1;
      return false;
    }
  }
  ...
  const sameDay = mtimeMs >= range.startMs && mtimeMs < range.endMs;
  const nearBoundary = mtimeMs >= (range.startMs - RESET_FILE_GRACE_MS)
    && mtimeMs < (range.endMs + RESET_FILE_GRACE_MS);
  if (!sameDay && !nearBoundary) {
    stats.skippedResetFileCount += 1;
    stats.droppedNoise.skippedByMtime += 1;
    return false;
  }

  return { filePath, mtimeMs, sameDay };
}

function collectSessionTranscriptLogs(targetDate, timeZone, range, stats) {
  const rt = getRuntime();
  const collected = [];
  ...
  try {
    ...
    for (const fileName of allFiles) {
      const decision = shouldConsiderSessionFile(fileName, allFiles, range, stats);
      if (decision) {
        stats.resetFilesConsidered += 1;
    ...
    for (const file of sessionFiles) {
      ...
      for (const line of lines) {
        ...
        const timestampMs = extractEntryTimestampMs(entry, entry.message);
        const inTargetDate = timestampMs !== null
          ? isTimestampInRange(timestampMs, range)
          : file.sameDay;
        if (!inTargetDate) {
...
function readCheckpointRawLogs(options = {}) {
  ...
  const collected = [
    ...
    ...collectSessionTranscriptLogs(targetDate, timeZone, range, stats),
  ];

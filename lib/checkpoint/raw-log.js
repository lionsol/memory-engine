...
function chunkItems(items, batchSize = RAW_LOG_MEMBERSHIP_BATCH_SIZE) {
  const chunks = [];
...
function collectRawLogIds(engineDb, chunkIds) {
  ...
  for (const batch of chunkItems(chunkIds)) {
    ...
    for (const row of rows) {
      rawLogIds.add(String(row.chunk_id || ""));
    }
  }
  return rawLogIds;
}
...
function collectDbRawLogs(targetDate, timeZone, range, stats) {
  ...
  try {
    withCheckpointDbs(({ engineDb, coreDb }) => {
      ...
      if (timeSelector.column === "event_at") {

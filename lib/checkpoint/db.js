...
const { patchWriteGuards } = require("../db/core-write-guard.cjs");
const {
  openCoreDbReadonly,
...
function withMeDb(fn, options = {}) {
  ...
  try {
    ...
    // Compatibility path for unmigrated checkpoint callers.
    // Remove after chunks_db references reach zero.
...
function inspectCheckpointBusyTimeouts(options = {}) {
  return withCheckpointDbs(({ engineDb, coreDb }) => ({
    ...
    readonlyEngine: options.readonlyEngine !== false,
  });
}

...
const { CommandExecutionError } = require("./command-failure.js");
const { realpathSync } = require("node:fs");
const { join } = require("node:path");

const NPM_CI_RUNTIME_HEADER_ENV = Object.freeze({ npm_config_nodedir: "/runtime" });
...
class CommandRegistry {
  ...
  run(id, input = {}) {
    ...
    if (descriptor.implementation === "canonical-archive") {
      ...
      return createCanonicalArchive({ root: input.sourceRoot, archivePath: input.archivePath, tarExecutable: this.plan.tar_executable, run: command => { const result = this.spawn({ ...command, cwd: input.sourceRoot, env: fixedEnv(), timeout: 120_000, maxOutput: 256 * 1024 }); if (result.code !== 0) throw new CommandExecutionError({ operationId: id, exitCode: result.code, stdout: result.stdout, stderr: result.stderr, message: `canonical archive failed:${result.stderr}` }); return result; } });
    }
    if (descriptor.implementation === "canonical-extract") {
      ...
      return extractCanonicalArchive({ archivePath: input.archivePath, destination: input.destination, tarExecutable: this.plan.tar_executable, run: command => { const result = this.spawn({ ...command, cwd: input.destination, env: fixedEnv(), timeout: 120_000, maxOutput: 256 * 1024 }); if (result.code !== 0) throw new CommandExecutionError({ operationId: id, exitCode: result.code, stdout: result.stdout, stderr: result.stderr, message: `canonical extraction failed:${result.stderr}` }); return result; } });
    }
    ...
    if ((descriptor.acceptedCodes || [0]).includes(result.code) === false) throw new CommandExecutionError({ operationId: id, exitCode: result.code, stdout: result.stdout, stderr: result.stderr, message: `${id} failed:${result.code}:${result.stderr}` });
    return result;
  }
}
...
function createRegistry({ plan, broker, spawn, sandbox } = {}) {
  ...
  descriptors["npm.ci_candidate"] = { class: "sandbox", executable: executable("node_executable"), args: (p, i) => [p.npm_cli, "--prefix", i.candidate, "ci", "--omit=dev", "--no-audit", "--no-fund", "--cache", i.cache], cwd: (p, i) => i.candidate, env: () => NPM_CI_RUNTIME_HEADER_ENV, validateInput: (p, i, b) => { candidateInput(p, i, b); b.assertWrite(i.cache, { root: "persistent_parent", mustExist: false }); } };

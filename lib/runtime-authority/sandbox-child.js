...
const { existsSync, lstatSync, mkdirSync, readlinkSync, realpathSync, writeFileSync, rmSync } = require("node:fs");
const { dirname, resolve } = require("node:path");
const { SANDBOX_OPERATIONS } = require("./constants.js");
const { getSandboxTimeoutPolicy } = require("./timeout-policy.js");

const RESOLVER_SOURCE_ROOTS = Object.freeze(["/etc", "/run", "/var/run", "/mnt/wsl"]);
const RESOLVER_TARGET_ROOTS = Object.freeze(["/run", "/var/run", "/mnt/wsl"]);
...
function exposeResolverTarget(mountExecutable, sandboxRoot) {
  const resolverPath = "/etc/resolv.conf";
  let target;
  let source;
  try {
    if (!lstatSync(resolverPath).isSymbolicLink()) return null;
    target = resolve(dirname(resolverPath), readlinkSync(resolverPath));
    source = realpathSync(resolverPath);
    if (!isWithinAnyRoot(target, RESOLVER_TARGET_ROOTS) || !isWithinAnyRoot(source, RESOLVER_SOURCE_ROOTS) || !lstatSync(source).isFile()) throw new Error("resolver target outside allow-list");
  } catch {
    throw new Error("sandbox resolver target unavailable");
  }
  bindReadOnly(mountExecutable, sandboxRoot, source, target.slice(1));
...
function setupNamespace(args) {
  ...
  exposeResolverTarget(args.mountExecutable, sandboxRoot);
...
function runInsideChroot(args, sandboxRoot, childArgs, env, innerTimeoutMs) {
  ...
  return execFileSync(args.chrootExecutable, [sandboxRoot, "/runtime/bin/node", "-e", code], {
    shell: false, timeout: innerTimeoutMs, maxBuffer: 256 * 1024,
...
function main(argv = process.argv.slice(2)) {
  ...
  const innerTimeoutMs = getSandboxTimeoutPolicy(operation).inner_timeout_ms;
  ...
  const output = runInsideChroot(args, sandboxRoot, childArgs, env, innerTimeoutMs);

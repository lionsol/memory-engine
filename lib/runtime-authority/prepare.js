...
const { claimRun, updateClaim, writeFailureEvidence, writeJson, writeChecksums } = require("./evidence.js");
...
async function prepareRuntimeAuthority({ planPath, now = new Date(), toolInspector = inspectPlanTools, toolRunner, sandbox = null, hooks = null, clock = () => new Date().toISOString(), claimUpdater = updateClaim, restorePublication = null } = {}) {
  ...
  try {
    ...
    context = { plan, planPath, planSha256: loaded.planSha256, broker, owned, tools: preMutationTools, stagingRoot: paths.staging, finalRoot: paths.final, journal: [] };
    ...
    failureStage = "AUTHORITY_ASSEMBLED";
    ...
    try {
      writeFailureEvidence({ parent: plan.persistent_parent, runId: plan.run_id, planSha256: loaded.planSha256, journalStage: failureStage, error, stagingRoot: paths.staging, broker });
    } catch { /* preserve the original failure if evidence capture fails */ }
    if (finalRenamed) {
      try {

...
class SandboxRunner {
  ...
  probe() {
    ...
    const timeoutPolicy = getSandboxTimeoutPolicy("capability-probe");
    ...
    try {
      const result = this.spawn({ ...command, cwd: root, env: { PATH: "/usr/bin:/bin", LC_ALL: "C", TZ: "UTC" }, timeout: timeoutPolicy.outer_timeout_ms, maxOutput: 64 * 1024 });
  ...
  run(operation, { executable, args, cwd, env }) {
    ...
    const timeoutPolicy = getSandboxTimeoutPolicy(operation);
    ...
    const result = this.spawn({ ...command, cwd: this.stagingRoot, env: { PATH: "/usr/bin:/bin", LC_ALL: "C", TZ: "UTC" }, timeout: timeoutPolicy.outer_timeout_ms, maxOutput: 256 * 1024 });
    if (result.code !== 0) throw new CommandExecutionError({ operationId: operation, exitCode: result.code, stdout: result.stdout, stderr: result.stderr, message: `sandbox operation failed:${operation}:${result.code}:${result.stderr}` });
    return result;
  }
}

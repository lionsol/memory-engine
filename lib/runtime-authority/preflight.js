...
function parseServiceOutput(output) {
  ...
  for (const line of String(output || "").split(/\r?\n/).filter(Boolean)) {
    ...
    if (separator <= 0) throw new Error("invalid named systemd status output");
    const key = line.slice(0, separator);
    if (Object.prototype.hasOwnProperty.call(fields, key)) throw new Error(`duplicate systemd status field:${key}`);
    fields[key] = line.slice(separator + 1);
  }
  const required = ["ActiveState", "SubState", "MainPID", "NRestarts"];
  const actual = Object.keys(fields).sort();
  const expected = [...required].sort();
  if (actual.length !== expected.length || actual.some((key, index) => key !== expected[index])) throw new Error("exact systemd status fields required");
  if (!/^\d+$/.test(fields.MainPID) || !/^\d+$/.test(fields.NRestarts)) throw new Error("invalid systemd status counters");
  const pid = Number(fields.MainPID);
  const restartCount = Number(fields.NRestarts);
  if (!Number.isSafeInteger(pid) || !Number.isSafeInteger(restartCount)) throw new Error("invalid systemd status counters");
  return {
    active: fields.ActiveState === "active",
    running: fields.SubState === "running",
    pid,

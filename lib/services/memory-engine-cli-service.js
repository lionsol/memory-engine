...
function createDefaultCliRuntime(options = {}) {
  ...
  const { withCoreDb, withEngineDbWritable, withHybridDbAccessScope } = database;
  ...
  const recordMemoryEvent = event => {
    try {
      withEngineDbWritable(db => insertMemoryEvent(db, event, { defaultSource: null }));
    } catch (error) {

...
function selectAutoRecallSource(layers, errors) {
  for (const layer of layers) {
    if (!layer.valid) return { value: {}, blocked: true };
    if (layer.value.autoRecall === undefined) continue;
    const value = layer.value.autoRecall;
    if (!isObject(value)) {
      errors.push(layer.name === "apiConfig" ? "invalid_object:apiConfig.autoRecall" : "invalid_object:autoRecall");
...
function findConfiguredSource(source, paths, errors) {
  for (const path of paths) {
    ...
    if (!isObject(container)) {
      errors?.push(`invalid_object:apiConfig.${containerName}`);
...
export function resolveEffectiveHybridRuntimeConfig({
  ...
  validateConfigContainer(global, "autoRecall", "apiConfig.autoRecall", errors);

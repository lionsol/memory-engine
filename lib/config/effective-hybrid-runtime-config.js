...
export function normalizeEvidenceWindow(value, errors) {
  const source = value === undefined || value === null ? {} : value;
  if (!isObject(source)) {
  ...
  const enabled = normalizeBoolean(source.enabled, false, errors, "productionEvidenceWindow.enabled");
  const epochId = typeof source.epochId === "string" && source.epochId.trim() ? source.epochId.trim() : null;
  if (enabled && !epochId) errors.push("missing_string:productionEvidenceWindow.epochId");
  return {
    enabled,
    epochId,
  };

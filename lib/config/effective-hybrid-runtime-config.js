...
function normalizeRecallHintRuntimeCanary(value, field, errors) {
  ...
  let valid = true;
  const allowed = new Set(["enabled", "sessionIds", "vectorExecutionMode", "executionProbe"]);
  for (const key of Object.keys(value)) {
    if (!allowed.has(key)) {
      errors.push(`unknown_field:${field}.${key}`);
      valid = false;
  ...
  const executionProbe = value.executionProbe === undefined || value.executionProbe === null
    ? null
    : value.executionProbe;
  if (executionProbe !== null && executionProbe !== "rh_l3_canonical_v1") {
    errors.push(`invalid_probe:${field}.executionProbe`);
    valid = false;
  }
  if (executionProbe !== null && vectorExecutionMode !== "parallel") {
    errors.push(`invalid_probe_mode:${field}.executionProbe`);
    valid = false;
  }
  return {
    present: true,
    ...
    value: {
      ...
      vectorExecutionMode: ["sequential", "parallel"].includes(vectorExecutionMode)
        ? vectorExecutionMode
        : "sequential",
      executionProbe: executionProbe === "rh_l3_canonical_v1"
        ? executionProbe
        : null,
    },
  };

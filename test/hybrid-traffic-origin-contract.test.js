...
test("before_tool_call contract fixture uses only host-provided identity fields", () => {
  ...
  const hookStart = lifecycleSource.indexOf('api.on("before_tool_call"');
  const hookEnd = lifecycleSource.indexOf('if (autoRecallConfig?.enabled', hookStart);
  assert.ok(hookStart >= 0 && hookEnd > hookStart);
  const hookSource = lifecycleSource.slice(hookStart, hookEnd);

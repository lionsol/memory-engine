...
function makeProductionFixture({ lifecycle = false, native = { sqlite: true, lancedb: true }, sqliteLoadThrows = false, lancedbLoadThrows = false, mutateRuntime = false, omitPackedRuntimeFile = null, externalNodeGypLink = null, stabilityDrift = null } = {}) {
  ...
  writeText(systemctl, [
    ...
    `gateway_pid) [ \"$2\" = \"synthetic-gateway.service\" ] && pid=999 ;;`,
    `gateway_restart) [ \"$2\" = \"synthetic-gateway.service\" ] && restarts=1 ;;`,
    `console_pid) [ \"$2\" = \"synthetic-console.service\" ] && pid=998 ;;`,
    `console_restart) [ \"$2\" = \"synthetic-console.service\" ] && restarts=1 ;;`,
    "esac; fi",
    "printf '%s\\n%s\\n%s\\n%s\\n' \"$active\" \"$running\" \"$pid\" \"$restarts\"",
    "",
  ].join("\n"), 0o755);

...
- [x] 4.2 Migrate the production DIRECT_CARD capability/admissibility/selector/card path and selection-derived telemetry under the separately authorized D.3-D.4-H2-E source stage (`PASS / SOURCE IMPLEMENTED / REPOSITORY-TESTED`); the event-local `senderIsOwner === true` host contract is consumed fail-closed, exact current Canonical/ProjectionArtifact/active `OWNER_EXPLICIT_ATTESTATION` evidence is required, and card-mode failures do not fall back to raw text. Implementation record: `docs/direct-card-pre-prompt-owner-audience-consumer-implementation-v1.md`.
- [x] 4.3 Deployment/runtime qualification **PASS / RUNTIME QUALIFIED / CLOSED** on 2026-08-24; deployed OpenClaw H2 `755ff5d396102c5a93baacf2d3bf6187a4fea713`, memory-engine qualification source `ae95fa25fed03c4fb414d38db9e010c0157c0a15`, and the same Owner/session/memory/query 0→1→0 attestation causal chain are recorded in `docs/direct-card-runtime-qualification-v1.md`. Cleanup restored AutoRecall/card-first disabled and zero active attestations.

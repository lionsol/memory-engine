...
- [x] 4.1.2 Decide the v1 `DIRECT_CARD` safe-to-disclose authority (`D.3-D.2 AUTHORITY DECISION: PASS / DECISION CLOSED`; sole positive authority: `OWNER_EXPLICIT_ATTESTATION` bound to the exact Canonical Memory and `DISCLOSURE_CARD` projection; production source migration remains `HOLD` with blocker `PRODUCTION_DISCLOSURE_ATTESTATION_PROVIDER_NOT_IMPLEMENTED`; docs/OpenSpec-only decision record `docs/direct-card-safe-to-disclose-authority-decision-v1.md`; no source, schema, runtime, deployment, configuration, DB/data, persistent-state, fixture, evaluator, or holdout mutation).

...
function usage() {
  return `Usage:\n  node bin/build-sustained-runtime-boundary-report.js\n    --config <openclaw-config.json>\n    [--checked-at <canonical-UTC-ISO>] [--out <report.json>] [--pretty]\n\nThe report reproduces active-memory's real default-enabled semantics and emits only booleans/status, never the raw configuration or secrets.`;
}
...
async function buildSustainedRuntimeBoundaryCli(argv = process.argv.slice(2)) {
  const options = parseArgs(argv);
  if (options.help) return { exitCode: 0, output: usage(), report: null, out: null };
  const { buildSustainedRuntimeBoundaryReport } = await import("../lib/recall/hybrid/sustained-runtime-boundary.js");
...
async function main(argv = process.argv.slice(2)) {
  try {
    const result = await buildSustainedRuntimeBoundaryCli(argv);
...
module.exports = { parseArgs, loadConfig, buildSustainedRuntimeBoundaryCli, main };

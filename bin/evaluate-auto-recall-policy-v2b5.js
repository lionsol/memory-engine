...
const DEFAULT_DATASET = "test/fixtures/auto-recall-policy-holdout.v2b5.jsonl";
...
function printHelp() {
  console.log([
    "Evaluate the AutoRecall v2-B5 holdout offline",
    "",
    "Usage:",
    "  node bin/evaluate-auto-recall-policy-v2b5.js [options]",
    "",
    "Options:",
...
function renderSummary(report, datasetPath) {
  ...
  const lines = [
    "# AutoRecall v2-B5 Fresh Independent Holdout Evaluation",
    "",
    "- dataset: " + datasetPath,
    ...
    "- runtime_false_positive_count: " + report.diagnostics.runtime_false_positive_count,
    "- runtime_false_negative_count: " + report.diagnostics.runtime_false_negative_count,
    "- oracle_gate: " + readiness.oracle_gate_pass,
    "- quantitative_gate: " + readiness.quantitative_gate_pass,
    "- family_gate: " + readiness.family_gate_pass,
    "- B5 status: " + report.current_evaluation_status,
...
async function main(argv = process.argv.slice(2)) {
  try {
    ...
    const { evaluateAutoRecallPolicyHoldoutV2B5Jsonl } = await import(
      "../lib/recall/auto-recall-policy-holdout-v2b5.js"
    );
    const datasetPath = resolve(process.cwd(), options.dataset);
    const content = readFileSync(datasetPath, "utf8");
    const report = evaluateAutoRecallPolicyHoldoutV2B5Jsonl(content);

...
const { buildRuntimeArtifactManifestV2 } = require("./runtime-artifact-manifest-v2-lib.cjs");

function usage() {
  return [
    "Usage:",
    "  node bin/build-runtime-artifact-manifest-v2.cjs --root <directory> [--checked-at <ISO>] [--out <file>] [--pretty]",
    "",
    "Builds a v2 artifact manifest with separate semantic, hardlink-topology, and exact identities.",
    "Invalid roots and unsafe entries produce a fail-closed manifest and exit status 2.",
  ].join("\n");
}
...
function parseArgs(argv = []) {
  ...
  if (args.checkedAt && Number.isNaN(Date.parse(args.checkedAt))) throw new Error("--checked-at must be a valid ISO timestamp");
  return args;
}
...
function main(argv = process.argv.slice(2)) {
  ...
  const report = buildRuntimeArtifactManifestV2({
    rootDir: args.root,

...
import { runRhL1DeterministicEquivalenceV1 } from "../lib/benchmark/rh-l1-deterministic-equivalence-v1.js";
...
export async function main() {
  ...
  const execution = await runRhL1DeterministicEquivalenceV1({ corpus, contract });
  const result = {
    status: execution.status,
    mode: "RH_L1_E1_DETERMINISTIC_EXECUTION_EQUIVALENCE",
...
if (import.meta.url === `file://${process.argv[1]}`) {
  main().then(result => {
    ...
    process.stderr.write(`${JSON.stringify({
      status: "FAIL",
      code: error?.code || "RH_L1_E1_EXECUTION_FAILED",
